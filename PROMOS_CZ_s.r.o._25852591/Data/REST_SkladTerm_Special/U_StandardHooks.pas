...
begin
  if AModule='STD_TransferQueue' then
   Result := QuotedStr('N') else
   Result := QuotedStr('A');
end;
...
begin
  Result := '';
    if AModule = 'STD_BillOfDeliveryWithoutDoc' then begin
      OValues.Edit;
      OValues.Append;
...
begin
  Result := False;
  if AModule='STD_BillOfDeliveryWithoutDoc' then Result:=true;
end;
...
var
 ...
 mCLSID:string;
begin
  if AModule in ['STD_ReceiptCardQueue','STD_BillOfDeliveryQueue','STD_TransferQueue','STD_InfoFromStores'] then begin
   mList:=TStringList.create;
   mList.add(aRow.S['StoreCard_ID']);
   mclsid:=aos.SQLSelectFirstAsString('SELECT DataSource FROM Reports WHERE ID='+Quotedstr('EL11000101'),'');
   mcount:=trunc(Arow.D['UnitQuantity']);
   //for i:=1 to mcount do begin
     CFxReportManager.PrintByIDs(NxCreateContext(AOS),mList,mCLSID,'EL11000101',rtoPrint,pekPDF,'Datamax_E-4205e', '', mcount,false);
   //end;
   ARow.SaveToFile('c:\ABRA_logs\json\data.json');
  end;
end;
