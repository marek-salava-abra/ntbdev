function GetInputForm(ASite: TSiteForm; var ADateFrom: TDate; var ADateTo: TDate): integer;
var
  mForm: TForm;
  mWizTitle: string;
  mWizText: string;
  mDtDateFrom, mDtDateTo: TDateEdit;
  mBtOK: TButton;

begin
  Result := 2;
  mWizTitle := 'Vstupní parametry';
  mWizText := 'Zadejte počáteční a koncové datum reportu';

  try
    mForm := CreateFormDialog('RepInputForm', mWizTitle, 350, 250, ASite);
    CreateHeadPanel('RepWizard', mWizTitle, mWizText, 350, 80, mForm);

    CreateLabel('RepLbDateFrom', 'Datum od:', 50, 100, -1, -1, nil, mForm);
    CreateLabel('RepLbDateTo', 'Datum do:', 50, 125, -1, -1, nil, mForm);


    mDtDateFrom := CreateDateEdit('RepDateFrom', '', 200, 100, 100, 0, ADateFrom, mForm);
    mDtDateTo := CreateDateEdit('RepDateTo', '', 200, 125, 100, 0, ADateTo, mForm);

    CreateButton('RepOK', 'OK', 1, 150, 170, 80, 25, true, mForm);
    CreateButton('RepCancel', 'Storno', 2, 240, 170, 80, 25, false, mForm);

    Result := mForm.ShowModal(ASite.GetParentForm);

    // naplnění hodnot
    ADateFrom := mDtDateFrom.Date;
    ADateTo := mDtDateTo.Date;
