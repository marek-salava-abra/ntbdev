...
procedure listStorePositionsForInv(AOS: TNxCustomObjectSpace; APath, AQueryParams, AResponse: TStringList);
var
  mSearchStr, doc_ID: String;
  dtRows: TMemTable;
  mSql: String;
...
begin
  ...
  try
    ...
    dtJSONBatches.Open;
    REST_JsonToDataSet(jsonI.AsArray, dtJSONBatches);
    dtJSONBatches.First;
    i := 0;
        ...
        try
          ...
          begin
            try
              ...
              jsonI.AsArray.O[i].S('StoreBatch_ID') := boStoreBatch.OID;
            except
...
////////////////////////////////////////////////////////////////////////////////
procedure listPartialInvProtocols(AOS: TNxCustomObjectSpace; APath, AQueryParams, AResponse: TStringList);
var
  dtRows: TMemTable;
...
////////////////////////////////////////////////////////////////////////////////
procedure listMainInvProtocols(AOS: TNxCustomObjectSpace; APath, AQueryParams, AResponse: TStringList);
var
  dtRows: TMemTable;
  mSql, mSearchStr: String;
  json: TJSONSuperObject;
...
begin
  ...
  try
    ...
    dialogJSON.S['text'] := DialogOnDocSave(AOS, gSkladTermDocType, gSkladTermModule, gSkladTermUser_ID, mDoc_ID, mDialogValues);
...
procedure listStoreCardsForInv(AOS: TNxCustomObjectSpace; APath, AQueryParams, AResponse: TStringList);
var
  mSearchStr: String;
  dtRows: TMemTable;
  mSql, sDocId: String;
  ...
  try
    ...
    try
      ...
      try
        ...
        mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_PartialInvProtocol, gSkladTermUser_ID, mJSON_Root, DOC_MainInvProtocol, boMIP);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if CFxOID.IsEmpty(sStoreBatchID) then
                  sStoreBatchID := CreateSerialNumber(AOS, fdStoreCard.AsString, jsonSerNums.O[j].S['SerNumName'], '');
              end
              else begin
                sStoreBatchID := jsonSerNums.O[j].S['SerNum_ID'];
              end;
      ...
      begin
        AddPhotos(AOS, gSkladTermDocType, boPIP.OID, mJSON_Root.A['documents']);
      end;
