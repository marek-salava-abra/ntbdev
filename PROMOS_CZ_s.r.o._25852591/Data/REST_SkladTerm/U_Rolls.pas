...
begin
  ...
  try
    mSql :=
      ...
      '  F.ID as "firm.id",' + nxCrLf +
      '  F.Code as "firm.code",' + nxCrLf +
      '  ' + get_FirmInfo_NameField(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID) + ' as "firm.name",' + nxCrLf +
      '  D.ID as "division.id",' + nxCrLf +
      '  D.Code as "division.code",' + nxCrLf +
      '  D.Name as "division.name"' + nxCrLf +
      'from BusOrders BO' + nxCrLf +
      'left join Firms F on F.ID = BO.Firm_ID' + nxCrLf +
      'left join Divisions D on D.ID = BO.Division_ID' + nxCrLf +
      'where' + nxCrLf +
...
begin
  ...
  dtHeader := TMemTable.Create(nil);
  LogWriteSectionStart('get_BusProjectInfo');
  try
    mSql :=
      'select' + nxCrLf +
      '  BP.ID as "id",' + nxCrLf +
...
function get_ListBusOrders(AOS: TNxCustomObjectSpace; ASearch: String): String;
var
...
begin
  ...
  try
    ...
    try
      ...
      FilterList(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, 'get_ListBusOrders', mParameters);
...
function get_ListBusProjects(AOS: TNxCustomObjectSpace; ASearch: String): String;
var
  mSelect, mFrom, mWhere, mJoin, mOrderBy: String;
  mSearchStr: String;
  mSql: String;
...
begin
  ...
  try
    ...
    try
      ...
      if trim(ASearch) <> '' then
      begin
        if (length(ASearch) = 12) and (copy(ASearch, 1, 2) = 'BP') then
          mSearchStr := copy(ASearch, 3, 10);
        mWhere := mWhere + nxCrLf +
      ...
      FilterList(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, 'get_ListBusProjects', mParameters);
...
function get_ListBusTransactions(AOS: TNxCustomObjectSpace; ASearch: String): String;
var
  mSelect, mFrom, mWhere, mJoin, mOrderBy: String;
  mSearchStr: String;
  mSql: String;
...
begin
  ...
  try
    ...
    try
      ...
      if trim(ASearch) <> '' then
      begin
        if (length(ASearch) = 12) and (copy(ASearch, 1, 2) = 'BT') then
          mSearchStr := copy(ASearch, 3, 10);
        mWhere := mWhere + nxCrLf +
      ...
      FilterList(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, 'get_ListBusTransactions', mParameters);
...
function get_ListDivisions(AOS: TNxCustomObjectSpace; ASearch: String): String;
var
  mSelect, mFrom, mWhere, mJoin, mOrderBy: String;
  mSql: String;
  mParameters: TStringList;
begin
  ...
  try
    ...
    try
      ...
      FilterList(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, 'get_ListDivisions', mParameters);
...
begin
  ...
  try
    ...
    case mRollType of
      ...
      'BUS_ORDER': mSql := get_ListBusOrders(AOS, mSearchStr);
      'BUS_PROJECT': mSql := get_ListBusProjects(AOS, mSearchStr);
      'BUS_TRANSACTION': mSql := get_ListBusTransactions(AOS, mSearchStr);
//      'CUSTOMER': get_
//      'DIALOG'
      'DIVISION': mSql := get_ListDivisions(AOS, mSearchStr);
      ...
      'STORE_CARD': mSql := listStoreCards(AOS, mSearchStr, mStoreId, mStorePositionId, mAllowedIds);
