...
procedure listDocQueue(AOS: TNxCustomObjectSpace; APath, AQueryParams, AResponse: TStringList);
var
  dtRows: TMemTable;
...
var
  mSql, mDocTypeB, mDoc_ID, mAuxField, mPerson_ID, mDocType, mAdditionalValues,
  ...
  json, dialogJSON, rowDialogJson: TJSONSuperObject;
  mRows: TNxCustomBusinessMonikerCollection;
  i, mTempId: Integer;
  dtHeader, dtStoreDocuments: TMemTable;
  dtRows, mDialog, mDialogValues, mRowDialog, mRowDialogValues: TMemTable;
  mSL: TStringList;
...
begin
  ...
  try
    ...
    try
      ...
      if (mDocType = DOC_LogStoreTransfer)
        or ((mDocType = DOC_JobOrder) and ABRA) then
      begin
        if mDocType = DOC_LogStoreTransfer then
        begin
          if mBO.GetFieldValueAsBoolean('Executed') then
            RaiseException(getString('error_already_processed_document'));
        end
        else if mDocType = DOC_JobOrder then
        begin
          if not ((mBO.GetFieldValueAsFloat('ReleasedAt$DATE') > 0)
              and (mBO.GetFieldValueAsFloat('FinishedAt$DATE') = 0)) then
            RaiseException(getString('error_joborder_not_in_right_state'));
        end;
        ...
        mSql :=
          ...
          '  and RTS.Document_ID = ' + QuotedStr(mDoc_ID) + nxCrLf +
          '  and ((RTS.Status = ' + IntToStr(TempStorageStatus_OPEN) +
          '      and RTS.User_ID <> ' + QuotedStr(gSkladTermUser_ID) + ')' + nxCrLf +
        ...
        if (mDocType = DOC_LogStoreTransfer) and not ABRA then
        begin
      ...
      begin
        ...
        if mDocType in [DOC_ShippingList, DOC_RemovalList] then
          ...
          mWhere := '  SD.ID = ' + QuotedStr(mDoc_ID) + nxCrLf
        else
        ...
        if mDocType = DOC_IssuedOrder then
          ...
          mWhere := mWhere +
            '  and N.Issue = 0' + nxCrLf +
            '  and (cast((cast(SD2.Quantity / SD2.UnitRate as double precision) * cast(mi.Quantity / MI.UnitRate as double precision)' + nxCrLf +
            '    * (1 + (cast(SD2.WastePercentage as double precision) / 100))) as  Numeric(18,3)) - coalesce(A.Quantity, 0) + coalesce(B.Quantity, 0)) > 0' + nxCrLf
        ...
        // dialog na radku
        mAdditionalValues := GetInputDialogValuesFields(mOS, gSkladTermModule, mDocType, gSkladTermUser_ID, 'rows', mRowDialogValues);

        mSql := getRowsSql(mOS, gSkladTermModule, gSkladTermUser_ID, mDocType, mAdditionalValues, mWhere);
        ...
        begin
          ...
          SetInputDialogValuesString(mRowDialogValues, dtRows);
...
var
  ...
  PLMContainerMater_ID, mProvideRow_ID: String;
  ...
  i, j, k, l: Integer;
  mSDNewFree, mPrePriFree, mFound, mDamagedFound, mAllProcessed: Boolean;
  ...
  procedure FillLimitedStoreBatch(ADocRowBatches: TNxCustomBusinessMonikerCollection; ARowId: String;
    AProcessed: Boolean);
  var
    mQuantity: Double;
    ...
    i: Integer;
  begin
    for i := 0 to ADocRowBatches.Count - 1 do
    begin
      mDocRowBatch := ADocRowBatches.BusinessObject(j);
      ...
      mQuantity := getSumUnitQuantityForSD2_IDAndBatch_ID(dtJSONRows, ARowId, dtJSONRows.FieldByName('StoreBatch_ID').AsString, AProcessed, False,
  ...
  procedure FillStoreBatches(ARow: TNxCustomBusinessObject; AProcessed: Boolean);
  var
    mStoreBatch: TNxCustomBusinessObject;
  ...
  begin
    ...
    begin
      FillLimitedStoreBatch(mDocRowBatches, ARow.OID, AProcessed);
    end
    else
    begin
      ...
      begin
        ...
        // vezmu si mnozstvi pro tuto sarzi ze vsech radku s ni
        // ulozim si id sarze, abych vedel, ze uz jsem ji zpracoval a dalsi radek s ni preskocil
        mQuantity := getSumUnitQuantityForSD2_IDAndBatch_ID(dtJSONRows, ARow.OID, dtJSONRows.FieldByName('StoreBatch_ID').AsString, AProcessed, False,
          mDocRowBatch.GetFieldValueAsFloat('UnitRate'));
        mProcessedBatches.Append(dtJSONRows.FieldByName('StoreBatch_ID').AsString);
        ...
        afterDocRowBatchFill(AOS, mModule, mDocRowBatch, ARow, mSD, dtJSONRows);
      end
  ...
  begin
    ...
    if not putQueueDocDetailStopPicking_FillSerialNumbers(AOS, mModule, jsonSerNums, ARow, mDocRowBatches) then
    begin
      ...
      begin
        ...
        begin
          ...
          for i := 0 to mDocRowBatches.Count - 1 do
          begin
            ...
            begin
              if mDocRowBatch.GetFieldValueAsString('StoreBatch_ID') = jsonSerNums.O[j].S['SerNum_ID'] then
              begin
                mFound := True;
                break;
              end;
  ...
  begin
    ...
    if mDocType = DOC_IssuedOrder then
    begin
      mDocQueue_ID := GetDocQueue_ID(AOS, mModule, DOC_ReceiptCard, mUser_Id, json, mDocType, mSD);
      mDocQueue_ID := GetValueOrDefault(mDocQueue_ID, GetDefaultDocQueue_ID(DOC_ReceiptCard));
    end
    else if mDocType = DOC_JobOrder then
    begin
      mDocQueue_ID := GetDocQueue_ID(AOS, mModule, DOC_MaterialDistribution, mUser_Id, json, mDocType, mSD);
      mDocQueue_ID := GetValueOrDefault(mDocQueue_ID, GetDefaultDocQueue_ID(DOC_MaterialDistribution));
    end
    else
    begin
      mDocQueue_ID := GetDocQueue_ID(AOS, mModule, mDocType, mUser_Id, json, mDocType, mSD);
    ...
    if mDocType in [DOC_IssuedOrder, DOC_JobOrder] then
    begin
      ...
      SaveDialogValues(mSDNew, json.O['Dialog'].A['values']);
  ...
  var
    mOriginSDField, mProductionTask_ID: String;
    mStoreBatchNoteField: String;
  begin
    mProductionTask_ID := '';
    ...
    if not (mDocType in [DOC_IssuedOrder, DOC_JobOrder]) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            mSDNewRow := mSDNewRows.BusinessObject[i];
            if mSDNewRow.GetFieldValueAsString(mProvideRowField) = dtJSONRows.FieldByName('StoreDocument2ProvideRow_ID').AsString then
            begin
              SaveDialogValues(mSDNewRow, json.A['rows'].O[dtJSONRows.FieldByName('jsonIndex').AsInteger].O['dialog'].A['values']);
              ...
              if (mDocType = DOC_JobOrder) and CFxOID.IsEmpty(mProductionTask_ID) then
                mProductionTask_ID := mSDNewRow.GetFieldValueAsString('ProductionTask_ID');
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if (mDocType in [DOC_RefundedBillOfDelivery, DOC_RefundedReceiptCard]) then
                        FillLimitedStoreBatch(mDocRowBatches, mSDNewRow.OID, AProcessed)
                      else
                      begin
                        // muze se stat, ze je jedna sarze rozdelena polohovakem na vice radku, proto ji nesmim pridavat vicekrat
                        if dtJSONRows.FieldByName('StoreBatch_ID').AsString <> mLastStoreBatch_ID then
                ...
                else if mDocType in [DOC_IssuedOrder, DOC_JobOrder] then
                  GenerateAndFillStoreBatch(mSDNewRow);
              end;

              fillSerialNumbers(mSDNewRow);
    ...
    if mDocType in [DOC_IssuedOrder, DOC_JobOrder] then
      mSDNewRows_ID := AddNewRows(mSDNew, mSDNewRows, mProductionTask_ID);
    ...
    begin
      LogWriteEvent('Before beforeSaveHook');
      beforeSaveHook(AOS, mModule, mDocType, mUser_Id, mSDNew, 1, json, dtJSONRows);
      LogWriteEvent('After beforeSaveHook');

      mSDNew.Save;

      LogWriteEvent('Before afterSaveHook');
      afterSaveHook(AOS, mModule, mDocType, mUser_Id, mSDNew, 1, json, dtJSONRows);
      LogWriteEvent('After afterSaveHook');

      if mDocType in [DOC_IssuedOrder, DOC_JobOrder] then
      begin
        LogWriteEvent('DOC_IssuedOrder section entered');
        LogWriteEvent('Checking CanTakePhotos');
        if REST_getJSONBool(json, 'CanTakePhotos') then
        begin
          AddPhotos(AOS, DOC_ReceiptCard, mSDNew.OID, json.A['documents']);
        end;
      end;
      ...
      if (dtDocumentQuantity.RecordCount > 0)
        and (putQueueDocDetailStopPicking_CreateLogStoreDocumentForNotPickedRows(AOS, mModule, mDocType, mUser_Id)
          or (mDocType in [DOC_IssuedOrder, DOC_JobOrder]))
      then
      begin
        mDocQueue_ID := GetDocQueue_ID(AOS, mModule, mLogStoreDocumentDocType, mUser_ID, json, mDocType, mSDNew);
        ...
        if (mDocType in [DOC_IssuedOrder, DOC_JobOrder])
            and MakeExecuteLogStoreDocument(AOS, mModule, mLogStoreDocumentDocType, mUser_Id, mLSDNotProcessed_ID) then
  ...
  function AddNewRows(ADocument: TNxCustomBusinessObject; ARows: TNxCustomBusinessMonikerCollection; AProductionTask_ID: String = ''): String;
  var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if AProductionTask_ID <> '' then
          mMySDRow.SetFieldValueAsString('ProductionTask_ID', AProductionTask_ID);
        ...
        FillCustomFields(mMySDRow, dtJSONRows, json);
  ...
  procedure findProvideIDs(var AOrders, AOrdersRows, AOrdersDamaged, AOrdersDamagedRows, AJOs, ACMs: TStringList;
    var ADamagedFound: Boolean; var AAllProcessed: Boolean);
  var
  ...
  begin
    ...
    while not dtJSONRows.EOF do
    begin
      if AAllProcessed and not dtJSONRows.FieldByName('Processed').AsBoolean then
        AAllProcessed := False;
      ...
      if(((mDocType in [DOC_IssuedOrder, DOC_JobOrder]) and dtJSONRows.FieldByName('Processed').AsBoolean)
        or (not (mDocType in [DOC_IssuedOrder, DOC_JobOrder]) and (not dtJSONRows.FieldByName('Processed').AsBoolean)
  ...
  function CreateNewStoreDocumentSA(ASD: TNxCustomBusinessObject): TNxCustomBusinessObject;
  var
    mSDNew: TNxCustomBusinessObject;
  begin
    LogWriteSectionStart('CreateNewStoreDocumentSA');
    // pokud neni OBP, zalozime volny doklad
    ...
    mSDNew.SetFieldValueAsString('Currency_ID', mSD.GetFieldValueAsString('Currency_ID'));
    mSDNew.SetFieldValueAsString('AccPresetDef_ID', mSD.GetFieldValueAsString('AccPresetDef_ID'));
    mSDNew.SetFieldValueAsString('AccDocQueue_ID', mSD.GetFieldValueAsString('AccDocQueue_ID'));
    mSDNew.SetFieldValueAsInteger('TradeType', mSD.GetFieldValueAsInteger('TradeType'));
    mSDNew.SetFieldValueAsString('TransportationType_ID', mSD.GetFieldValueAsString('TransportationType_ID'));
    mSDNew.SetFieldValueAsString('VATCountry_ID', mSD.GetFieldValueAsString('VATCountry_ID'));
    Result := mSDNew;
    LogWriteSectionEnd;
  end;

  function CreateNewStoreDocumentSAByClone(ASDOriginal: TNxCustomBusinessObject): String;
  var
    mSDNew: TNxCustomBusinessObject;
    mSDNewRows: TNxCustomBusinessMonikerCollection;
  begin
    LogWriteSectionStart('CreateNewStoreDocumentSAByClone');
    mSDNew := ASDOriginal.Clone;
    try
      mSDNew.ExplicitTransaction := True;
      if ABRA then
        mSDNew.SetFieldValueAsString('Address_ID', mSD.GetFieldValueAsString('Address_ID'));
      mSDNewRows := mSDNew.GetLoadedCollectionMonikerForFieldCode(mSDNew.GetFieldCode('rows'));
      mSDNewRows.MarkForDeleteAll;
      mSDNew.Save;
      Result := mSDNew.OID;
    finally
      mSDNew.Free;
  ...
  try
    ...
    try
      ...
      begin
        ...
        SaveDialogValues(mBO, json.O['Dialog'].A['values']);
        ...
        try
          ...
          begin
            ...
            try
              ...
              // poresim radu
              mDocQueue_ID := GetDocQueue_ID(AOS, mModule, mDocType, mUser_ID, json, DOC_BillOfDelivery);
          ...
          try
            ...
            if mDocType = DOC_IssuedOrder then
              ...
              mSDClassID := Class_MaterialDistribution
            else
            ...
            begin
              ...
              if SeparatedDocumentByClone then
              begin
                LogWriteSectionStart('SeparatedDocumentByClone');
                // pokud vytvarim oddeleny doklad pres Clone, tak jen v pripade, ze je nejaky nezpracovany radek a nevychazelo se z objednavky
                mProvideRow_ID := '';
                mAllProcessed := True;
                dtJSONRows.First;
                while not dtJSONRows.Eof do
                begin
                  ...
                  begin
                    mAllProcessed := False;
                    mProvideRow_ID := dtJSONRows.FieldByName('StoreDocument2ProvideRow_ID').AsString;
                    LogWriteEvent(Format('Not processed row found. ProvideRow_ID: %s', [mProvideRow_ID]));
                    break;
                  end;
                  dtJSONRows.Next;
                end;

                if not mAllProcessed and (mProvideRow_ID = '') then
                begin
                  mSDNew_ID := CreateNewStoreDocumentSAByClone(mSD);
                  mSD.Refresh;
                end;
                LogWriteSectionEnd;
              end;
              ...
              if mListDocType = '' then
                 SaveDialogValues(mSD, json.O['Dialog'].A['values']);
              ...
              begin
                ...
                // jinak nastavime zpracovane mnozstvi
                  ...
                  begin
                    ...
                    SaveDialogValues(mSDRow, json.A['rows'].O[dtJSONRows.FieldByName('jsonIndex').AsInteger].O['dialog'].A['values']);

                    // pokud je artikl s evidenci sarzi a nejde o PRP
                    if (mSDRow.GetFieldValueAsInteger('StoreCard_ID.Category') = 2) then
                    begin
                      fillStoreBatches(mSDRow, True);
                    end;
                    // vyplnim ser. cisla
                    fillSerialNumbers(mSDRow);

                    // vyplnim custom fieldy
                    FillCustomFields(mSDRow, dtJSONRows, json);
              ...
              //pridame nove radky
              if json.B['CanAddItems'] and not (mDocType in [DOC_IssuedOrder, DOC_JobOrder]) then
              begin
              ...
              LogWriteEvent('Checking CanTakePhotos');
              if REST_getJSONBool(json, 'CanTakePhotos') and not (mDocType in [DOC_IssuedOrder, DOC_JobOrder]) then
              begin
                AddPhotos(AOS, mDocType, mSD.OID, json.A['documents']);
              end;
              ...
              if dtDocumentQuantity.RecordCount > 0 then
                ...
                begin
                  mDocQueue_ID := GetDocQueue_ID(AOS, mModule, mLogStoreDocumentDocType, mUser_ID, json, mDocType, mSD);
                  ...
                  if MakeExecuteLogStoreDocument(AOS, mModule, mLogStoreDocumentDocType, mUser_Id, mLSD_ID) then
              ...
              begin
                ...
                try
                  ...
                  mDocQueue_ID := GetDocQueue_ID(AOS, mModule, DOC_IncomingTransfer, mUser_ID, json, mDocType, mSD);
                  mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString :=
                  ...
                  if dtDocumentQuantity.RecordCount > 0 then
                  begin
                    mDocQueue_ID := GetDocQueue_ID(AOS, mModule, DOC_LogStoreInput, mUser_ID, json, DOC_IncomingTransfer, mPrePri);
                    ...
                    if MakeExecuteLogStoreDocument(AOS, mModule, DOC_LogStoreInput, mUser_Id, mLSDPrePri_ID) then
            ...
            if ((askForNewDocumentCreation(AOS, mModule, mDocType, mUser_ID) and json.B['AskForNewDocumentCreation'])
              or (not askForNewDocumentCreation(AOS, mModule, mDocType, mUser_ID) and createNewDocument(AOS, mModule, mDocType, mUser_ID))
              or (mDocType in [DOC_IssuedOrder, DOC_JobOrder]))
            ...
            begin
              ...
              try
                ...
                findProvideIDs(mSLROs, mSLRORows, mDamagedDocs, mDamagedRowsDocs, mJOs, mCMs, mDamagedFound, mAllProcessed);

                if (not mAllProcessed) or (mDocType in [DOC_IssuedOrder, DOC_JobOrder]) then
                begin
                  ...
                  try
                    ...
                    begin
                      ...
                      begin
                        ...
                        mSDNew := AOS.CreateObject(mSDClassID);
                        mSDNew.ExplicitTransaction := True;
                        mSDNew.Load(mSDNew_ID);
                      end
                      else
                        mSDNew := CreateNewStoreDocumentSA(mSD);
                    end;
          ...
          LogWriteSectionStart('Changing statuses');
          mSD := GetStoreDocBO(AOS, mDocType);
          try
            LogWriteEvent('Changing status of main document...');
            mSD.ExplicitTransaction := True;
            mSD.Load(dtStoreDocuments.FieldByName('Provide_ID').AsString, nil);

            if putQueueDocDetailStopPicking_changeSDStatus(AOS, mModule, mDocType, mUser_ID, mSDNew_ID, 0, mSD, nil) then
            begin
              // u objednavky musim rozlisit, zda se zpracovalo vse nebo ne
              if (mDocType = DOC_IssuedOrder) and not mAllProcessed then
                  ChangeStatusByRule(mSD, PRECHOD_UKONCENI2(mDocType, mModule))
              else if mDocType <> DOC_JobOrder then
            ...
            if (mDocType = DOC_RefundedBillOfDelivery) and useBillOfDeliveryForRefunding then
            begin
              LogWriteEvent('Changing status of refunded bill of delivery...');
            ...
            begin
              LogWriteEvent('Changing status of incoming transfer...');
              mPrePri := AOS.CreateObject(Class_IncomingTransfer);
              try
                mPrePri.ExplicitTransaction := True;
                mPrePri.Load(mPrePri_ID, nil);
            ...
            begin
              LogWriteEvent('Changing status of document with new rows...');
              mSDNew := AOS.CreateObject(mSDClassID);
              try
                mSDNew.ExplicitTransaction := True;
            ...
            begin
              LogWriteEvent('Changing status of seperated (new) document...');
              mSDNew := AOS.CreateObject(mSDClassID);
              try
                mSDNew.ExplicitTransaction := True;
            ...
            begin
              LogWriteEvent('Changing status of document with damaged rows...');
              mSDNew := AOS.CreateObject(mSDClassID);
              try
                mSDNew.ExplicitTransaction := True;
      ...
      begin
        LogWriteEvent('Changing status of list...');
        mBO.Refresh;
    ...
    begin
      ...
      ConfirmLSD(AOS, mLogStoreDocumentClass, mLSD_ID, 'putQueueDocDetailStopPicking');
    end;
    ...
    begin
      ...
      ConfirmLSD(AOS, Class_LogStoreInput, mLSD_ID, 'putQueueDocDetailStopPicking');
    end;
    LogWriteSectionEnd;
  finally
    mBO.Free;
...
begin
  ...
  CFxProfiler.EnterProc(REST_LogName, 'ModifyVisibleModulesString');
  try
    Result := '';
      ...
      begin
        ...
        begin
          ...
          case mModule of
           ...
           'STD_MaterialDistributionQueue':     mDocumentType := DOC_MaterialDistribution;
           'STD_ProductReceptionQueue':         mDocumentType := DOC_ProductReception;
           else mDocumentType := '';
          end;
        ...
        if mModule in ['STD_ReceiptCardQueue', 'STD_ReceiptCardFastQueue', 'STD_BillOfDeliveryQueue', 'STD_BillOfDeliveryFastQueue', 'STD_RemovalListsQueue',
            ...
            'STD_OutgoingTransformationQueue', 'STD_IncomingTransformationQueue', 'STD_TransferBetweenPositionsQueue', 'STD_ReceiptCardFromIOQueue',
            'STD_MaterialDistributionQueue', 'STD_ProductReceptionQueue'] then
    ...
    CFxProfiler.ExitProc(REST_LogName, 'ModifyVisibleModulesString');
    LogWriteSectionEnd;
  end;
end;
...
begin
  ...
  CFxProfiler.EnterProc(REST_LogName, 'GetMainMenuButtons');
  try
    ...
    CFxProfiler.ExitProc(REST_LogName, 'GetMainMenuButtons');
    LogWriteSectionEnd;
  end;
end;
...
var
 ...
 mInputValues, mOutputValues: TMemTable;
begin
  ...
  try
    ...
    try
      ...
      mOutputValues.Edit;
      parseBarcodeForRowSpecial(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, mBarcode, mInputValues, nil, mOutputValues, nil, nil, nil);
      if mOutputValues.Modified then
        mOutputValues.Post
      else
        mOutputValues.Cancel;

      SetResponse(AResponse, PlainResponse(mOutputValues.FieldByName('StoreCard_ID').AsString));
    finally
      mOutputValues.Free;
      mInputValues.Free;
    end;
