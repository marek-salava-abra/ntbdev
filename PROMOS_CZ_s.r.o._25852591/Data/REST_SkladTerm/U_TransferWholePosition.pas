...
function isTransferWholePositionContentsAllowed(AOS: TNxCustomObjectSpace; AStorePosition_ID, AUnblockPositions: String; var AMessage: String): Boolean;
var
...
begin
  ...
  try
    ...
    mSql := 'select ' +
      ...
      'group by LSD.ID ';
    AOS.SQLSelect2(mSql, dtLogStoreDocs);
...
procedure getTransferWholePositionContentsAllowed(AOS: TNxCustomObjectSpace; APath, AResponse: TStringList);
var
  mStorePosition_ID, mUnblockPositions: String;
  mMessage: String;
...
begin
  ...
  mBlockedFirst := False;
  mUnblockPositions := POVOLENE_STAVY_PRESUN_CELE_POZICE(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, nil, mBlockedFirst);

  if isTransferWholePositionContentsAllowed(AOS, mStorePosition_ID, mUnblockPositions, mMessage) then
    SetResponse(AResponse, PlainResponse(''))
  else
    SetPlainResponse(AResponse, mMessage, HTTP_SC_ExpectationFailed);
  LogWriteSectionEnd;
end;

procedure putTransferWholePositionContents(AOS: TNxCustomObjectSpace; APath: TStringList; ABody: String; AResponse: TStringList);
var
  mStorePositionFrom_ID, mStorePositionTo_ID, mDocQueue_ID: String;
  mMessage, mSql, mLastLSD_ID, mUnblockPositions: String;
...
begin
  ...
  try
    ...
    case Request_Start(AOS, mRequestID, 'putTransferWholePositionContents') of
      1: begin
    ...
    try
      mUnblockPositions := POVOLENE_STAVY_PRESUN_CELE_POZICE(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, json, mBlockedFirst);

      // znovu zkontrolujeme, jestli neni pozice blokovana doklady v nepovolenych stavech
      if not isTransferWholePositionContentsAllowed(AOS, mStorePositionFrom_ID, mUnblockPositions, mMessage) then
      begin
        SetPlainResponse(AResponse, mMessage, HTTP_SC_ExpectationFailed);
        AOS.RollBack;
        exit;
      ...
      mSql := 'select LSD.ID as "LSD_ID", LSD.DocumentType as "DocumentType", LSD2.ID as "LSD2_ID", ' +
        ...
        '  and LSD.Executed = ''N'' ' +
        'order by LSD.ID ';
      AOS.SQLSelect2(mSql, dtBlockingLSDRows);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mLSD := AOS.CreateObject(GetStoreDocClass(dtBlockingLSDRows.FieldByName('DocumentType').AsString));
            try
      ...
      AOS.SQLSelect2(mSql, dtLSPContents);
      ...
      try
        ...
        mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_LogStoreTransfer, gSkladTermUser_ID, json);
        mLSD.SetFieldValueAsString('DocQueue_ID',
          GetValueOrDefault(mDocQueue_ID, GetDefaultDocQueue_ID(DOC_LogStoreTransfer)));

        mLSD.SetFieldValueAsString('Firm_ID', FIRM_OWN);
        if not ABRA then
          mLSD.SetFieldValueAsString('StoreMan_ID', SqlSelectStr(AOS, 'select Person_ID from SecurityUsers where ID = ' + QuotedStr(gSkladTermUser_ID)));
        mLSDRows := mLSD.GetLoadedCollectionMonikerForFieldCode(mLSD.GetFieldCode('Rows'));
        ...
        ConfirmLSD(AOS, Class_LogStoreTransfer, mLSD.OID, 'putTransferWholePositionContents', not AOS.InTransaction);
      finally
        mLSD.Free;
      end;
      ...
      begin
        ...
        begin
          ...
          // s presuny mezi pozicemi nepocitame
          mLSD := GetLogStoreDocBOID(mSL[i], AOS, gSkladTermDocType);
          try
            mLSD.ExplicitTransaction := True;
            mLSD.Load(mSL[i], nil);
            mLSDRows := mLSD.GetLoadedCollectionMonikerForFieldCode(mLSD.GetFieldCode('Rows'));
      ...
      AOS.RollBack;
      SetPlainResponse(AResponse, getString('error_while_trasfer_whole_position') + ExceptionMessage, HTTP_SC_ExpectationFailed);
...
procedure PutDispatchWholePosition(AOS: TNxCustomObjectSpace; APath: TStringList; ABody: String; AResponse: TStringList);
var
  mRequestID, mPersonField, mSql, mLSD_ID, mLastStore_ID, mLastStoreCard_ID, mLastStoreBatch_ID,
    mFirm_ID, mDocQueue_ID: String;
  mTemporaryStorageID: Integer;
  json: TJSONSuperObject;
...
begin
  ...
  try
    ...
    // pred transakci overime, jestli uz tento request nebezi nebo neni dokonce dokonceny. Pokud ne, tak ho zalozime.
    case Request_Start(AOS, mRequestID, 'PutDispatchWholePosition') of
      1: begin
        SetPlainResponse(AResponse, getString('request_in_process'), HTTP_SC_ExpectationFailed);
        exit;
      end;
    ...
    try
      ...
      // vytvorim vydejku
      mBoD := AOS.CreateObject(Class_BillOfDelivery);
      try
        ...
        begin
          mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, json);
          mBoD.SetFieldValueAsString('DocQueue_ID',
            GetValueOrDefault(mDocQueue_ID, GetDefaultDocQueue_ID(gSkladTermDocType)));
        end;
        ...
        begin
          ...
          begin
            ...
            mBoDRow.SetFieldValueAsString('Division_ID',
              GetValueOrDefault(GetDivision_ID(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, mBoD), STREDISKO_HLAVNI));
        ...
        afterSaveHook(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, mBoD, 0, json, dtJsonRows);

        ChangeStatusByRule(mBoD, PRECHOD_VYTVORENI(gSkladTermDocType, gSkladTermModule));

        // vytvorime polohovak
        if dtLSPContents.RecordCount > 0 then
          if CreateLogStoreDocument(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, '', mBoD, json, dtJsonRows, dtLSPContents) then
          begin
            mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_LogStoreOutput, gSkladTermUser_ID, json, gSkladTermDocType, mBoD);
            mLSD_ID := REST_Create_LogStoreDocument(AOS, mBoD, '',
              Class_LogStoreOutput,
              GetValueOrDefault(mDocQueue_ID, GetDefaultDocQueue_ID(DOC_LogStoreOutput)),
              LogStoreOutput_StoreGateway_ID, dtLSPContents, gSkladTermUser_ID,
              False);
          end;
      finally
        mBoD.Free;
      ...
      AOS.RollBack;
      SetPlainResponse(AResponse, getString('error_while_dispatch_whole_position') + ExceptionMessage, HTTP_SC_ExpectationFailed);
      Request_Cancel(AOS, mRequestID);
      glog.WriteEvent(logError, 'PutDispatchWholePosition - error - ' + ExceptionMessage);
    end;

    // polohovaky potvrdime az mimo hlavni transakci
    ConfirmLSD(AOS, Class_LogStoreOutput, mLSD_ID, 'PutDispatchWholePosition');
  finally
    json.Free;
    dtJsonRows.Free;
    dtLSPContents.Free;
