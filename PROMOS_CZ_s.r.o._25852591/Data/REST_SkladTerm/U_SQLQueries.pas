...
function getRowsSql(AOS: TNxCustomObjectSpace; AModule, AUser_ID, ADocType, AAdditionalFields: String; var OWhere: String): String;
var
...
begin
  ...
  if ADocType in [DOC_ShippingList, DOC_RemovalList] then
    ...
    mSql :=
      'select' + nxCrLf +
      '  SD.ID as "XX_Parent_ID",' + nxCrLf +
      '  N.PosIndex as "PosIndex",' + nxCrLf +
      '  SD2.ID as "ID",' + nxCrLf
  else
    mSql :=
      'select' + nxCrLf +
  ...
  mSql := mSql +
    ...
    '  ' + getUnitsSql + ' as "StoreCardUnits",' + nxCrLf +
    '  SC.EAN as "ean",' + nxCrLf +
    ...
    '  ' + putQueueDocDetailStartPicking_StoreCardCategory(AOS, AModule, ADocType, AUser_ID, 'getRowsSql') + ' as "StoreCardCategory",' + nxCrLf +
...
//      '  ' + customFields(AOS, AUser_Id, AModule) + ' "customFields", ' + nxCrLf +
    '  ' + DisableQuantityEdit(AOS, AModule, ADocType, AUser_ID) + ' as "DisableQuantityEdit",' + nxCrLf +
    '  ' + rowAuxText2(AOS, AModule, ADocType, AUser_ID, 'getRowsSql') + ' as "AuxText2",' + nxCrLf +
    ...
    '  cast('''' as varchar(2000)) as "dialogString",' + nxCrLf +
    '  ' + putQueueDocDetailStartPicking_rowsAuxFields(AOS, AModule, ADocType, AUser_ID) + nxCrLf;

  if ADocType in [DOC_IssuedOrder, DOC_JobOrder] then
    mSql := mSql +
      '  ' + QuotedStr(ADocType) + ' as "StoreDocumentType",' + nxCrLf
  else
    mSql := mSql +
  ...
  mSql := mSql +
    '  ' + EnterStoreBatchExpirationDate(AOS, AModule, ADocType, AUser_ID, mExpirationEnterType, mStoreBatchExpirationField) + ' as "EnterStoreBatchExpirationDate",' + nxCrLf +
  ...
  begin
    mSql := mSql +
      '  SD.Division_ID as "Division_ID",' + nxCrLf +
      '  SD.BusProject_ID as "BusProject_ID",' + nxCrLf +
      '  SD.BusOrder_ID as "BusOrder_ID",' + nxCrLf +
      '  SD.BusTransaction_ID as "BusTransaction_ID",' + nxCrLf +
    ...
    if useMainUnits(AOS, AModule, ADocType, AUser_Id) then
      mSql := mSql +
        '  cast((cast(SD2.Quantity as double precision) * cast(mi.Quantity as double precision)' + nxCrLf +
        '    * (1 + (cast(SD2.WastePercentage as double precision) / 100))) as  Numeric(18,3)) - coalesce(A.Quantity2, 0) + coalesce(B.Quantity2, 0) as "UnitQuantity",' + nxCrLf +
        '  cast((cast(SD2.Quantity as double precision) * cast(mi.Quantity as double precision)' + nxCrLf +
        '    * (1 + (cast(SD2.WastePercentage as double precision) / 100))) as  Numeric(18,3)) - coalesce(A.Quantity2, 0) + coalesce(B.Quantity2, 0) as "UnitQuantityOrig",' + nxCrLf
    else
      mSql := mSql +
        '  cast((cast(SD2.Quantity / SD2.UnitRate as double precision) * cast(mi.Quantity / MI.UnitRate as double precision)' + nxCrLf +
        '    * (1 + (cast(SD2.WastePercentage as double precision) / 100))) as  Numeric(18,3)) - coalesce(A.Quantity, 0) + coalesce(B.Quantity, 0) as "UnitQuantity",' + nxCrLf +
        '  cast((cast(SD2.Quantity as double precision) * cast(mi.Quantity as double precision)' + nxCrLf +
        '    * (1 + (cast(SD2.WastePercentage as double precision) / 100))) as  Numeric(18,3)) - coalesce(A.Quantity, 0) + coalesce(B.Quantity, 0) as "UnitQuantityOrig"' + nxCrLf;
  end
  else
  begin
    mSql := mSql +
  ...
  if ADocType in [DOC_ShippingList, DOC_RemovalList] then
    ...
    mSql := mSql +
      'from PLMJOInputItems SD2' + nxCrLf +
      'join PLMJONodes N ON N.ID = SD2.Owner_ID' + nxCrLf +
      'join PLMJONodes NMI ON NMI.ID = N.Master_ID' + nxCrLf +
      'join PLMJobOrders SD on SD.ID = NMI.Parent_ID' + nxCrLf +
      'join PLMJOOutputItems mi on mi.Owner_ID = NMI.ID' + nxCrLf +
      'join ProductionTasks PT on PT.ID = SD.Productiontask_ID' + nxCrLf +
      'left join' + nxCrLf +
      '  (select sum(coalesce(B.Quantity, 0)) AS Quantity2, sum(coalesce(B.Quantity / B.UnitRate, 0)) as Quantity, A.Parent_ID' + nxCrLf +
      '  from PlMmiplMaterialDistrib A' + nxCrLf +
      '  left join StoreDocuments2 B on B.id = A.StoreDocument2_id' + nxCrLf +
      '  GROUP BY A.Parent_ID' + nxCrLf +
      '  ) A ON A.Parent_ID = SD2.ID' + nxCrLf +
      'left join' + nxCrLf +
      '  (select sum(coalesce(B.Quantity, 0)) AS Quantity2, sum(coalesce(B.Quantity / B.UnitRate, 0)) as Quantity, A.Parent_ID' + nxCrLf +
      '  FROM PLMmiplRMaterialDistrib A' + nxCrLf +
      '  left join StoreDocuments2 B on B.id = A.StoreDocument2_id' + nxCrLf +
      '  GROUP BY A.Parent_ID' + nxCrLf +
      '  ) B ON B.Parent_ID = SD2.ID' + nxCrLf
  else
    mSql := mSql +
  ...
  if ADocType = DOC_JobOrder then
    mSql := mSql +
      'join Stores S on S.ID = SD2.SupposedStore_ID' + nxCrLf
  else
    mSql := mSql +
  ...
  begin
    mSql := mSql +
      'left join PLMJobOrdersSN DRB on DRB.Parent_ID = SD2.ID' + nxCrLf +
  ...
  if ADocType = DOC_JobOrder then
    mSql := mSql +
      'join StoreCards SC on SC.ID = N.StoreCard_ID' + nxCrLf
  else
    mSql := mSql +
  ...
  else if ADocType in [DOC_IssuedOrder, DOC_JobOrder] then
    mSql := mSql +
  ...
  mCustomerSql := customGetRowsSql(AOS, AModule, AUser_Id, ADocType, mSql, OWhere);
...
function getStoreCardInfoSql(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String; AStoreCardIds: TStringList; AFirm_ID, AStore_ID: String;
  AToBatch_ID, AAdditionalFields: String; AIsNewRow: Boolean): String;
var
...
begin
  ...
  begin
    ...
    if UseUnitsEANs and not CFxOID.IsEmpty(mStoreUnitId) then
      mSql := mSql +
    ...
    mSql := mSql +
      '  ' + putQueueDocDetailStartPicking_StoreCardAuxInfoForSerNumField(AOS) + ' as "StoreCardAuxInfoForSerNum$BOOL",' + nxCrLf +
      '  ' + putQueueDocDetailStartPicking_StoreCardCategory(AOS, AModule, ADocType, AUser_ID, 'getStoreCardInfoSql') + ' as "StoreCardCategory",' + nxCrLf +
      '  ' + getUnitsSql + ' as "StoreCardUnits",' + nxCrLf +
      ...
      '  cast(' + AvailableInStock_SummaryCustomValue(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, mTitle, 0) + ' as varchar(30)) as "SUM_CustomValue",' + nxCrLf +
      '  ' + mTitle + 'as "SUM_CustomTitle",' + NxCrLf +
      '  SC.EAN as "ean",' + nxCrLf +
      '  ' + DefaultUnitQuantity(AOS, AModule) + ' as  "DefaultUnitQuantity",' + nxCrLf +
      '  ' + StoreCardListAuxText(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID) + ' as "listAuxText",' + nxCrLf +
      '  ' + rowAuxText2(AOS, AModule, ADocType, AUser_ID, 'getStoreCardInfoSql') + ' as "AuxText2",' + NxCrLf +
      '  cast('''' as varchar(2000)) as "dialogString",' + nxCrLf +
      '  ' + DisableQuantityEdit(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID) + ' as "DisableQuantityEdit",' + nxCrLf;
    ...
    mSql := mSql +
      StoreCard_Where(AOS,  AModule, ADocType, AUser_ID, AIsNewRow);
...
var
  mSqlCondition, mSql, mStoreBatchNoteField, mStoreBatchExpirationField, mTitle: String;
begin
  ...
  Result :=
    ...
    '  ' + mStoreBatchNoteField + ' as "StoreBatchNote",' + NxCrLf +
    '  (select sum(SSB.Quantity - SSB.BookedQuantity) / SU.UnitRate from StoreSubBatches SSB where SSB.StoreBatch_ID = ' + QuotedStr(AStoreBatch_ID) + ') as "SUM_Available",' + NxCrLf +
    '  cast(' + AvailableInStock_SummaryCustomValue(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, mTitle, 1) + ' as varchar(30)) as "SUM_CustomValue",' + nxCrLf +
    '  ' + mTitle + 'as "SUM_CustomTitle",' + NxCrLf;
...
function getStorePositionInfoSql(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID, AStorePosition_ID: String; AQueryParams: TStringList): String;
begin
  Result :=
    'select' + nxCrLf +
    '  LSP.ID as "id",' + nxCrLf +
    ...
    'where' + nxCrLf +
    '  LSP.ID = ' + QuotedStr(AStorePosition_ID) + nxCrLf +
    StorePosition_Where(AOS,  AModule, ADocType, AUser_ID, AQueryParams);
end;
...
begin
  ...
  Result :=
    ...
    QuotedStr(NxBoolToStr(askForNewDocumentCreation(AOS, AModule, ADocType, AUser_ID))) + ' as "AskForNewDocumentCreation$BOOL",' + nxCrLf +
    ...
    CreateTransferIn(AOS, AModule, ADocType, AUser_ID) + ' as "CreateTransferIn$BOOL",' + nxCrLf +
    ...
    QuotedStr(NxBoolToStr(CanSaveWithoutProcessingAllRows(AOS, AModule, ADocType, AUser_ID))) + ' as "pars.SaveWoProcessAll$BOOL"' + nxCrLf;
end;
...
var
  mTable, mColumns, mJoins, mDocType, mDescriptionField: String;
  i: Integer;
begin
  Result := '';
  ...
  begin
    ...
    if mDocType in [DOC_ShippingList, DOC_RemovalList] then
      ...
      mColumns := mColumns + '   -1 as "ItemCount",' + nxCrLf
    ...
    mDescriptionField := '';
    mDescriptionField := listDocQueue_Field_Description(AOS, AModule, mDocType, AUser_Id);
    if mDescriptionField <> '' then
      mColumns := mColumns +
        '   ' + mDescriptionField + ' as "Description",' + nxCrLf;
    ...
    else if ABRA and (mDocType = DOC_JobOrder) then
      Result := Result +
       '   1 = 1' + NxCrLf
    else
      Result := Result +
    ...
    if not (mDocType in [DOC_ShippingList, DOC_RemovalList, DOC_IssuedOrder, DOC_JobOrder]) then
    begin
...
begin
  Result :=
    ...
    '  SC.EAN as "ean",' + nxCrLf +
    '  SC.Category as "StoreCardCategory", ' + nxCrLf +
    '  coalesce(PIB.RealQuantity, PIR.RealQuantity) as "UnitQuantity",' + nxCrLf +
    '  coalesce(PIB.UnitRate, PIR.UnitRate) as "UnitRate", ' + nxCrLf +
    '  coalesce(PIB.QUnit, PIR.QUnit) as "UnitCode", ' + nxCrLf +
    '  coalesce(LSP.ID, '''') as "StorePositionFrom_ID", ' + nxCrLf +
  ...
  Result := Result +
    '  ' + rowAuxText2(AOS, AModule, ADocType, AUser_ID, 'GetPartialInvProtocolsRowsSql') + ' as "AuxText2" ' + nxCrLf +
  ...
  Result := Result +
    ' order by ' + nxCrLf +
    ' 	LSP.Code, SC.Code, SB.Name';
end;

procedure GetStoreCardDetailInfo(AOS: TNxCustomObjectSpace; AStoreCardIds: TStringList; AExtendedInfoStore_ID: String; AWithExtendedInfo, AWithUnits: Boolean;
  OOutput: TStringList; AExtendedInfo, AUnits, AEANs: TMemTable);
var
  mSql: String;
begin
  ...
  begin
    mSql :=
      ...
      '  cast(' + AvailableInStock_ByStoreCustomValue(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, 0) + ' as varchar(30)) as "CustomValue"' + nxCrLf +
      ...
      'where' + nxCrLf +
      '  SSC.StoreCard_ID in (''' + ReplaceStr(AStoreCardIds.DelimitedText, ',', ''',''') + ''')' + nxCrLf +
      '  and SSC.Quantity <> 0' + nxCrLf;

    if not CFxOID.IsEmpty(AExtendedInfoStore_ID) then
      mSql := mSql +
        '  and S.ID = ' + QuotedStr(AExtendedInfoStore_ID) + nxCrLf;
  ...
  begin
    mSql :=
      ...
      'join StoreCards SC on SC.ID = SU.Parent_ID' + nxCrLf +
      'where' + nxCrLf +
      '  SU.Parent_ID in (''' + ReplaceStr(AStoreCardIds.DelimitedText, ',', ''',''') + ''')';
    AOS.SQLSelect2(mSql, AUnits);
    if AUnits.Active then
    begin
      AUnits.AddIndex('id', REST_XX_Parent_ID + ';PosIndex', [ixUnique]);
      AUnits.IndexName := 'id';
      OOutput.AddObject('units=', AUnits);
    end;

    mSql :=
      'select' + nxCrLf +
      '  SE.Parent_ID as "XX_Parent_ID",' + nxCrLf +
      '  SE.ID as "StoreEAN_ID",' + nxCrLf +
      '  SE.EAN as "StoreEAN"' + nxCrLf +
      'from StoreEANs SE' + nxCrLf +
      'join StoreUnits SU on SU.ID = SE.Parent_ID' + nxCrLf +
      'where SU.Parent_ID in (''' + ReplaceStr(AStoreCardIds.DelimitedText, ',', ''',''') + ''')';
    AOS.SQLSelect2(mSql, AEANs);
    if AEANs.Active then
    begin
      AEANs.AddIndex('id', REST_XX_Parent_ID + ';StoreEAN', [ixPrimary]);
      AEANs.IndexName := 'id';
      OOutput.AddObject('units.eans=', AEANs);
    end
  end;
end;
