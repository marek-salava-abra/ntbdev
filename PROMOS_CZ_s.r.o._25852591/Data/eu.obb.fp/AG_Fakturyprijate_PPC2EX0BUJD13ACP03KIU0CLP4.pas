uses 'eu.obb.fp.ParseData', 'eu.obb.fp.Progress';
    ...
    if mOpenDlg.Execute then begin
      try
        ...
        try
           ...
           //'var_symbol','doklad','firma','dic','ic','datum','splatnost','mena_zbyva','mena'
            mFirm_ID:=GetFirm_ID(mOS,mParRow.ParamByName('dic').AsString);
           if NxIsEmptyOID(mFirm_ID) then mFirm_id:='AAA1000000';
            mSMBO:=mos.CreateObject(Class_ReceivedInvoice);
            msmbo.New;
            mSMBO.prefill;
            msmbo.SetFieldValueAsString('DocQueue_ID','1700000101');
            ...
            msmbo.SetFieldValueAsString('ExternalNumber',mParRow.ParamByName('doklad').AsString);
            ...
            mplrow.SetFieldValueAsFloat('TAmount',NxIBStrToFloat(mParRow.ParamByName('mena_zbyva').AsString));
            //if not(NxIsBlank(mParRow.ParamByName('mena').AsString)) then mplrow.SetFieldValueAsFloat('TAmount',NxIBStrToFloat(mParRow.ParamByName('mena_zbyva').AsString));
            //mplrow.SetFieldValueAsString('Division_ID',GetDivision_ID(mos,mParRow.ParamByName('stredisko').AsString+'0'));
            mSMBO.save;
            msmbo.Free;
