...
begin
  ...
  mMAction.Hint := 'Změní hodnotu minima dle požadavku';
        ...
        for i:=0 to mList.Count - 1 do begin
            ...
            //NxShowSimpleMessage(mStoreCard_ID+' '+mStoreSubCard_ID,mSite);
              if not(NxIsEmptyOID(mStoreSubCard_ID)) then begin
                  ...
                  mQuantity:=scrGetQuantity(mSite.BaseObjectSpace,mStoreCard_ID, mStore_ID,mdate);
                  mQuantity:=mQuantity*(mDayAfter/mDayBefore);
                  mQuantity:=NxRoundByValue(mQuantity,ctup,1);
                  mbo.SetFieldValueAsFloat('LowLimitQuantity',mQuantity);
                  if mBO.GetFieldValueAsFloat('X_PromosLowLimit') >0 then begin
                   //if mBO.GetFieldValueAsFloat('X_PromosLowLimit')>mQuantity then mbo.SetFieldValueAsFloat('LowLimitQuantity',mBO.GetFieldValueAsFloat('X_PromosLowLimit'));
                  end;
                  j:=j+1;
                  end;
                  mbo.Save;
...
const
  cSQL = 'SELECT ssc.ID FROM StoreSubcards ssc left join storecards sc on sc.id=ssc.storecard_id WHERE ssc.StoreCard_ID=''%s'' and ssc.store_id=''%s'' and sc.X_NotCalc=''N'' ';
var
  mList : TStringList;
begin
  mList := TStringList.Create;
