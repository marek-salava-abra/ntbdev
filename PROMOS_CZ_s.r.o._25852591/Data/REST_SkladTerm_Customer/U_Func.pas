...
begin
  ...
  try
    ...
    try
      ...
      mBO.PMChangeStateByTransition(PRECHOD_UKONCENI(GetStoreDocType(mDoc_ID, AOS), gSkladTermModule));
...
begin
  ...
  try
    ...
    json.S['storeDocument_ID'] :=  '5C41000101';
    json.S['storeDocument2_ID'] :=  '4K21000101';
    json.I['unitQuantity'] :=  1;
    json.B['saveDocument'] :=  False;
...
begin
  Result :=
    ...
    '  ' + putQueueDocDetailStartPicking_StoreCardCategory(AOS, AModule, ADocType, AUser_ID, 'GetMySQLRows') + ' as "StoreCardCategory",' + nxCrLf +
...
//      '  ' + customFields(AOS, AUser_Id, AModule) + ' "customFields", ' + nxCrLf +
    ...
    '  ' + rowAuxText2(AOS, AModule, ADocType, AUser_ID, 'GetMySQLRows') + ' as "AuxText2",' + nxCrLf +
...
begin
  ...
  begin
    mDocTypeParameter := APath.Strings[1];
    if APath.Count = 3 then
      mSearchStr := APath.Strings[2];
  end else
  begin
    SetPlainResponse(AResponse, getString('wrong_parameters_count'), HTTP_SC_BadRequest);
    exit;
  ...
  try
    ...
    mDocTypes.Add(mDocTypeParameter);
...
var
  ...
  // vrati radu dokladu
  function GetDocQueueForDocument(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID, ADefaultDocQueue_ID: String; AJson: TJSONSuperObject;
    ASourceDocType: String = ''; ADocument: TNxCustomBusinessObject = nil; AStore_ID: String = ''): String;
  var
    mDocQueue_ID: String;
  begin
    Result := '';

    mDocQueue_ID := GetDocQueue_ID(AOS, AModule, ADocType, AUser_ID, AJson, ASourceDocType, ADocument);
  ...
  begin
    ...
    try
        ...
        try
          ...
          try
            ...
            mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString :=
              GetDocQueueForDocument(AOS, mModule, mDocType, mUser_ID, RADA_PRIJEMKA, json, DOC_IssuedOrder, mIO);
      ...
      begin
        ...
        mSD.SetFieldValueAsString('DocQueue_ID', GetDocQueueForDocument(AOS, mModule, mDocType, mUser_ID, RADA_PRIJEMKA, json));
      end;
        ...
        if ACreateLSD and (dtDocumentQuantity.RecordCount > 0) then
          if CreateLogStoreDocument(AOS, mModule, mDocType, mUser_Id, '', mSD, json, dtJSONRows, dtDocumentQuantity) then
            mLSD_ID := REST_Create_LogStoreDocument(AOS, mSD, '',
              ...
              GetDocQueueForDocument(AOS, mModule, mLogStoreDocumentDocType, mUser_ID, mLogStoreDocument_DocQueue_ID, json, mDocType, mSD),
  ...
  try
    ...
    try
      ...
      DataSet_CreataHeader(dtJSONRows, 'jsonIndex=I,StoreDocument_ID=S10,StoreDocument2_ID=S10,StoreDocument2Provide_ID=S10,StoreDocument2ProvideRow_ID=S10,' +
        ...
        'StoreBatchExpirationDate=S16,IsNotConfirmed=B,ContentUnit=S5');
      dtJSONRows.AddIndex('ByJsonIndex', 'jsonIndex', [ixUnique]);
