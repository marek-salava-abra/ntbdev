...
begin
  mTime:=Frac(Now);
  {if mTime<(1/24) then} os.SQLExecute('update storesubcards set quantity=0 where store_id='+QuotedStr('1420000101')+' and storecard_id in (select id from storecards where X_Import_Simon=''A'')');
  mJSON:=TJSONSuperObject.Create;
  mJSON2:=TJSONSuperObject.Create;
  mJSONArray:=TJSONSuperObjectArray.Create;
   mJSON:=API_GET('https://api.simonfm.cz/SimonFM/StoreSubcards?select=StoreCard_ID.Code as code,Quantity,AverageStorePrice&where=storecard_id in (class storecards select id where X_Export_Promos eq true)');
  ...
  for i:=0 to mJSONArray.Length-1 do begin
     ...
     if not(NxIsEmptyOID(mStoreCard_ID)) then begin
         mSSC_ID:=OS.SQLSelectFirstAsString('Select id from storesubcards where storecard_id='+QuotedStr(mStoreCard_ID)+' and store_id='+QuotedStr('1420000101'),'');
  ...
  Success := True;
  LogInfoStr := 'Records Simon: '+IntToStr(j)+'  Records KT: '+IntToStr(k);
end;
