...
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'TTComponents'));

    Result := TTabSheet.Create(APageControl);
    Result.PageControl := APageControl;
    Result.Caption := '&Dopravy';
    Result.Name := 'tabParamsTT';
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'TTComponents'));
    mGrd.Parent := mPnl;
    ...
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := Roll_TransportationTypes;
    ...
    mGridColumn.Caption := 'Název dopravy';
...

{
    ...
    if not mCanEdit then
      mAct.Tag := -1;
    mListBox.Items.AddObject('mactLSEdit', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactLSRefresh';
    mAct.Caption := 'Občerstvit';
    mAct.Category := Result.Name;
    mAct.ShortCut := TextToShortCut('F11');
    mAct.OnExecute := @actLSRefreshExecute;
    mListBox.Items.AddObject('mactLSRefresh', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactLSSave';
    mAct.Caption := 'Uložit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactLSSave', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactLSCancel';
    mAct.Caption := 'Zrušit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactLSCancel', mAct);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(mSite, 'TTComponents'));
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSEdit'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSRefresh'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSSave'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSCancel'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSSave'))).Enabled := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSCancel'))).Enabled := True;

  setParamsEnabled(TWinControl(mListBox.Items.Objects(mListBox.Items.IndexOf('tabParamsTT'))), True);
end;
...
begin
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSEdit'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSRefresh'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSSave'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSCancel'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSSave'))).Enabled := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactLSCancel'))).Enabled := False;

  setParamsEnabled(TWinControl(aListBox.Items.Objects(aListBox.Items.IndexOf('tabParamsTT'))), False);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'TTComponents'));
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            //GlobParams.GetOrCreateParam(dtString, 'ParStorecardType_ID').AsString := '';
             mStoreCardBO:=ASite.BaseObjectSpace.CreateObject('5B5FINKNYNDL3C5P00CA141B44');
            ...
            mBO.SetFieldValueAsString('X_TransportationType_ID', mDataSet.FieldByName('Property_ID').AsString);
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    ' PAR.X_TransportationType_ID as Property_ID, SC.Name as Property_Name,' +
    ' PAR.X_PosIndex as PosIndex, PAR.X_FloatValue1 as Quantity' +
    ' from DefRollData PAR ' +
    ' LEFT JOIN TransportationTypes SC ON SC.ID=PAR.X_TransportationType_ID '+
  ...
  if pgcDataViews.ActivePage <> TTabSheet(pgcDataViews.FindChildControl('tabParamsTT')) then Exit;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject('5B5FINKNYNDL3C5P00CA141B44');
          try
...
begin
  Result := True;
  mListBox := TListBox(NxFindChildControl(aSite, 'TTComponents'));
  if not (TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSEdit'))).Enabled) then begin
    ...
    try
      ...
      Result := TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactLSEdit'))).Enabled;
    end;
  end;
end;
