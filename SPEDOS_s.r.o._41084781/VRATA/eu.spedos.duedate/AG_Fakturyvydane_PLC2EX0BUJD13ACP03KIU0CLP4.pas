...
begin
  ...
  mAction.Caption := 'Změna splatnosti/platby';
  mAction.Items.Add('Změna datumu splatnosti');
  mAction.Items.Add('Změna datumu platby');
  mAction.Hint := 'změní splatnosti nebo termín platby na dokladu';
...
begin
  ...
  if Assigned(mBO) then begin
   if index=0 then begin
    if NxMessageBox('Dotaz','Přejete si změnit splatnost na '+IntToStr(mlist.Count)+' fakturách?' , mdConfirm, mdbYesNo, 0, 0, False, msite)= mrYes then begin
      if GetDate(mdate,mSite,'splatnosti') then begin
       for i:=0 to mList.Count-1 do begin
          mbo.ObjectSpace.SQLExecute(format('update issuedinvoices set duedate$date=%s where id=''%s'' ',[IntToStr(trunc(mdate)),mList.Strings[i]]));
        end;
   ...
   if index=1 then begin
    if NxMessageBox('Dotaz','Přejete si změnit termín platby na '+IntToStr(mlist.Count)+' fakturách?' , mdConfirm, mdbYesNo, 0, 0, False, msite)= mrYes then begin
      if GetDate(mdate,mSite,'platby') then begin
        for i:=0 to mList.Count-1 do begin
         mbo.ObjectSpace.SQLExecute(format('update issuedinvoices set X_PaymentDate=%s where id=''%s'' ',[IntToStr(trunc(mdate)),mList.strings[i]]));
        end;
        TDynSiteForm(mSite).RefreshData;
        TDynSiteForm(mSite).ActiveDataSet.SeekID(mbo.OID);
      end;
