          ...
          if mBookmarkList.count=0 then begin
              ...
              if index=3 then begin
                  mresult:=false;
                      ...
                      if (mresult) and (index=3)  then begin
                          ...
                          try
                                  ...
                                  mid:=iPrintDocument(TDynSiteForm(mSite).CurrentObject,mACLSID,mid_report,NxCreateContext(msite.BaseObjectSpace),mStringlist,mfilename,adir);
                                                          ...
                                                          if  TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('Firm_ID.OrgIdentNumber') = '31708587' then begin
                                                            skdir:=copy(adir,1,31)+(Format('%s\%s', ['Slovensko', TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('Period_id.code')]));

                                                          //  NxShowSimpleMessage(skdir,nil);

                                                            mResult:=DirectoryExists(Format('%s\%s\%s', [skdir, 'Slovensko', TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('Period_id.code')]));
                                                                            if not mresult then begin    // řada
                                                                                    mResult:=NxCreateDir(Format('%s\%s\%s\%s', [skdir, 'Slovensko',TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('Period_id.code'),'FP']));
                                                                            end;
                                                          ...
                                                          mid:=iPrintDocument(TDynSiteForm(mSite).CurrentObject,mACLSID,mid_report,NxCreateContext(msite.BaseObjectSpace),mStringlist,mfilename,skdir);
                    ...
                    MI:=msite.BaseObjectSpace.SQLExecute('Update issuedinvoices set X_Print_ID=' + quotedstr(mid_report) + ',x_archiv=''A'' where id=' + QuotedStr(TDynSiteForm(mSite).CurrentObject.oid));
