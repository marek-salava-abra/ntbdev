    ...
    if mBookmark.count=0 then begin
       try
        mPrintList:=tstringlist.create;
                mPrintList.Add(Format('%s', [TDynSiteForm(mSite).CurrentObject.OID]));
                          ...
                          //CFxInternet.SMSendMailWithMoreFiles(1,AMailAccount, AMailPassword, AServer,
                   //25, AMailFrom, mTo,'KRYBUSOVA@SPEDOS.CZ','' ,ASubjectMail, ABodyMail,1, mSoubor);
                         SendInternalMail(mOS,mTo,'krybusova@spedos.cz',ASubjectMail,Abodymail,mSoubor,'','','','','');
       ...
       for i := 0 to mBookmark.Count-1 do begin // projdu vsechny oznacene zaznamy
                ...
                mPrintList.Add(Format('%s', [TDynSiteForm(mSite).CurrentObject.OID]));
                          ...
                          AsubjectMail:='Výplatní páska '+TDynSiteForm(msite).CurrentObject.GetFieldValueAsString('WorkingRelation_ID.Employee_ID.Person_ID.FullName');
                          ABodyMail:='Vyplatni paska je prilozena jako priloha tohoto emailu';
                          if not(NxIsValidEMail(mTo,False)) then NxShowSimpleMessage('Není platný email '+mto+' pro výplatní pásku '+TDynSiteForm(msite).CurrentObject.DisplayName,mSite);
