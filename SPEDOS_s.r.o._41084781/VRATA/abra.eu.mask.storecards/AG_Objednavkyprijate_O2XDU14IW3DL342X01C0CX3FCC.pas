        ...
        if mDBGrid = nil then RaiseException('DBGrid nenalezen');
          ...
          if mBookmarkList.count=0 then begin
              ...
              if index=2 then begin
                 mID := Newprv(mCustomBusinessObject);
                    if not NxIsEmptyOID(mID) then
                   //     mSite.ShowDynForm('B10I5SAOS3DL3ACU03KIU0CLP4', Nil, Nil, False, 'DoEdit;'+mID);
              end;
...
begin
          ...
          mMAction.Hint := 'Skladové doklady';
          mMAction.Caption := 'Vytvoření skladových dokladů';
...
begin
  result := '';
  try
    mIM := NxCreateDocumentImportManager(abo.ObjectSpace, Class_Receivedorder, Class_OutgoingTransfer);
          mIM.AddInputDocument(abo.OID);

      x := TNxParameters.Create;
      try
        x.GetOrCreateParam(dtString, 'DocQueue_ID', pkInput).AsString := 'R100000101';
        mIM.LoadParams(x);
      finally
        x.Free;
      end;
      ...
      for i := 0 to mHead.Rows.Count - 1 do begin
       ...
       TDynSiteForm.ShowDynFormWithNewDocument('BD0I5SAOS3DL3ACU03KIU0CLP4',NxCreateContext(ABO.ObjectSpace), mHead);
      Result := mHead.DisplayName;
    finally
      mIM.Free;
    end;
