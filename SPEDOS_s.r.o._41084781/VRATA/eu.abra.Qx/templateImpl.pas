...
function OperationWrapper(Self: TNxWebServicesHelper;ActionName: String;Values: String):String;
begin
  try
    if (UpperCase(ActionName)=UpperCase('AppendEAN2StoreCard')) then
      Result := 'OK|' + AppendEAN2StoreCard(Self, Values)
    else
      RaiseException('eu.abra.Qx.templaceImpl.OperationWrapper:  Nepodporovana akce "' + ActionName + '"');
  except
...
function AppendEAN2StoreCard(Self: TNxWebServicesHelper; data : String) : String;

  function getMainUnitBO(AStoreCardBO : TNxCustomBusinessObject) : TNxCustomBusinessObject;
  var
    mUnits : TNxCustomBusinessMonikerCollection;
    i : integer;
  begin
    mUnits := AStoreCardBO.GetLoadedCollectionMonikerForFieldCode(AStoreCardBO.GetFieldCode('StoreUnits'));
    for i := 0 to mUnits.Count - 1 do begin
      if (mUnits.BusinessObject[i].GetFieldValueAsString('Code') = AStoreCardBO.GetFieldValueAsString('MainUnitCode') ) then begin
        Result := mUnits.BusinessObject[i];
        exit;
      end;
    end;
    NxScriptingLog.WriteEvent(logError, 'eu.abra.Qx.templaceImpl.OperationWrapper: Nenalezena jednotka');
  end;
var
  json : TJSONSuperObject;
  mEANBO, mUnitBO, mStoreCardBO : TNxCustomBusinessObject;
  mStoreCard_ID, mEAN : String;
  mEANs : TNxCustomBusinessMonikerCollection;
  i : integer;
begin
  json := TJSONSuperObject.ParseString(data, false);
  mStoreCard_ID := json.S('storeCard_ID');
  mEAN := json.S('ean');
  if Length(mEAN) = 0 then
    RaiseException('EAN nebyl zadán');
  mStoreCardBO := Self.ObjectSpace.CreateObject(Class_StoreCard);
  try
    if not mStoreCardBO.Test(mStoreCard_ID) then begin
      RaiseException('Nenalezena skladová karta');
    end;
    mStoreCardBO.load(mStoreCard_ID, nil);
    mUnitBO := getMainUnitBO(mStoreCardBO);
    mEANs := mUnitBO.GetLoadedCollectionMonikerForFieldCode(mUnitBO.GetFieldCode('StoreEANs'));
    for i := 0 to mEANs.Count - 1 do begin
      if (mEANs.BusinessObject[i].GetFieldValueAsString('EAN')=mEAN) then begin
        Result := 'EAN již na kartě přiřazen';
        exit;
      end;
    end;
    ...
    mStoreCardBO.save;
    Result := mStoreCardBO.DisplayName;
  finally
    mStoreCardBO.Free;
  end;
    ...
    try
      ...
      try
        ...
        if (AHead.Values('VATDocumentType')= 'IssuedInvoice') or (AHead.Values('VATDocumentType') = 'CashReceived') then begin
          x.GetOrCreateParam(dtString, 'DocQueue_ID', pkInput).AsString := mVATDocQueue_ID;
          x.GetOrCreateParam(dtString, 'StoreDocQueue_ID', pkInput).AsString := AHead.Values('BillOfDeliveryDocQueue_ID');
        end else
          x.GetOrCreateParam(dtString, 'DocQueue_ID', pkInput).AsString := AHead.Values('BillOfDeliveryDocQueue_ID');
...
begin
  ...
  mRow.SetFieldValueAsBoolean('CompletePrices', false);
  //        mRow.SetFieldValueAsFloat('UnitPrice', iFloat(TStringList(ARows.Items(j)).Values('UnitPrice')));
  //        mRow.SetFieldValueAsFloat('TotalPrice', iFloat(TStringList(ARows.Items(j)).Values('TotalPrice')));
