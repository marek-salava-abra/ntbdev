...
begin
 ...
 if mList.count>0 then begin
   if NxMessageBox('Dotaz', 'Přejete si obchodně '+mtext+' '+IntToStr(mlist.Count)+' označených objednávek??', mdConfirm, mdbYesNo, 0, 0, False, Nil)=mrYes then begin
           ...
           if not(NxIsEmptyOID(mRowBO.GetFieldValueAsString('BusOrder_ID'))) then begin
             if not(mRowBO.GetFieldValueAsBoolean('BusOrder_ID.X_Closed')) then begin
                ...
                if mDateClosed=0 then begin
                 //GetDate(mDateClosed,mSite,'uzavření');
                 NxShowSimpleMessage('Není faktura, nelze uzavřít.',mSite);
                 exit;
                end;
                if mDateClosed>0 then begin
