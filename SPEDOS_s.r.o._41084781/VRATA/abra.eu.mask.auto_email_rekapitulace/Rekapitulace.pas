...
//         InputQuery('AAA','SSS', cSQLIssuedOrders);
        ...
        LogInfoStr:=LogInfoStr+ ('Vraceno ' + inttostr(mIIs.Count) + ' řádků');
...
//                 NxShowSimpleMessage('dd',nil);
           memail:='';
            ...
            memail:= mIssuedOrder.GetFieldValueAsString('Firm_ID.X_Hromadna_objednavka');
              ...
              Blat_subject:= 'Objednávky Spedos Vrata :' + mIssuedOrder.GetFieldValueAsString('Docqueue_ID.CODE') + '-' + inttostr(mIssuedOrder.GetFieldValueAsinteger('ordnumber')) +  '/' + mIssuedOrder.GetFieldValueAsString('Period_ID.CODE');
              Blat_body:=     'Vážený Dodavateli,'#13#10' '#13#10   +
                            ...
                            // blat_to:='martin.skacel@abra.eu';
                             ...
                             //CFxInternet.SMSendMailWithMoreFiles(1,'objednavky.vrata','MKzpv561vrata', 'posta.spedos.cz',587,'objednavky.vrata@spedos.cz',blat_to,'mojtekova@spedos.cz','' ,Blat_subject, Blat_body,2, Blat_File);
...
begin
  if not(ato='') then begin
     ...
     if not(NxIsEmptyOID(ADivision_ID)) then
