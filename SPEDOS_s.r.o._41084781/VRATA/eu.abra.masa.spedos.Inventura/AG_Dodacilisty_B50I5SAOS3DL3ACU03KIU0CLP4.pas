        ...
        ProgressInit(mSite, 'Zakládám doklady...', mSheet.UsedRange.Rows.Count);
               ...
               while i<mSheet.UsedRange.Rows.Count+1 do begin
                 ...
                 if not(VarToStr(mSheet.Cells[i-1, 6])=VarToStr(mSheet.Cells[i, 6])) then begin
                  ...
                  mAvailableQ:=GetAvailableQuantity(mOS,mStoreCard_ID,GetStore_ID(mOS,VarToStr(mSheet.Cells[i, 3])));
                  if mAvailableQ>0 then begin
                    if not(NxIsEmptyOID(mStoreCard_ID)) {and not(NxIsEmptyOID(mStoreUnit_ID))} and (NxIBStrToFloat(VarToStr(mSheet.Cells[i, 2]))<0)
                     and (-NxIBStrToFloat(VarToStr(mSheet.Cells[i, 2]))<=mAvailableQ) then begin
                      ...
                      mRow.SetFieldValueAsString('Division_ID','D000000101');                 //GAJDOŠ - upraveno ID střediska na 702
                      ...
                      mRow.SetFieldValueAsString('Store_ID',GetStore_ID(mOS,VarToStr(mSheet.Cells[i, 3])));
                      mRow.SetFieldValueAsString('StoreCard_ID',mStoreCard_ID);
                      mRow.SetFieldValueAsFloat('Quantity',mAvailableQ);
                      mRow.SetFieldValueAsString('Division_ID','D000000101');                 //GAJDOŠ - upraveno ID střediska na 702
                      mRow.SetFieldValueAsString('BusOrder_ID',GetBusOrder_ID(mOS,VarToStr(mSheet.Cells[i, 6])));
                    end;
                   end;
