...
begin
        ...
        mBookmarkList := mDBGrid.SelectedRows; // *** v mBookmark jsou id označených záznamu
          ...
          if mBookmarkList.count=0 then begin
                  ...
                  try
                     msite.BaseObjectSpace.SQLSelect('Select distinct BusOrder_id from receivedorders2 where parent_id=' + quotedstr(TDynSiteForm(mSite).CurrentObject.oid) , mr);
                           ...
                           for i:=0 to mr.count-1 do begin
                                        ...
                                        if index=0 then begin
                                         ...
                                         {CFxInternet.HTTPPostBinary('https://sod.spedos.cz/api/api.abra-zakazka.php?',
                                             ...
                                             //end;     }
                                              ...
                                              mWinHTTP2.Open('POST','https://sod.spedos.cz/api/api.abra-zakazka.php?cis_zak='+ mBO.GetFieldValueAsString('Code')+'&Rodneico='+ GetICO(msite.BaseObjectSpace)+'&uzavreno=1');
                                        ...
                                        ' where bo.id=' + quotedstr(mr.Strings[i]));
                                         ...
                                         try
                                              ...
                                              mJSON:= TJSONSuperObject.CreateNew;
                                              mWinHTTP2:= CreateOleObject('WinHttp.WinHttpRequest.5.1');
                                              mWinHTTP2.Open('POST','https://sod.spedos.cz/api/api.abra-zakazka.php?cis_zak='+ mBO.GetFieldValueAsString('Code')+'&Rodneico='+ GetICO(msite.BaseObjectSpace)+'&uzavreno=0');
                                              mWinHTTP2.SetRequestHeader('Authorization','Basic YUJyYTpza1M4Zi1zeFI=');
                                              mWinHTTP2.Send();
                                              mJSON := TJSONSuperObject.ParseString(mWinHTTP2.ResponseText, True);
                                             //end;
                     ...
                     msite.BaseObjectSpace.SQLSelect('Select distinct BusOrder_id from receivedorders2 where parent_id=' + quotedstr(TDynSiteForm(mSite).CurrentObject.oid) , mr);
                        if mr.count>0 then begin
                           for i:=0 to mr.count-1 do begin
                                        ...
                                        if index=0 then begin mi:=msite.BaseObjectSpace.SQLExecute('update busOrders bo set bo.X_Closed =''A'', bo.x_Change_closed=(select max(head.docdate$date) from issuedinvoices head left join issuedinvoices2 rox on rox.parent_id=head.id where rox.BusOrder_id=' +
                                         quotedstr(mr.Strings[i]) + ') where bo.id=' + quotedstr(mr.Strings[i]));
                                         mBO:=msite.BaseObjectSpace.CreateObject(Class_BusOrder);
                                         mbo.Load(mr.Strings[i],nil);
                                         mStream := TMemoryStream.Create;
