...
procedure mClosed(Sender: TMultiAction; Index: integer);
var
 mSite: TDynSiteForm;
 mDBGrid : TDBGrid;
 mTabList: TTabSheet;
...
mSite := TComponent(Sender).DynSite;
    ...
    if mDBGrid = nil then RaiseException('DBGrid nenalezen');
        ...
        if index=0 then begin
            msite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set DeliveredQuantity=Quantity where id=' + quotedstr(TDynSiteForm(mSite).CurrentObject.OID)) ;
        end;
        if index=1 then begin
           msite.BaseObjectSpace.SQLExecute('update IssuedOrders set Closed='+quotedstr('A') + ' where id=' + quotedstr(TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('Parent_ID'))) ;
        end;
...
mSite := TComponent(Sender).DynSite;
    ...
    if mDBGrid = nil then RaiseException('DBGrid nenalezen');
        ...
        if (mbo.GetFieldValueAsString('Storecard_ID')='4XN1000101') and (not NxIsEmptyOID(mbo.GetFieldValueAsString('X_parent_ID'))) then begin
            ...
            mquantity:=FloatToStr(mbo.GetFieldValueAsFloat('Quantity'));
            aaa:=InputQuery('Náhrada karty dle popisu: ',
                                 Trim(copy(mbo.GetFieldValueAsString('X_description'),1,100))+' z množství '+FloatToStr(mbo.GetFieldValueAsFloat('Quantity'))+ ' ' +mbo.GetFieldValueAsString('Qunit') + ' na množství v '+ mQunit,mquantity);


            if aaa=true then begin
                      mr:=TStringList.create;
                      try
                          ...
                          if mr.Count>0 then begin
                              for  i:=0 to mr.Count-1 do begin
                                ...
                                mbo_document:=mbo.ObjectSpace.CreateObject('I3CRLLN3XYVO30ZDQ2WB4CRZW0');       // řádek ML
                                    try
...
begin
  ...
  mMAction.ShowControl := True;
  mMAction.ShowMenuItem := True;
  mMAction.Caption := 'Vyčištění řádku bez vazby';
  mMAction.Hint := 'Vyřešení objednávky bez převodky';
  mMAction.Category := 'tabList';
  mMAction.OnExecuteItem := @MClosed;
  mMAction.Items.Add('Vyřešení řádku objednávky bez převodky');
  mMAction.Items.Add('Vyřešení dokladu objednávky bez převodky');
