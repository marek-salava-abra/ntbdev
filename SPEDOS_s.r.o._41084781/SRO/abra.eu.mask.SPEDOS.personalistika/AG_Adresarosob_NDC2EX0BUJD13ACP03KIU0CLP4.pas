...
begin
  ...
  mAction.ShowMenuItem := True;
  mAction.Category := 'tabList,tabDetail';
  mAction.Caption := 'Zaměstnanec ano/ne';
  mAction.OnExecute := @Zamestnanec_Click;
...
procedure Zamestnanec_Click(Sender: TObject);
var
  mSite: TSiteForm;
  ...
  if Sender is TComponent then begin
    ...
    if Assigned(mSite) and (mSite is TBusRollSiteForm) then begin
      mPerson:= TBusRollSiteForm(mSite).CurrentObject;
       mperson.SetFieldValueAsBoolean('IsEmployee', not(mPerson.GetFieldValueAsBoolean('IsEmployee')));
       mPerson.save;
       if mPerson.GetFieldValueAsBoolean('IsEmployee')=true then mString:='Ano';
       if mPerson.GetFieldValueAsBoolean('IsEmployee')=false then mString:='Ne';
       ShowMessage('Hodnota zaměstnec byla změněna na :' +mString);
       mPerson.free;
