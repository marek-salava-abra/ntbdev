...
begin
  ...
  mAction.Caption := 'Vynulováni ZLP';
...
begin
 ...
 if NxMessageBox('Dotaz','Přejete si vynulovat '+IntToStr(mlist.count)+' záloh?' , mdConfirm, mdbYesNo, 0, 0, False, msite)= mrYes then begin
  for i:=0 to mList.count-1 do begin
     mBO:=mOS.CreateObject(Class_ReceivedDepositInvoice);
     mBO.Load(mList.Strings[i],nil);
     if mbo.GetFieldValueAsFloat('PaidAmount')=0 then begin
       mOS.SQLExecute(format('update receiveddinvoices set amount=0,localamount=0 where id=''%s'' ',[mBO.OID]));
       mOS.SQLExecute(format('update receiveddinvoices2 set tamount=0,localtamount=0 where parent_id=''%s'' ',[mBO.OID]));
     end;
     mbo.Free;
  end;
 end;
