...
begin
    ...
    if mBookmark.Count= 0 then begin
        ...
        if mid='' then begin
           ...
           try
               msite.BaseObjectSpace.SQLSelect('Select SO.id from ServiceAssemblyForms SO where SO.X_Typ_poruchy_ID='+quotedstr(mbo.oid),mr_sd);
            ...
            if mi_SP+mi_SD+mi_SA=0 then begin
                ...
                if mI_Result=1 then mbo.Delete;
                 ...
                 if (mI_Result=1) or (mI_Result=6) then begin
                    mID:=iSelectTyp_poruchy(msite.GetAbraOLEApplication);
                           if mI_Result=1 then begin
                              if mID<>'' then begin
                                    if mI_SP>0 then mi:=msite.BaseObjectSpace.SQLExecute('update ServiceAssemblyForms set X_Typ_poruchy_ID=' + quotedstr(mID) + ' where X_Typ_poruchy_ID=' + quotedstr(mbo.OID)) ;
