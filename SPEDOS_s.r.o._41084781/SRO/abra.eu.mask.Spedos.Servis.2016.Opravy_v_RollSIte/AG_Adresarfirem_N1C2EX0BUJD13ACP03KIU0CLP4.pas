...
begin
    ...
    if mBookmark.Count= 0 then begin
        ...
        if mid='' then begin
           ...
           try
               msite.BaseObjectSpace.SQLSelect('Select SO.id from ServicedObjects SO where SO.PayerFirm_ID='+quotedstr(mbo.oid),mr_sd);
           ...
           mr_sd:=TStringList.create;
           try
               msite.BaseObjectSpace.SQLSelect('Select sd.id from ServiceDocuments SD left join ServiceDocStates SS on sd.ServiceDocState_ID=ss.id where sd.PayerFirm_ID='+quotedstr(mbo.oid) + ' and ss.PosIndex<=50',mr_sd);
               if mr_sd.count>0 then begin
               mI_SD:=mr_SD.Count;
               end;
            finally
            ...
            if mi_SP+mi_SD+mi_SA=0 then begin
                ...
                if mI_Result=1 then begin
                 ...
                 mI_Result:=Mformx(msite,'Upozornění. Smazat položku ?','Pozor, položka je použita v ' + inttostr(mI_SP) + ' servisovaných předmětech, v ' + inttostr(mI_SD) + ' SL a v ' + inttostr(mI_SA) +' ML.  Změnit ?', 'Vše a vymazat','Otevřené a skrýt','','Zrušit změny');
                 if (mI_Result=1) or (mI_Result=6) then begin
                    mID:=iSelectfirm(msite.GetAbraOLEApplication);
                           if mI_Result=1 then begin
                              if mID<>'' then begin
                                    if mI_SP>0 then mi:=msite.BaseObjectSpace.SQLExecute('update ServicedObjects set PayerFirm_ID=' + quotedstr(mID) + ' where PayerFirm_ID=' + quotedstr(mbo.OID)) ;
                                    if mI_SD>0 then mi:=msite.BaseObjectSpace.SQLExecute('update ServiceDocuments set PayerFirm_ID=' + quotedstr(mID) + ' where PayerFirm_ID=' + quotedstr(mbo.OID)) ;
                                    mbo.SetFieldValueAsBoolean('Hidden',true);
                                    mbo.save;
                              end;
                           end;
                           if mI_Result=6 then begin
                              if mID<>'' then begin
                                    ...
                                    if mI_SD>0 then mi:=msite.BaseObjectSpace.SQLExecute('update ServiceDocuments X set X.PayerFirm_ID=' + quotedstr(mID)  +
                                          'where (select ss.PosIndex from ServiceDocuments SD left join ServiceDocStates SS on ss.id=sd.ServiceDocState_ID where sd.PayerFirm_ID=' + quotedstr(mbo.OID)
                                          +' and x.id=sd.id)>15') ;
    ...
    try
        ...
        if mid='' then begin
          ...
          if mi_SP+mi_SD+mi_SA=0 then begin
                 ...
                 mI_Result:=Mformx(msite,'Upozornění','Pozor, položka je použita v ' + inttostr(mI_SP) + ' servisovaných předmětech, v ' + inttostr(mI_SD) + ' SL a v ' + inttostr(mI_SA) +' ML', '','','','Zrušit');
            end;
      end;
      finally
      mr.free;
