...
begin
  ...
  if not processSpecialGetRequests(AContext, AHeaders, APath, ABody, AResponse) then
  begin
    case APath.Strings[0] of
      ...
      'storeCardInfoPositions': get_StoreCardInfoPositions(mOS, APath, AResponse);
      'storePositionInfo': get_StorePositionInfo(mOS, APath, AResponse);
      'storePositionInfo2': get_StorePositionInfo2(mOS, APath, AResponse);
      'storeInfo': get_StoreInfo(mOS, APath, AResponse);
      'storeBatchInfo': get_StoreBatchInfo(mOS, APath, AResponse);
      'firmInfo': get_FirmInfo(mOS, APath, AResponse);
      'availableQuantity': get_AvailableQuantity(mOS, APath, AResponse);
      'temporaryStorage': Get_TemporaryStorage(mOS, APath, AResponse);
      'listDocQueue': listDocQueue(mOS, APath, AResponse);
      'listWithoutDocQueue': listWithoutDocQueue(mOS, APath, AResponse);
      'listPartialInvProtocols': listPartialInvProtocols(mOS, APath, AResponse);
      'listMainInvProtocols': listMainInvProtocols(mOS, APath, AResponse);
      'listStoreCardsForInv': listStoreCardsForInv(mOS, APath, AResponse);
      'listStorePositionsInv': listStorePositionsForInv(mOS, APath, AResponse);
      ...
      'storeCardInfoBatchesOnPositions': get_StoreCardInfoBatchesOnPosition(mOS, APath, AResponse);
