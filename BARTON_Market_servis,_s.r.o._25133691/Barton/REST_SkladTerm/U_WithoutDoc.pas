    ...
    try
      ...
      try
        ...
        if dtDocumentQuantity.RecordCount > 0 then
          ...
          begin
            mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_LogStoreOutput, gSkladTermUser_ID, gSkladTermDocType, mPreV);
        ...
        try
          ...
          mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_IncomingTransfer, gSkladTermUser_ID, gSkladTermDocType, mPreV);
          ...
          if dtDocumentQuantity.RecordCount > 0 then
          begin
            mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_LogStoreInput, gSkladTermUser_ID, DOC_IncomingTransfer, mPreP);
      ...
      begin
        ...
        try
          ...
          glog.WriteEvent(logDebug, 'mPreV.ChangeStatusBySwitchRule - begin - ' + mPreV.OID + ' - ' + PRECHOD_VYTVORENI(DOC_OutgoingTransfer, gSkladTermModule));
      ...
      begin
        ...
        try
          ...
          glog.WriteEvent(logDebug, 'mPreP.ChangeStatusBySwitchRule - begin - ' + mPreP.OID + ' - ' + PRECHOD_VYTVORENI(DOC_IncomingTransfer, gSkladTermModule));
...
var
  ...
  mTemporaryStorageID: Integer;
  jsonDocuments, jsonCustomFields: TJSONSuperObjectArray;
  dtJSONRows, dtDocumentQuantity: TMemTable;
  ...
  procedure FillCustomFields(ARow: TNxCustomBusinessObject);
  var
    j: Integer;
  begin
    // kontrola, zda jsou nejaka pole definovana
    ...
    begin
      if(jsonCustomFields.O[j].S['type'] = 'text') then
      begin
        ARow.SetFieldValueAsString(jsonCustomFields.O[j].S['field'], jsonCustomFields.O[j].S['textValue']);
      end
      else if(jsonCustomFields.O[j].S['type'] = 'number') then
      begin
        ARow.SetFieldValueAsInteger(jsonCustomFields.O[j].S['field'], jsonCustomFields.O[j].I['numberValue']);
      end;
    end;
  end;
        ...
        begin
          ...
          begin
            ...
            try
              mStoreBatch.New;
              mStoreBatch.Prefill;
              mStoreBatch.SetFieldValueAsString('StoreCard_ID', ARow.GetFieldValueAsString('StoreCard_ID'));
              mStoreBatch.SetFieldValueAsBoolean('SerialNumber', True);
              mStoreBatch.SetFieldValueAsString('Name', jsonSerNums.O[i].S['SerNumName']);
              mStoreBatch.SetFieldValueAsString('Specification', jsonSerNums.O[i].S['AuxText']);
              mStoreBatch.Save;
  ...
  begin
    ...
    try
      ...
      mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, mSDNewDocType, gSkladTermUser_ID, gSkladTermDocType, mSD);
      mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString :=
      ...
      FillLogStoreDocumentDataset(dtJSONRows, dtDocumentQuantity, mJSON_Root, 'RowType=1', mStoreField, mStorePositionField);

      // vytvorime polohovak
      if dtDocumentQuantity.RecordCount > 0 then
        if CreateLogStoreDocument(AOS, gSkladTermModule, DOC_IncomingTransformation, gSkladTermUser_ID, '', mSDNew, mJSON_Root, dtJSONRows, dtDocumentQuantity) then
        begin
          mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, DOC_LogStoreInput, gSkladTermUser_ID, DOC_IncomingTransformation, mSDNew);
    ...
    try
      ...
      try
        ...
        DataSet_CreataHeader(dtJSONRows, 'jsonIndex=I,StoreDocument2_ID=S10,StoreDocument2Provide_ID=S10,StoreDocument2ProvideRow_ID=S10,' +
          ...
          ',StoreBatchNew_ID=S10,StoreCardNewCategory=I,ContentUnit=S5,EnterBatchExpirationDate=B,StoreBatchExpirationDate=S16'
        ...
        begin
          mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, gSkladTermDocType, mSD);
        ...
        begin
          mRelNumber := GetRelationWithDocument(gSkladTermDocType);
          jsonDocuments := mJSON_Root.A['documents'];
          for i := 0 to jsonDocuments.Length - 1 do
          begin
            Relation_CreateAndSave(AOS, mRelNumber, mSD_ID, jsonDocuments.S[i], 1);
          end;
        end;
        ...
        begin
          ...
          if dtDocumentQuantity.RecordCount > 0 then
            ...
            begin
              mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, mLogStoreDocumentDocType, gSkladTermUser_ID, gSkladTermDocType, mSD);
      ...
      begin
        ...
        try
          ...
          glog.WriteEvent(logDebug, 'mPreV.ChangeStatusBySwitchRule - begin - ' + mSD.OID + ' - ' + PRECHOD_VYTVORENI(gSkladTermDocType, gSkladTermModule));
      ...
      begin
        ...
        try
          ...
          glog.WriteEvent(logDebug, 'mPreV.ChangeStatusBySwitchRule - begin - ' + mSD.OID + ' - ' + PRECHOD_VYTVORENI(mSDNewDocType, gSkladTermModule));
...
procedure listWithoutDocQueue(AOS: TNxCustomObjectSpace; APath, AResponse: TStringList);
var
  dtRows: TMemTable;
...
var
  mJson: TJSONSuperObject;
  jsonSerNums, jsonDocuments: TJSONSuperObjectArray;
  mBO, mRow, mStoreBatch: TNxCustomBusinessObject;
  mRows: TNxCustomBusinessMonikerCollection;
  ...
  begin
    ...
    try
      DataSet_CreataHeader(mDialogValues, REST_DialogValuesDatasetHeader);
      mDialogValues.Open;
      REST_JsonToDataSet(mJson.O['Dialog'].A['values'], mDialogValues);
  ...
  try
    ...
    try
      ...
      DataSet_CreataHeader(dtJSONRows, 'jsonIndex=I,StoreDocument2_ID=S10,StoreDocument2Provide_ID=S10,StoreDocument2ProvideRow_ID=S10,' +
        ...
        'StoreBatch_ID=S10,StorePositionTo_ID=S10,Processed=B,UnitQuantity=F,UnitRate=F,UnitCode=S10,Division_ID=S10,AuxText2=S100' +
        ',AuxText3=S100,AuxText4=S100,StoreCardCategory=I,StoreBatchNoteVisibility=S1,StoreBatchNote=S100,RowType=I,StoreCardNew_ID=S10' +
        ',StoreBatchNew_ID=S10,StoreCardNewCategory=I,ContentUnit=S5'
        + rowsDatasetFields(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID));
      dtJSONRows.AddIndex('ByJsonIndex', 'jsonIndex', [ixUnique]);
      dtJSONRows.IndexName := 'ByJsonIndex';
      dtJSONRows.Open;
      ...
      try
        ...
        try
          ...
          begin
            mDocQueue_ID := GetDocQueue_ID(AOS, gSkladTermModule, gSkladTermDocType, gSkladTermUser_ID, '', nil, mJson.S['Store_ID']);
          ...
          begin
            mRelNumber := GetRelationWithDocument(gSkladTermDocType);
            jsonDocuments := mJson.A['documents'];
            for i := 0 to jsonDocuments.Length - 1 do
            begin
              Relation_CreateAndSave(AOS, mRelNumber, mBO_ID, jsonDocuments.S[i], 1);
            end;
          end;
