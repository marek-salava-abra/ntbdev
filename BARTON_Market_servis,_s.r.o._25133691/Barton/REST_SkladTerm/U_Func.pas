...
function HTTP_Authorization(AOS: TNxCustomObjectSpace; ADeviceId, AAuthorization: String;
...
begin
  ...
  begin
    if ABRA then
      Result := not NxIsBlank(ADeviceId) and CheckLicense(AOS, ADeviceId, ErrDesc)
    else
  ...
  begin
    ...
    begin
      ErrDesc := Format(getString('not_licenced'), [ADeviceId]);
      glog.WriteEventFmt(logWarning, 'Authorization: Device "%s" not licensed', [ADeviceId])
    end;
  end;
...
begin
  ...
  begin
    ...
    gLog.WriteEventFmt(logDebug, 'DocumentQuantity - UPDATE - %s,%s,%s,%s,%f,%f,%s',
    ...
    dtDocumentQuantity.Post;
    gLog.WriteEventFmt(logDebug, 'DocumentQuantity - ADD    - %s,%s,%s,%s,%f,%f,%s',
      [AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID, AQuantity, AUnitRate, AContentUnit]);
  end;
end;
