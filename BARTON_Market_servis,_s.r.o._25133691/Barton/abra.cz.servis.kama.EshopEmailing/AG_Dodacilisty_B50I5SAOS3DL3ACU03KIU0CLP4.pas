...
begin
  if CheckDefinitionExists(Self.BaseObjectSpace, 21) then begin
    mAction := Self.GetNewMultiAction;
    mAction.ShowControl := True;
    mAction.ShowMenuItem := True;
    mAction.Caption := 'Výzva k odběru';
    mAction.Hint := 'Odeslání dokladu emailem - výzva k osobnímu odběru';
  ...
  if Sender is TComponent then begin
    ...
    if Assigned(mSite) then begin
      if mSite is TDynSiteForm then begin
        if (Index=1) or ((Index = 0) and (NxMessageBox('Dotaz', 'Provést odeslání výzvy e-mailem?', mdConfirm, mdbYesNo, 2, 0, False, nil) = mrYes)) then begin
          if CheckDefinitionExists(mSite.BaseObjectSpace, 21) then begin
            ...
            try
              ...
              for i := 0 to mIDs.Count - 1 do begin
                ...
                if Index = 0 then
                  mSave := EshopAction(mBO, 21);
                if Index = 1 then
                  mSave := EshopAction(mBO, 21, '', TDynSiteForm(mSite));
                if mSave then begin
