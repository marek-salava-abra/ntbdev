               ...
               mCode:=VarToStr(mSheet.Cells[i, 1]);
                 if not(NxIsBlank(mCode)) then begin
                  ...
                  if not(NxIsBlank(VarToStr(mSheet.Cells[i, 15]))) then
                   ...
                   mscbo.SetFieldValueAsString('X_BMS_Obal_ID', mOS.SQLSelectFirstAsString('Select id from defrolldata where clsid='+QuotedStr(Class_BMS_obal)+
                  ...
                  if NxIBStrToFloat(VarToStr(mSheet.Cells[i, 14]))>0 then begin
                    ...
                    mStorePriceBO.SetFieldValueAsString('PriceList_ID','1100000101');
