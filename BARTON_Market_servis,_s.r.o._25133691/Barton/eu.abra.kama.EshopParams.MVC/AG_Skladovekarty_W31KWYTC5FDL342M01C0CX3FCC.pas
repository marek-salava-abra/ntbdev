...
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'lbxParameters'));

    Result := TTabSheet.Create(APageControl);
    Result.PageControl := APageControl;
    Result.Caption := 'Parametry výrobku';
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'lbxParameters'));
    mGrd.Parent := mPnl;
    mGrd.Name := 'grdParams';
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Vlastnost';
    ...
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := cRollPropertyNameCLSID;
    ...
    mGridColumn.Caption := 'Název vlastnosti';
    ...
    mAct := Self.GetNewAction;
    mAct.Name := 'actAddMore';
    mAct.Caption := 'Přidat k více prod.';
    mAct.Category := Result.Name;
    mAct.Hint := 'Přidá aktuálně vybraný parametr k více jiným produktům';
    mAct.OnExecute := @actAddMoreExecute;
    if not mCanEdit then
      mAct.Tag := -1;
    mListBox.Items.AddObject('actAddMore', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'actRemoveMore';
    mAct.Caption := 'Odebrat z více prod.';
    mAct.Category := Result.Name;
    mAct.Hint := 'Odebere aktuálně vybraný parametr z více jiných produktů';
    mAct.OnExecute := @actRemoveMoreExecute;
    if not mCanEdit then
      mAct.Tag := -1;
    mListBox.Items.AddObject('actRemoveMore', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'actLSRefresh';
    mAct.Caption := 'Občerstvit';
    mAct.Category := Result.Name;
    mAct.Hint := 'Občerstvení seznamu parametrů';
    ...
    try
      ...
      if mTmpDts.Active then begin
        ...
        while not mTmpDts.Eof do begin
          if not ParamExists(mDataset, mTmpDts.FieldByName('Property_ID').asstring, mTmpDts.FieldByName('RollValue').asstring) then begin
            mDataSet.Append;
...
function ParamExists(ADts: TDataset; APropertyID, ARollValue: string): Boolean;
begin
  result := false;
  ADts.First;
  while not ADts.Eof do begin
    if (ADts.FieldByName('Property_ID').AsString = APropertyID) and (ADts.FieldByName('RollValue').AsString = ARollValue) then begin
      result := true;
      exit;
    end;
    ADts.Next;
...
begin
  ...
  if Sender.Tag = -1 then begin
    NxShowMessage(Application.Title, 'Nemáte oprávnění editace parametrů skladových karet', mdInformation, False, mSite.GetSiteAppForm);
    Exit;
  end;
  mListBox := TListBox(NxFindChildControl(mSite, 'lbxParameters'));
...
//zkopíruje paramert na více jiných karet

procedure actAddMoreExecute(Sender: TAction);
var mSite: TSiteForm;
  j, mCnt: Integer;
  mGrid: TMultiGrid;
  mListBox: TListBox;
...
begin
  ...
  if Assigned(mDataSet) then begin
    mRol := TOpenRolSite.Create(mSite.SiteContext, Roll_StoreCards);
    mRol.MultiChoice := True;
    mRol.Open;
    mCnt := 0;
    for j := 0 to mRol.SelectedList.Count - 1 do begin
      mParamID := GetParamIDFromStorecard(mSite.BaseObjectSpace, mRol.SelectedList.Strings[j], mDataSet.FieldByName('ID').AsString);
      if NxIsEmptyOID(mParamID) then begin
        AddParamToStorecard(mSite.BaseObjectSpace, mRol.SelectedList.Strings[j], mDataSet.FieldByName('ID').AsString);
        inc(mCnt);
      end;
    end;
    NxShowMessage('Informace', Format('Počet nově přidaných parametrů: %s',[IntToStr(mCnt)]), mdInformation, false, mSite.FindParentForm);
  end;
end;

function GetParamIDFromStorecard(AOS: TNxCustomObjectSpace; AStorecardID, AParamID: string): string;
var mSQL, mSQL2: string;
  mTmpDts: TDataset;
  mStr: TStrings;
begin
  result := '';
  mSQL := 'Select * from DefRollData PAR where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) +
    ' and PAR.ID=' + QuotedStr(AParamID);
  mTmpDts := TMemoryDataset.Create(nil);
  try
    AOS.SQLSelect2(mSQL, mTmpDts);
    if mTmpDts.Active then begin
      mTmpDts.First;
      while not mTmpDts.Eof do begin
        mSQL2 := 'Select ID from DefRollData PAR where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) +
          ' and X_Storecard_ID=' + QuotedStr(AStorecardID) +
          ' and X_Property_ID=' + QuotedStr(mTmpDts.FieldByName('X_Property_ID').AsString) +
          ' and X_RollValue=' + QuotedStr(mTmpDts.FieldByName('X_RollValue').AsString); {+
          ' and X_StorecardType_ID=' + QuotedStr(mTmpDts.FieldByName('X_StorecardType_ID').AsString);}
        mStr := TStringList.Create;
        try
          AOS.SQLSelect(mSQL2, mStr);
...
procedure AddParamToStorecard(AOS: TNxCustomObjectSpace; AStorecardID, AParamID: string);
var
  mSQL: string;
  mTmpDts: TDataset;
  ...
  if not NxIsEmptyOID(AStorecardID) then begin
    ...
    try
      ...
      if mTmpDts.Active then begin
        ...
        mBO := AOS.CreateObject(cBOPropertyCLSID);
        try
          mTmpDts.First;
          while not mTmpDts.Eof do begin
            mBO.New;
            mBO.Prefill;
            mBO.SetFieldValueAsString('X_Storecard_ID', AStorecardID);
            mBO.SetFieldValueAsString('X_Property_ID', mTmpDts.FieldByName('X_Property_ID').AsString);
            {mBO.SetFieldValueAsString('X_StorecardType_ID', mTmpDts.FieldByName('X_StorecardType_ID').AsString);}
            mBO.SetFieldValueAsInteger('X_DataType', mTmpDts.FieldByName('X_DataType').AsInteger);
            mBO.SetFieldValueAsString('X_RollValue', mTmpDts.FieldByName('X_RollValue').AsString);
            mBO.SetFieldValueAsString('X_Value', mTmpDts.FieldByName('X_Value').AsString);
            mBO.SetFieldValueAsString('X_Value_SK', mTmpDts.FieldByName('X_Value_SK').AsString);
            mBO.SetFieldValueAsString('X_Value_ENUS', mTmpDts.FieldByName('X_Value_ENUS').AsString);
            mBO.SetFieldValueAsString('X_Value_DE', mTmpDts.FieldByName('X_Value_DE').AsString);
            mBO.SetFieldValueAsString('X_PosIndex', IntToStr(GetPosIndexFromSCParams(AOS, AStorecardID) + 10));//mTmpDts.FieldByName('X_PosIndex').AsString);
            mBO.Save;
            mTmpDts.Next;
          end;
        finally
...
function GetPosIndexFromSCParams(AOS: TNxCustomObjectSpace; AStorecardID: string): integer;
var mSQL: String;
    mStr: TStrings;
    mStrPosIndex: String;
    mPosInt: Integer;
begin
  result := 0;
  mSQL := 'Select max(X_PosIndex) from DefRollData where CLSID=' + QuotedStr(cBOPropertyCLSID) + ' and X_Storecard_ID=' + QuotedStr(AStorecardID);
  mStr := TStringList.Create;
  AOS.SQLSelect(mSQL, mStr);
  try
    if mStr.Count >0 then begin
     mStrPosIndex := mStr.Strings[0];
     if TryStrToInt(mStrPosIndex, mPosInt) then begin
       result := mPosInt;
     end;
    end;
  finally
...
//odstraní parametr z více jiných karet

procedure actRemoveMoreExecute(Sender: TAction);
var mSite: TSiteForm;
  mListBox: TListBox;
  mDataSet: TMemoryDataSet;
  mRol: TOpenRolSite;
...
begin
  ...
  if Assigned(mDataSet) then begin
    ...
    for j := 0 to mRol.SelectedList.Count - 1 do begin
      mParamID := GetParamIDFromStorecard(mSite.BaseObjectSpace, mRol.SelectedList.Strings[j], mDataSet.FieldByName('ID').AsString);
      if not NxIsEmptyOID(mParamID) then begin
        RemoveParamFromStorecard(mSite.BaseObjectSpace, mParamID);
        inc(mCnt);
      end;
    end;
    NxShowMessage('Informace', Format('Počet odebraných parametrů: %s', [IntToStr(mCnt)]), mdInformation, false, mSite.FindParentForm);
  end;
end;

procedure RemoveParamFromStorecard(AOS: TNxCustomObjectSpace; AParamID: string);
var mBO: TNxCustomBusinessObject;
begin
  mBO := AOS.CreateObject(cBOPropertyCLSID);
  mBO.Load(AParamID, nil);
  mBO.Delete;
end;
...
begin
  if Sender.Enabled then begin
    mSite := NxFindSiteForm(Sender);
    mListBox := TListBox(NxFindChildControl(mSite, 'lbxParameters'));
    if NxMessageBox('Potvrzení', 'Opravdu zrušit provedené zmeny ?', mdConfirm, mdbYesNo, 1, nil, False, mSite.GetSiteAppForm) = mrNo then Exit;
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'lbxParameters'));
  mDataSet := TMemoryDataSet(mListBox.Items.Objects(mListBox.Items.IndexOf('dsParams')));
      ...
      while not mDataSet.Eof do begin
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            mBO.SetFieldValueAsString('X_Storecard_ID', mOID);
            {mBO.SetFieldValueAsString('X_StorecardType_ID', mDataSet.FieldByName('StorecardType_ID').AsString);}
            mBO.SetFieldValueAsString('X_Property_ID', mDataSet.FieldByName('Property_ID').AsString);
            mBO.SetFieldValueAsString('X_RollValue', mDataSet.FieldByName('RollValue').AsString);
            mBO.SetFieldValueAsInteger('X_DataType', mDataSet.FieldByName('DataType').AsInteger);
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    {' PAR.X_StorecardType_ID as StorecardType_ID, SCT.X_VALUE as StorecardType_Name,' +}
    ' PAR.X_Property_ID as Property_ID, PRO.X_VALUE as Property_Name,' +
    ' PAR.X_RollValue as RollValue, ROV.X_VALUE as RollValue_Name,' +
    ' PAR.X_DataType as DataType,' +
    ' PAR.X_PosIndex as PosIndex' +
    ' from DefRollData PAR ' +
    {' LEFT JOIN DefRollData SCT ON SCT.ID=PAR.X_StorecardType_ID and SCT.CLSID=' + QuotedStr(cBOSCTypeCLSID) +}
    ' LEFT JOIN DefRollData PRO ON PRO.ID=PAR.X_Property_ID and PRO.CLSID=' + QuotedStr(cBOPropertyNameCLSID) +
    ' LEFT JOIN DefRollData ROV ON ROV.ID= PAR.X_RollValue and ROV.CLSID=' + QuotedStr(cBORollValueCLSID) +
    ' where PAR.HIDDEN = ' + QuotedStr('N') + ' AND PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) + ' and PAR.X_Storecard_ID=%s order by PAR.X_PosIndex';
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          ...
          try
            ...
            DataSet.FieldByName('Property_ID_Name').AsString := mBO.GetFieldValueAsString('X_VALUE');
          finally
            mBO.Free;
          end;
      ...
      if not NxIsEmptyOID(mRollValue) then begin
        ...
        if mRollValueName = '' then begin
          ...
          try
            ...
            DataSet.FieldByName('RollValue_Name').AsString := mBO.GetFieldValueAsString('X_VALUE');
          finally
            mBO.Free;
          end;
