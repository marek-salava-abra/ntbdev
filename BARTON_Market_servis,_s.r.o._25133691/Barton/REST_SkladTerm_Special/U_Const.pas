...
const
  ...
  NoncancelableWork            = True; // zda je mozne prerusit praci na dokladu bez smazani rozpracovaneho stavu
  ...
  ShowDocumentQueueCount       = True; // zda se v hlavnim menu maji zobrazit u tlacitek pocty dokladu ve fronte
  ...
  SKLAD_HLAVNI      = '1210000101'; // pouzije se napr. pro predvyplneni skladu, taky pri vytvoreni novych volnych dokladu
  FIRM_OWN          = 'X4P0000101'; // vlastni firma
  STREDISKO_HLAVNI  = '2100000101'; // středisko

  RADA_PRIJEMKA           = 'O600000101'; // rada Prijemka
  RADA_PREVYDEJ           = 'Q600000101'; // rada Prevodka vydej
  RADA_PREPRIJEM          = 'S600000101'; // rada Prevodka prijem
  RADA_VYDEJKA            = 'P600000101'; // rada Vydejka
  RADA_VRATKA_VYDEJKY     = 'R600000101'; // rada Vratka vydejky
  RADA_DILCI_INV_PROTOKOL = 'NB00000101'; // rada Dilci inventarni protokol
  ...
  LogStoreOutput_StoreGateway_ID = '1010000101'; // Nasklad. a vyskladnovaci misto pro Vyskladneni z pozic
  LogStoreInput_DocQueue_ID      = '1900000101'; // rada Naskladneni do pozic
  LogStoreInput_StoreGateway_ID  = '2010000101'; // Nasklad. a vyskladnovaci misto pro Naskladneni do pozic
  ...
  CLIENT_CURRENT_VERSION = '5.10.3';    // aktualni verze, ctecky si kontroluji, jestli ji maji
  CLIENT_CURRENT_VERSION_URL = 'http://192.168.211.114:8082/SkladTerm-release-5.10.3-20231011.apk'; // adresa na APK nove verze - musi byt vyplneno pri aktualizovani z verze 5.2.1 a nizsich (AG-6185)
  CLIENT_CURRENT_VERSION_DOCUMENT_ID = '1040000101'; // ID dokumentu v systému, pod kterým je uloženo APK - musi vyplneno pri aktualizaci z verze 5.3.1 a novejsich (AG-6185)
...
begin
  Result :='STD_ReceiptCardQueue;STD_BillOfDeliveryQueue' +
      ';STD_InfoFromStores;STD_AddingEANs' +
      ';STD_TransferBetweenPositions;STD_TransferBetweenPositionsToAux' +
      ';STD_TransferBetweenPositionsFromAux;STD_TransferQueue' +
      ';STD_TransferWithoutDoc;STD_TransferInQueue' +
      ';STD_InventorizationFree;ABRA_ReceivedOrderWithoutDoc' +
      ';ABRA_MaterialDistributionQueue;SWISS_ProductReceptionQueue' +
      ';STD_ReceiptCardWithoutDoc';

     // JOKO 'ABRA_ReceiptCardDamagedQueue;STD_ShippingListsQueue;STD_RemovalListsQueue;STD_ReceiptCardQueue;STD_BillOfDeliveryQueue;STD_TransferQueue;STD_InfoFromStores;STD_TransferBetweenPositions;STD_TransferBetweenPositionsToAux;STD_TransferBetweenPositionsFromAux' +
     //';STD_ReceiptCardFastQueue;STD_BillOfDeliveryFastQueue;STD_ReceiptCardFastWithoutDoc;STD_BillOfDeliveryFastWithoutDoc;STD_TransferWithoutDoc;STD_ReceiptCardWithoutDoc;STD_BillOfDeliveryWithoutDoc' +
     //';STD_InventorizationFree;ABRA_PLAB_FreePrint;ABRA_PLAB_ReceiptCardQueue';
    {';STD_TransferWithoutDoc;STD_InventorizationByDIP;STD_InventorizationFree;STD_BillOfDeliveryWithoutDoc;STD_ReceiptCardWithoutDoc;SWISS_MaterialDistributionQueue;SWISS_ProductReceptionQueue' +
    ';ABRA_ReceiptCardEANQueue;STD_TransferInQueue;ABRA_RefMaterialDistributionQueue;ABRA_ReceivedOrderWithoutDoc;ABRA_OrdersRequestsWithoutDoc;ABRA_MaterialDistributionQueue;STD_RefundedBillOfDeliveryQueue;ABRA_KLAUSTIMBER_BillOfDeliveryWithoutDoc' +
    ';ABRA_VASKY_ReceiptCardQRQueue;ABRA_RAYCOM_ReceiptCardPrintQueue;ABRA_KONSEPTI_BillOfDeliveryQueue';
  {if AUserStore_ID = SKLAD_HLAVNI then
    Result := 'STD_ReceiptCardQueue;STD_BillOfDeliveryQueue;STD_TransferQueue;STD_InfoFromStores;STD_TransferBetweenPositions;STD_TransferBetweenPositionsToAux;STD_TransferBetweenPositionsFromAux'
  else
    Result := 'DOBR_DispatchPickup;STD_InfoFromStores;DOBR_TransferBetweenLocations;DOBR_TransferBetweenPositions;DOBR_InputQueue;DOBR_InputWithoutDoc';}
end;
...
begin
  case docTyp of
    ...
    DOC_IncomingTransfer:               Result := 'V040000101';
