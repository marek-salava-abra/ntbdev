...
begin
 if AFieldCode = Self.GetFieldCode('Quantity') then begin
    if AValue.AsFloat<>0 then begin
        mr1:= TStringList.Create;
                Try
                        self.ObjectSpace.SQLSelect(format('SELECT id FROM StoreUnits WHERE (parent_ID=''%s'') and (code=''%S'')',
                        [self.GetFieldValueAsString('Storecard_ID'),Self.GetFieldValueAsString('Qunit')]),mr1);
                       if mr1.Count=0 then begin
                                ShowMessage('Skladová jednotka neni podporována,prosím opravte');
                                ...
                                Try
                                        ...
                                        mBO_Storeunit.GetFieldValueAsFloat('UnitRate') *  AValue.AsFloat ;
                                        self.SetFieldValueAsFloat('Totalprice',pocet);
                                finally
                                mBO_Storeunit.free;
                                end;
...
begin
  if NxIsEmptyOID(self.GetFieldValueAsString('BusOrder_ID')) then self.SetFieldValueAsString('BusOrder_ID','2030000101');
  if NxIsEmptyOID(self.GetFieldValueAsString('Division_ID')) then self.SetFieldValueAsString('Division_ID','2100000101');
