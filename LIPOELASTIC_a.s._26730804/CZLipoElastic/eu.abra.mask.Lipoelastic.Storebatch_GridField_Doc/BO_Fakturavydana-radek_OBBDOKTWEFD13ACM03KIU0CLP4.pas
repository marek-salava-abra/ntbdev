...
var
  ...
  mBillOfDeliverRowBO:TNxCustomBusinessObject;
  mr1:TStringList;
begin
  ...
  if AFieldCode = Self.GetFieldCode('X_StoreBatch_ID') then begin
    if  (not NxIsEmptyOID(Self.GetFieldValueAsString('X_StoreBatch_ID'))) then begin
        if AOriginalValue.AsString<> AValue.AsString then begin
                ...
                Try
                        self.ObjectSpace.SQLSelect(format('SELECT code FROM StoreUnits WHERE (parent_ID=''%s'') and (Description=''%S'')',
                       ...
                       if mr1.Count=0 then begin
                             ...
                             {   //ShowMessage('Skladová jednotka pro šarži nalezena');
                                self.SetFieldValueAsString('QUnit',mr1.Strings[0]);
                                mBillOfDeliverRowBO:=self.GetMonikerForFieldCode(self.GetFieldCode('ProvideRow_ID')).BusinessObject;
                                 mBO_Moniker:=mBillOfDeliverRowBO.GetLoadedCollectionMonikerForFieldCode(mBillOfDeliverRowBO.GetFieldCode('DocRowBatches'));
