...
begin
  ...
  if Assigned(mMG) then begin
        b := True;
        for i:=mMG.ColumnCount-1 downto 0 do
          if mMG.Columns[i].FieldName = 'TotalPercentualDiscount' then
            b := False;
        if b then begin
          mFieldDef := TFieldDef.Create(mMG.DataSource.DataSet.FieldDefs, 'TotalPercentualDiscount', ftFloat, 0, False, 302);
          with mFieldDef.CreateField(mMG.DataSource.DataSet, nil, 'TotalPercentualDiscount', False) do begin
            ReadOnly:= true;
            FieldName:= 'TotalPercentualDiscount';
            FieldKind:= fkData;
          end;
          iPreparePosition(3, 0, 75);
          mMGColRowDiscount:= TNxMultiGridColumn.Create(mMG.Owner);
          mMGColRowDiscount.FieldName := 'TotalPercentualDiscount';
          mMGColRowDiscount.Caption := '#% sleva';
          mMGColRowDiscount.ReadOnly := true;
          mMGColRowDiscount.Kind := ckText;
          mMGColRowDiscount.Elastic := false;
          mMGColRowDiscount.Width := 70;
          mMGColRowDiscount.Layout := 3;
          mMGColRowDiscount.Line := 0;
          mMGColRowDiscount.Order := 6;
          mMG.AddColumn(mMGColRowDiscount);
        end;
 ...
 if Assigned(mMG) then begin
        b := True;
        for i:=mMG.ColumnCount-1 downto 0 do
          if mMG.Columns[i].FieldName = 'TAmount' then
            b := False;
        if b then begin
          mFieldDef := TFieldDef.Create(mMG.DataSource.DataSet.FieldDefs, 'TAmount', ftFloat, 0, False, 307);
          with mFieldDef.CreateField(mMG.DataSource.DataSet, nil, 'TAmount', False) do begin
            ReadOnly:= true;
            FieldName:= 'TAmount';
            FieldKind:= fkData;
          end;
          iPreparePosition(3, 0, 76);
          mMGColTAmount:= TNxMultiGridColumn.Create(mMG.Owner);
          mMGColTAmount.FieldName := 'TAmount';
          mMGColTAmount.Caption := '#Po slevě s DPH %';
          mMGColTAmount.ReadOnly := true;
          mMGColTAmount.Kind := ckText;
          mMGColTAmount.Elastic := false;
          mMGColTAmount.Width := 70;
          mMGColTAmount.Layout := 3;
          mMGColTAmount.Line := 0;
          mMGColTAmount.Order := 7;
          mMG.AddColumn(mMGColTAmount);
        end;
