...
procedure Busxxx_Doplneni(Self: TNxCustomBusinessObject);
begin
              // *** obchodní prípad ***
              ...
              // *** zakázka ***
              if NxIsEmptyOID(Self.getFieldValueAsString('BusOrder_ID')) then begin
                        //  mBusOrder_ID:=GetBusOrder_ID(self);
                                if not nxisblank(mBusOrder_ID) then self.SetFieldValueAsString('BusOrder_id',mBusOrder_ID);

              end;
              if NxIsEmptyOID(Self.getFieldValueAsString('BusProject_ID')) then begin
