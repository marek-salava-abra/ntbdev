uses 'abra.eu.mask.2017.vyroba.Libs';
var
xSite: TDynSiteForm;
mDBGrid : TDBGrid;
mTabList: TTabSheet;
...
function mManager (OS:TNxCustomObjectSpace;mCLSInput:string;mCLSOutput:string;mBO_input:string;mBO_output:string;mRowsInput:TNxCustomBusinessMonikerCollection;mRowsOutput:TNxCustomBusinessMonikerCollection;mParams:TNxParameters):boolean;
var
...
begin
 ...
 mxpocet:=0;
  ...
  try
        if NxIsEmptyOID(TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('X_Cilovy_sklad')) then begin
        ...
        try
                ...
                try
                  ...
                  if NxIsEmptyOID(TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('X_Cilovy_sklad')) then begin
                        mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString := TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('DocQueue_ID.X_Delivery_ID');
                  end else begin
                     mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString := TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('DocQueue_ID.X_Prevodka_ID');
                       end;
                  ...
                  if not (NxIsEmptyOID(TDynSiteForm(mSite).CurrentObject.GetFieldValueAsString('X_Cilovy_sklad'))) then begin
                  ...
                  for ii:=0 to mRowsOutput.Count-1 do begin
                      if ((mRowsOutput.BusinessObject[ii].GetFieldValueAsString('Storecard_id.StoreCardCategory_ID.Code')='PB')
                                                 or (mRowsOutput.BusinessObject[ii].GetFieldValueAsString('Storecard_id.StoreCardCategory_ID.Code')='Px')) then begin
                                                    ...
                                                    //mxpocet:=mxpocet-1;
                                                 //end else begin
                                                 //NxShowSimpleMessage('Doprava',nil);
                                                 mxpocet:=mxpocet+1;
                  ...
                  if mxpocet>0 then begin mManager.OutputDocument.Save;
  ...
  try
        ...
        mDBGrid := TMultiGrid(NxFindChildControl(TDynSiteForm(NxFindSiteForm(Sender)).MainPanel, 'grdRows'));
            ...
            //mActualRow := mDBGrid.DataSource.DataSet.GetBookmark;
                  ...
                  try
                      msite.BaseObjectSpace.SQLSelect('select sum(X_vychystano) from Receivedorders2 where parent_id=' + quotedstr(mbo.oid),mr);
                      if NxIBStrToFloat(mr.Strings[0])>0 then begin
                          NxShowSimpleMessage('Doklad je již vychystáván, ale Neodepsán. Množství bude připočítáno k aktuálním stavům, nebo nejprve odepište',mSite);
                          if false then begin
...
begin
    ...
    mTabList := TTabSheet(NxFindChildControl(xSite.MainPanel, 'tabList'));
                ...
                mManager := NxCreateDocumentImportManager(xsite.BaseObjectSpace,'01CPMINJW3DL342X01C0CX3FCC','05DOXDMCSZDL3FUD00C5OG4NF4');

                //mBO := xsite.BaseObjectSpace.CreateObject('01CPMINJW3DL342X01C0CX3FCC');
                mParams := TNxParameters.Create();
                //mList := tStringlist.create;
                try
                  mManager.AddInputDocument(TDynSiteForm(xSite).CurrentObject.OID);
                  mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString := TDynSiteForm(xSite).CurrentObject.GetFieldValueAsString('DocQueue_ID.X_delivery_ID');
                  mManager.LoadParams(mParams);
                  mManager.Execute;
                  mManager.OutputDocument.SetFieldValueAsString('Firm_ID',mManager.InputDocument.GetFieldValueAsString('Firm_ID'));
                  mManager.OutputDocument.SetFieldValueAsString('Description',mManager.InputDocument.GetFieldValueAsString('Description'));
                  ...
                  //for ii:=0 to mRows.Count-1 do begin
                  //    mRows.BusinessObject[ii].SetFieldValueAsFloat('Quantity',0);
                  //end;
                  mvychystano:=0;
                  while mPokracovani do begin
                     mPokracovani:=InputQuery('Dohledání položky','EAN',mEAN);
                         mr:=TStringList.create;
                         try
                            xSite.BaseObjectSpace.SQLSelect('select su.Parent_ID from StoreEANs SE left join StoreUnits Su on su.id=se.Parent_ID where se.ean=' + quotedstr(mEAN),mr);
                            if mr.count>0 then begin
                               if mr.count=1 then begin
                                   mRows := mManager.OutputDocument.GetLoadedCollectionMonikerForFieldCode(mManager.OutputDocument.GetFieldCode('Rows'));
                                   for ii:=0 to mRows.Count-1 do begin
                                          ...
                                          if mRows.BusinessObject[ii].getFieldValueAsString('Storecard_id')=mr.Strings[0] then begin
                                                ...
                                                if index=0 then begin
                                                    mpocet:=mRows.BusinessObject[ii].GetFieldValueAsFloat('Quantity');
                                                    mpocet:= mpocet+1;
                                                    mRows.BusinessObject[ii].SetFieldValueAsFloat('Quantity',mpocet);
                                                 end;
                                                if index=1 then begin
                                                     mpocet:=mRows.BusinessObject[ii].GetFieldValueAsFloat('Quantity');
                                   ...
                                   if mEAN<>'' then NxShowSimpleMessage('Pro ean ' + mEAN + ' je v systému více skladových karet',nil);

                               end;
                             end else begin
                                if mEAN<>'' then NxShowSimpleMessage('Pro ean ' + mEAN + ' není v systému žádná skladová karta',nil);
                             end;
                  ...
                  for ii:=0 to mRows.Count-1 do begin
                      if mRows.BusinessObject[ii].getFieldValueAsFloat('Quantity')=0 then begin
                          mRows.BusinessObject[ii].MarkForDelete;
                      end;
                  end;

                  xSite.ShowDynFormWithNewDocument('B50I5SAOS3DL3ACU03KIU0CLP4', xsite.SiteContext, mManager.OutputDocument);
                  //mManager.OutputDocument.Save;
                  //NxShowSimpleMessage('Byl úspěšně vytvořen doklad',nil);
