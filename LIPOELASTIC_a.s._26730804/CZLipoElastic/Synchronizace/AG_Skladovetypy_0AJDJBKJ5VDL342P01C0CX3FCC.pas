...
const
mTable='StoreCardCategories';
mApiTable='StoreCardCategories';
...
begin
     mQuery:='{}';
                  ...
                  mquery:=mquery + ', "Hidden": '  +  BoolToStr(Self.GetFieldValueAsBoolean('Hidden')) +' ' ;
                                    ...
                                    mquery:=mquery + ', "AnalyticalAccount":"'  +  Self.GetFieldValueAsString('AnalyticalAccount') +'" ' ;
                                    mquery:=mquery + ', "CostPriceSourceType":"'  +  Self.GetFieldValueAsString('CostPriceSourceType') +'" ' ;

                                    mquery:=mquery + ', "IntrastatStatus":"'  +  inttostr(Self.GetFieldValueAsInteger('IntrastatStatus')) +'" ' ;
                                    mquery:=mquery + ', "IntrastatTransportStatus":"'  +  inttostr(Self.GetFieldValueAsInteger('IntrastatTransportStatus')) +'" ' ;

                                    mquery:=mquery + ', "ToAccountBillOfDelivery":'  +  booltostr(Self.GetFieldValueAsBoolean('ToAccountBillOfDelivery')) +' ' ;
                                    mquery:=mquery + ', "ToAccountInventory":'  +  booltostr(Self.GetFieldValueAsBoolean('ToAccountInventory')) +' ' ;
                                    mquery:=mquery + ', "ToAccountReceiptCard":'  +  booltostr(Self.GetFieldValueAsBoolean('ToAccountReceiptCard')) +' ' ;
                                    mquery:=mquery + ', "ToAccountTransfer":'  +  booltostr(Self.GetFieldValueAsBoolean('ToAccountTransfer')) +' ' ;
                                    mquery:=mquery + ', "SplitIntrastat":'  +  booltostr(Self.GetFieldValueAsBoolean('SplitIntrastat')) +' ' ;
                                    mquery:=mquery + ', "IntrastatTransport":'  +  booltostr(Self.GetFieldValueAsBoolean('IntrastatTransport')) +' ' ;

                                 //   mquery:=mquery + ', "X_Syntetika_nakladu":"'  +  inttostr(Self.GetFieldValueAsInteger('X_Syntetika_nakladu')) +'" ' ;
                                 //   mquery:=mquery + ', "X_Syntetika_vynosu":"'  +  inttostr(Self.GetFieldValueAsInteger('X_Syntetika_vynosu')) +'" ' ;
                                 //   mquery:=mquery + ', "X_Syntetika_stavu":"'  +  inttostr(Self.GetFieldValueAsInteger('X_Syntetika_stavu')) +'" ' ;

                                //    mquery:=mquery + ', "U_Text1":"'  +  Self.GetFieldValueAsString('U_Text1') +'" ' ;
                                //    mquery:=mquery + ', "U_Text2":"'  +  Self.GetFieldValueAsString('U_Text2') +'" ' ;
                                //    mquery:=mquery + ', "X_Tuzemsky":"'  +  Self.GetFieldValueAsString('X_Tuzemsky') +'" ' ;
                               //     mquery:=mquery + ', "X_vEU":"'  +  Self.GetFieldValueAsString('X_vEU') +'" ' ;
                               //     mquery:=mquery + ', "X_mimoEU":"'  +  Self.GetFieldValueAsString('X_mimoEU') +'" ' ;
                               //     mquery:=mquery + ', "X_tisk_stitku":"'  +  Self.GetFieldValueAsString('X_tisk_stitku') +'" ' ;
                                     mquery:=mquery + '}' ;
...
begin
     mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (Code,Name,ID,Hidden) VALUES (' +
...
//                                    end;
                        ...
                        IF mid='' THEN BEGIN
                            //NxShowSimpleMessage('Nový záznam se stejným ID',nil);
                                ...
                                NxShowSimpleMessage(mNewQuery,nil);
