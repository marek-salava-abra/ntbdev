...
begin
 result:='';
   {mTargetList:=tstringlist.create;
    TRY
     xsite.BaseObjectSpace.SQLSelect('Select a.code&A.X_CLSID FROM DefRollData A WHERE (A.Hidden = ' + quotedstr('N') + ' ) AND (A.CLSID = '+ quotedstr('C2HCXLEGT5H4340LIKNWZLXHI0') +
                                    ' ) AND (UPPER(A.X_Parent_ID) =' + quotedstr('ABRAAPI') + ')',mTargetList);
    if mTargetList.count=0 then begin
          NxShowSimpleMessage('Není možné synchronizovat',nil);
          exit;
    end;
    }
    mTargetList:=tstringlist.create;
    ...
    for i:=0 to mTargetList.count-1 do begin // ****cyklus pro jednotlicá spojení
                ...
                i:=strtoint(trim(copy(mTargetList.strings[i],1,1)));
