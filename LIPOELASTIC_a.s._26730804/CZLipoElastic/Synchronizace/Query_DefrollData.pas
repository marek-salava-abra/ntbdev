...
begin
                  ...
                  if iTarget=1 then  begin
                         mQuery:=mquery + ', "name":"' +                                   copy(Self.GetFieldValueAsString('Name'),1,80) +'" '                             ;
                  end;
                  if iTarget=2 then begin
                        if trim(Self.GetFieldValueAsString('X_SK_Nazev')) <>'' then begin
                            mQuery:=mquery + ', "name":"' +                                   copy(Self.GetFieldValueAsString('X_SK_Nazev'),1,80) +'" '                             ;
                  ...
                  mquery:=mquery + ', "X_Parent_ID":"'  +  Self.GetFieldValueAsString('X_Parent_ID') +'" ' ;
                  mquery:=mquery + ', "X_CLSID":"'  +  Self.GetFieldValueAsString('X_CLSID') +'" ' ;
