...
const
mTable='StoreAssortmentGroups';
mApiTable='StoreAssortmentGroups';
...
begin
    mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (Code,Name,ID,Hidden) VALUES (' +
                                                    ...
                                                    + ','+ quotedstr(Self.OID)
                                                    + ','+ BoolToStr(Self.GetFieldValueAsBoolean('Hidden'))
                                                    + ')"}';
   result:=mNewQueryID;
end;
                        ...
                        IF mID='' THEN BEGIN
                            //NxShowSimpleMessage('Nový záznam se stejným ID',nil);
                             ...
                             // mstring:=                      inputbox('BO - plné data','Put',mtarget+mApiTable+'/' + mid + '       ' + mQuery)    ;
                              ...
                              //mboolean:=InputQuery('AA','BB','PUT '+mtarget + mApiTable+'/' + mid  + '              ' + mQuery)  ;
                              if (copy(mString,1,3)='201') or (copy(mString,1,3)='200') or (copy(mString,1,3)='204')then begin
                                    ...
                                    try
                                          for mICount:=0 to mr.count-1 do begin
                                               ...
                                               mbo.load(mr.Strings[mICount],nil);
                                                  mid:=GetOrCreateAPI(mbo,TBusRollSiteForm(mSite));
                                          end;
                                    finally
                                       mbo.free;
                                    end;
