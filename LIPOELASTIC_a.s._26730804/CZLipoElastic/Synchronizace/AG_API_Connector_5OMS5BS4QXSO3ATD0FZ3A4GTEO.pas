 ...
 result:='';
  mTargetList:=tstringlist.create;
    ...
    for i:=0 to mTargetList.count-1 do begin // ****cyklus pro jednotlicá spojení
                mTarget:=trim(copy(mTargetList.strings[i],2,100));
                itarget:=strtoint(trim(copy(mTargetList.strings[i],1,1)));
          ...
          if mTarget<>msource then begin
                     ...
                     IF mManual then BEGIN                   // **** ruční vykopírování údajů
                            mQuery:=GetQueryDefroll(mBO,itarget,mQuery);
...
//                                    end;
                        ...
                        IF mid='' THEN BEGIN
                             ...
                             //   mstring:=  InputBox('API', 'POST', mtarget+'/script/Synchronizace/API/NewValueWithID' + chr(10) + chr(10) + mNewQueryID);
                             ...
                             // mstring:=                      inputbox('BO - plné data','Put',mtarget+mApiTable+'/' + mid + '       ' + mQuery)    ;
                              ...
                              // mstring:=  InputBox('API', 'PUT', mtarget+ '/' +mApiTable + '/' + mid + chr(10) + chr(10) + mQuery);
...
//  if Sender is TAction then mSite := NxFindSiteForm(Sender);
    ...
    if not Assigned(mSite) then begin
     ...
     if mINovych+mIKuprave+mIUpravenych>0 then begin
         NxShowSimpleMessage('Celkově záznamů: ' + inttostr(mIUpravenych) + chr(10)
                             +'z toho nových záznamů: ' + inttostr(mINovych)
...
begin
  ...
  try
    ...
    if copy(muser.GetFieldValueAsString('X_Button_parametr'),8,1)='1' then begin    // hromadná změna stavu
              ...
              mMAction.Items.Add('Všech záznamů ');
...
procedure _DeleteObject_PostHook(xsite: TBusRollSiteForm; AObject: TNxCustomBusinessObject);
var
  aString:string;
  mstring:string;
  ARequest:string;
...
begin
   ...
   mTargetList:=tstringlist.create;
    TRY
     xsite.BaseObjectSpace.SQLSelect('Select a.code||A.X_CLSID COLLATE Czech_CS_AS FROM DefRollData A WHERE (A.Hidden = ' + quotedstr('N') + ' ) AND (A.CLSID = '+ quotedstr('C2HCXLEGT5H4340LIKNWZLXHI0') +
                                    ' ) AND (UPPER(A.X_Parent_ID) =' + quotedstr('ABRAAPI') + ')',mTargetList);
    if mTargetList.count=0 then begin
          NxShowSimpleMessage('Není možné vymazat',nil);
          exit;
    end;
          ...
          for i:=0 to mTargetList.count-1 do begin // ****cyklus pro jednotlicá spojení
                    ...
                    mQueryID:='{'
                              + ' "class": "' + mApiTable +'",'
                              +' "select": ["ID",],'
                              + ' "where": " id = ' + QuotedStr(AObject.OID)
                              +' " '
                              +'}';
                              mString:= APICallRest(AObject,'Post',mtarget+'/','query','',mQueryID,true);
                          ...
                          if mid<>'' then begin
                                   mString:= APICallRest(AObject,'DELETE',mtarget+'/',mApiTable,'/' + mid ,'{}',true);
