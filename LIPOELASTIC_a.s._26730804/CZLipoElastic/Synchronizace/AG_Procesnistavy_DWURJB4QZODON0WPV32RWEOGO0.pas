...
begin
    mQuery:='{}';
                  ...
                  mquery:=mquery + ', "clsid":"' +  Self.GetFieldValueAsString('clsid') +'" ';
                  mquery:=mquery + ', "Hidden": '  +  BoolToStr(Self.GetFieldValueAsBoolean('Hidden')) +' ' ;
                  mquery:=mquery + ', "countstatus": '  +  BoolToStr(Self.GetFieldValueAsBoolean('countstatus')) +' ' ;

                  mquery:=mquery + ', "color":' +  Self.GetFieldValueAsString('color') +' ';
                  mquery:=mquery + ', "sequencenumber":' +  Self.GetFieldValueAsString('sequencenumber') +' ';
                  mquery:=mquery + ', "systemstate":' +  Self.GetFieldValueAsString('systemstate') +' ';
                  mquery:=mquery + ', "x_font_color":' +  Self.GetFieldValueAsString('x_font_color') +' ';
                  mquery:=mquery + ', "x_fontstyle":"' +  Self.GetFieldValueAsString('x_fontstyle') +'" ';
...
begin
     mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (Code,clsid,ID,Hidden,countstatus,color,sequencenumber,systemstate) VALUES (' +
                                            quotedstr(Self.GetFieldValueAsString('Code'))
                                            + ','+ quotedstr(Self.GetFieldValueAsString('clsid'))
                                            + ','+ quotedstr(Self.OID)
                                            + ','+ quotedstr('N')
                                            + ','+ quotedstr('N')
                                            + ','+ quotedstr(Self.GetFieldValueAsString('color'))
                                            + ','+ quotedstr(Self.GetFieldValueAsString('sequencenumber'))
                                            + ','+ quotedstr(Self.GetFieldValueAsString('systemstate'))
                                            + ')"}';
