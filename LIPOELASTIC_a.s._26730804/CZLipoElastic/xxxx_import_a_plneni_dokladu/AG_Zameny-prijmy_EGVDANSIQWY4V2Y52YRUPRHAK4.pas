...
var
  ...
  mProvide_ID:string;
begin
    mCLSBO:='JFQYSEOTKPC4RAMLQVLUK5NV34';
    magenda:='EGVDANSIQWY4V2Y52YRUPRHAK4';
    mShowProgres:=true;
                      ...
                      mHead.SetFieldValueAsString('Firm_ID', mID_odberatel);
                                   ...
                                   mquantity:=NxIBStrToFloat(copy(mWorkList.Strings[0],21,10));
                                    ...
                                    mRow.SetFieldValueAsString('Division_ID',mID_Division);
                                        ...
                                        // ****** dohledaní provide_ID a provide row_ID
                                              mr:=tstringlist.create;
                                              try
                                                   mSite.BaseObjectSpace.SQLSelect('select id from storedocuments2 where parent_ID=' + quotedstr(mProvide_ID) + ' and storecard_ID=' + quotedstr(mrow.GetFieldValueAsString('Storecard_ID')),mr);
                                                   if mr.count>0 then begin
                                                        mRow.SetFieldValueAsString('Provide_ID',mProvide_ID);
                                                        mRow.SetFieldValueAsString('ProvideRow_ID',mr.Strings[0]);
                                                      //  if (i>5) and (i<10) then NxShowSimpleMessage(mr.Strings[0],nil);
                                                   end;
                                              finally
                                                  mr.free;
                                              end;
