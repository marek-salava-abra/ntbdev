     ...
     mHead.New;
                                   ...
                                   mXMLHead.loadFromFile(AFileName);
                                       ...
                                       for i := 0 to mXMLHead.getElementsCountInArray('Doc.Row') - 1 do begin
                                          ...
                                          for ii := 0 to mXMLHead.getElementsCountInArray('Doc.Row['+inttostr(i)+'].batch') - 1 do begin
                                                  ...
                                                  //NxShowSimpleMessage('novy radek  ' + mbatch_ID + ' - ' + mStoreCard_ID,nil);
                                                 mRow := mHead.Rows.AddNewObject;
                                                 mRow.Prefill;
                                                 //mRow.SetFieldValueAsInteger('PosIndex',ii);
                                                    mRow.SetFieldValueAsInteger('RowType',3);
                                                      mRow.SetFieldValueAsString('Store_ID',mstore_id);
                                                     if mStoreCard_ID<>'' then mRow.SetFieldValueAsString('Storecard_ID',mStoreCard_ID) else mRow.SetFieldValueAsString('Storecard_ID','3NQ1000101');
              ...
              if Not mhead.Validate() then begin
                    ...
                    try
                     ...
                     TDynSiteForm(msite).ShowDynFormWithNewDocument('B50I5SAOS3DL3ACU03KIU0CLP4', mSite.SiteContext, mHead);
              end else begin
                  mHead.save ;
...
if not Assigned(Self.BaseObjectSpace) then
 ...
 // if copy(mUser.GetFieldValueAsstring('X_Parametr'),1,1)='1' then begin
          ...
          mMAction.Caption := 'DL - importem';
          mMAction.Items.Add('DL - importem');
          //mMAction.Items.Add('Jen příjemka a fv 2 řádkem');
          //mMAction.Items.Add('Jen vratky');
