...
var
  ...
  procedure _xCanDelete_Hook(Self: TDynSiteForm; var ACanDelete: Boolean);
begin
 if not(osNew in self.CurrentObject.State) then begin
   if (trim(self.CurrentObject.GetFieldValueAsString('X_ExternalDocument'))<>'') and (self.CurrentObject.GetFieldValueAsDateTime('X_Synchronizace$Date')=0) then begin
           ACanDelete:=false;
