...
begin
 // NxShowSimpleMessage(AObject.OID,nil);
end;
...
var
  ...
  mbatch_number:double;
  mxx:TStringList;
  mi:integer;
   IsExtBatchName:boolean;
  mstring:string;
begin
 mSite := NxFindSiteForm(Sender);
       ...
       if true then begin
//       if (copy(mbo.GetFieldValueAsString('Firm_ID.X_Stitek_parametr'),6,1)<>'1') or (NxGetActualUserID_1(mbo)='4PU1000101') then begin

          IsExtBatchName:=false;
          mstring:='';
          try
              mstring:= msite.BaseObjectSpace.SQLSelectFirstAsString('SELECT f.X_stitek_parametr from ReceivedOrdersToIssuedOrders ROTIO left join ReceivedOrders RO on RO.ID=ROTIO.SourceHeader_ID left join Firms F on f.ID=ro.Firm_id where ROTIO.TargetHeader_ID = ' + QuotedStr(mbo.oid));
               if copy(mstring,8,1)='1' then begin
                      IsExtBatchName:=True;
               end;
          finally
          end;
                          ...
                          for i := 0 to mMon.Count - 1 do begin
                            ...
                            if mMon.BusinessObject[i].getFieldValueAsInteger('Storecard_ID.category')=2 then begin
                                 ...
                                 if mSiciDavka=0 then mSiciDavka:=mQuantity_pomoc;
                                 ...
                                 try
                                      ...
                                      ' AND (A.X_parent_id =' + QuotedStr(mMon.BusinessObject[i].OID) + ') and (a.code=' + quotedstr(mbo.OID)+') AND (A.X_parent2_id =' + QuotedStr(mMon.BusinessObject[i].GetFieldValueAsString('Storecard_ID')) + ')' ,mr);
                                          ...
                                          if mMon.BusinessObject[i].getFieldValueAsstring('Storecard_ID.StoreCardCategory_ID.X_PerefixBatch')<>''
                                           ...
                                           try
                                               ...
                                               if mxx.count>0 then begin
                                                        ...
                                                        mbatch_number:=NxIBStrToFloat(mxx.Strings[0])+1;
                                               end else begin
                                                        mbatch_number:=1;
                                               end;
                                           finally
                                           ...
                                           for j:=0 to trunc((mQuantity_pomoc-mQuantityUsed)/mSiciDavka) do begin
                                               ...
                                               try
                                                     ...
                                                     mBO_Sarze.SetFieldValueAsString('Name',mBatch_name_pomoc + (RightStr('00000' + NxFloatToIBStr(mbatch_number + j),3)));
                                                     ...
                                                     mBO_Sarze.SetFieldValueAsString('X_Order',copy(mbo.GetFieldValueAsString('ExternalNumber'),1,30));  //    mBO.GetFieldValueAsString('ExternalNumber')
                                                     ...
                                                     mBO_Sarze.SetFieldValueAsDateTime('ProductionDate$DATE',Now);
                                                      if IsExtBatchName then begin
                                                           //NxShowSimpleMessage('Zapis pro amoenu',nil);
                                                           //NxShowSimpleMessage(mMon.BusinessObject[i].getFieldValueAsstring('X_ExternalSpecification'),nil);
                                                           if trim(mMon.BusinessObject[i].getFieldValueAsstring('X_ExternalSpecification'))<>'' then begin
                                                             //NxShowSimpleMessage('Sarže - ' + mMon.BusinessObject[i].getFieldValueAsstring('X_ExternalSpecification'),nil);
                                                              mBO_Sarze.SetFieldValueAsstring('X_External_Name',mMon.BusinessObject[i].getFieldValueAsstring('X_ExternalSpecification'));
                                                           end;
                                                       end;
          ...
          mbo.save;
          mbo.Refresh;
          NxShowSimpleMessage('Generování dokončeno.',nil);
      end else begin
          NxShowSimpleMessage('Pro uvedeného dodavatele není dovoleno generovat šarže. ' + chr(13) + ' Změnte nastavení v adresáři firem ve formuláři "Předvyplnění" , položka "Parametry" , hodnota "Zákaz generovat šarže"' , nil);
      end;
...
begin
  ...
  mMAction.Hint := 'Vyčištení pohybu';
