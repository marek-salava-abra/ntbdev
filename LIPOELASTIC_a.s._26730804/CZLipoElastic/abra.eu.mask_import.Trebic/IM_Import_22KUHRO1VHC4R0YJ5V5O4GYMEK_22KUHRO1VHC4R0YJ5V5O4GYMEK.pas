...
begin
     //if AInputRow.GetFieldValueAsInteger('Storecard_ID.Category') =2 then begin
             mr:=TStringList.create;
                  try
                       AInputRow.ObjectSpace.SQLSelect('SELECT A.ID FROM DefRollData A WHERE A.CLSID = ''SLARSB0H4CK4T32XPZTP33J3XS'' and A.X_Parent_ID ='+quotedstr(AInputRow.oid),mr);
                       if mr.count>0 then begin
                            mBO_pohyb_sarze:=AInputRow.ObjectSpace.CreateObject('SLARSB0H4CK4T32XPZTP33J3XS');
                            try
