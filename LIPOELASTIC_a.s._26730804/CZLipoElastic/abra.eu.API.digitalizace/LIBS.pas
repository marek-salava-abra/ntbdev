...
function GetVP_ID(xSite:TSiteForm;mtable:string;mID:string):string;
var
mr_local:tstringlist;
begin
              mr_local:=TStringList.create;
              try
                 xsite.BaseObjectSpace.SQLSelect(format('select a.id from %s A where A.ID=%s',
                 [mtable,mID]),mr_local) ;

                 //NxShowSimpleMessage(format('select a.id from %s A where A.ID=%s',
                 //[mtable,mID]),nil)  ;
                 if mr_local.count>0 then begin
                      result:= mr_local.Strings[0] ;
                      //NxShowSimpleMessage(mr_local.Strings[0],nil);
                 end else begin
                     result:='';
                 end;
              finally
