...
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'GCComponents'));
    ...
    Result.Caption := '&Náklady a výnosy';
    Result.Name := 'tabParamsGC';
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'GCComponents'));
    mGrd.Parent := mPnl;
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Kód oblasti';
    ...
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := 'Y5N0T3IQYG14FJE3I5HLMCOC0S';
    ...
    mGridColumn.Caption := 'Název oblasti';
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Layout := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Line := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).FieldName := 'CostAccount_ID';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Nákladový účet';
    ...
    if not mCanEdit then
      mAct.Tag := -1;
    mListBox.Items.AddObject('mactGCLSEdit', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactGCLSRefresh';
    mAct.Caption := 'Občerstvit';
    mAct.Category := Result.Name;
    mAct.ShortCut := TextToShortCut('F11');
    mAct.OnExecute := @actLSRefreshExecute;
    mListBox.Items.AddObject('mactGCLSRefresh', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactGCLSSave';
    mAct.Caption := 'Uložit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactGCLSSave', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactGCLSCancel';
    mAct.Caption := 'Zrušit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactGCLSCancel', mAct);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(mSite, 'GCComponents'));
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSEdit'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSRefresh'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSSave'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSCancel'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSSave'))).Enabled := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSCancel'))).Enabled := True;

  setParamsEnabled(TWinControl(mListBox.Items.Objects(mListBox.Items.IndexOf('tabParamsGC'))), True);
end;
...
begin
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSEdit'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSRefresh'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSSave'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSCancel'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSSave'))).Enabled := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactGCLSCancel'))).Enabled := False;

  setParamsEnabled(TWinControl(aListBox.Items.Objects(aListBox.Items.IndexOf('tabParamsGC'))), False);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'GCComponents'));
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            //GlobParams.GetOrCreateParam(dtString, 'ParStorecardType_ID').AsString := '';
             mStoreCardBO:=ASite.BaseObjectSpace.CreateObject('3ALUAMFUYQI411CNBR5XTGAUES');
             mStoreCardBO.load(mDataSet.FieldByName('Property_ID').AsString,nil);

            mBO.SetFieldValueAsString('X_Parent_ID', mOID);
            mBO.SetFieldValueAsString('X_AccRegion_ID', mDataSet.FieldByName('Property_ID').AsString);
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    ' PAR.X_AccRegion_ID as Property_ID, SC.Name as Property_Name,' +
    ' PAR.X_PosIndex as PosIndex, PAR.X_StoreAccount_ID as StoreAccount_ID, PAR.X_GainAccount_ID as GainAccount_ID, PAR.X_CostAccount_ID as CostAccount_ID' +
    ' from DefRollData PAR ' +
    ' LEFT JOIN Defrolldata SC ON SC.ID=PAR.X_AccRegion_ID '+
    ' where sc.clsid='+QuotedStr('3ALUAMFUYQI411CNBR5XTGAUES')+' and  PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) + 'and PAR.X_rel_def='+QuotedStr(cRelDef)+' and PAR.X_Parent_ID=%s order by PAR.X_PosIndex';
  ...
  if pgcDataViews.ActivePage <> TTabSheet(pgcDataViews.FindChildControl('tabParamsGC')) then Exit;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject('3ALUAMFUYQI411CNBR5XTGAUES');
          try
...
begin
  Result := True;
  mListBox := TListBox(NxFindChildControl(aSite, 'GCComponents'));
  if not (TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSEdit'))).Enabled) then begin
    ...
    try
      ...
      Result := TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactGCLSEdit'))).Enabled;
    end;
  end;
end;
