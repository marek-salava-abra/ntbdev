...
begin
    mQuery:='{'   ;
                  ...
                  mquery:=mquery + ', "AutoRelease": '  +  BoolToStr(Self.GetFieldValueAsBoolean('AutoRelease')) +' ' ;
                  mquery:=mquery + ', "LumpoverView": '  +  BoolToStr(Self.GetFieldValueAsBoolean('LumpoverView')) +' ' ;
                mquery:=mquery +'}';
                        ...
                        IF mid='' THEN BEGIN
                            //NxShowSimpleMessage('Nový záznam se stejným ID',nil);
                                 ...
                                 if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0)  and (index=1) then
                                                                mboolean:=InputQuery('API','POST ',mtarget+'/script/Synchronizace/API/NewValueWithID' + Chr(10) + chr(10) +mNewQuery);
