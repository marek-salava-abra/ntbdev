...
begin
 ...
 mJSON:=API_GET('http://api.abra-cz.prod.ad.lipoelastic.com:82/Lipoelastic/Pictures/' + self.OID);
  ...
  //mQuery :=parse
    ...
    mQuery:='{'   ;
                  ...
                  mquery:=mquery + ', "ExternalFile": '  +  BoolToStr(Self.GetFieldValueAsBoolean('ExternalFile')) +' ' ;
                  mquery:=mquery + ', "IsProtected": '  +  BoolToStr(Self.GetFieldValueAsBoolean('IsProtected')) +' ' ;


                  mquery:=mquery + ', "Class_ID":"' +  Self.GetFieldValueAsString('Class_ID') +'" ';
                  mquery:=mquery + ', "PathAndFileName":"' +  Self.GetFieldValueAsString('PathAndFileName') +'" ';
                //  mquery:=mquery + ', "PictureData":' +  NxFloatToIBStr(Self.GetFieldValueAsFloat('PictureData')) +' ';
                  mquery:=mquery + ', "PictureTitle":' +  NxFloatToIBStr(Self.GetFieldValueAsFloat('PictureTitle')) +' ';
...
begin
    mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (PictureTitle,ID,Class_ID,ExternalFile) VALUES (' +
                        ...
                        IF mid='' THEN BEGIN
                           ...
                           if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0)  and (index=1) then
                              ...
                              if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0) and (index=1) then
                                  ...
                                  mjson:=TJSONSuperObject.Create;
                                  Try
                                        mJSON:=API_GET('http://api.abra-cz.prod.ad.lipoelastic.com:82/LipoElastic/Pictures/' + mbo.OID);
                                        NxShowSimpleMessage(mJSON.S['DisplayName'],nil);
                                        mboolean:=InputQuery('API','PUT  - doplnění a aktualizace',msource+'/'+mApiTable+'/' + mid + Chr(10) + chr(10) +mQuery);
