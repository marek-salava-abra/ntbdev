...
const
mTable='CRPCompetences';
mApiTable='CRPCompetences';
...
begin
    mQuery:='{'   ;
                  ...
                  mquery:=mquery + ', "Category":"' +  Self.GetFieldValueAsString('Category') +'" ';
                  ...
                  mquery:=mquery + ', "IsMobile":"' +  BoolToStr(Self.GetFieldValueAsBoolean('IsMobile')) +'" ';
                  mquery:=mquery + ', "IsRange":"' +  BoolToStr(Self.GetFieldValueAsBoolean('IsRange')) +'" ';
                  mquery:=mquery + ', "IsStandard":"' +  BoolToStr(Self.GetFieldValueAsBoolean('IsStandard')) +'" ';
                  mquery:=mquery + ', "TBC":"' +  NxFloatToIBStr(Self.GetFieldValueAsFloat('TBC')) +'" ';
                  mquery:=mquery + ', "Validity":"' +  NxFloatToIBStr(Self.GetFieldValueAsFloat('Validity')) +'" ';
                mquery:=mquery +'}';
...
begin
    mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (Code,Name,ID) VALUES (' +
                        ...
                        IF mid='' THEN BEGIN
                            //NxShowSimpleMessage('Nový záznam se stejným ID',nil);
                                 ...
                                 if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0)  and (index=1) then
                                                                mboolean:=InputQuery('API','POST - založení ',mtarget+'/script/Synchronizace/API/NewValueWithID' + Chr(10) + chr(10) +mNewQuery);
                             ...
                             // mstring:=                      inputbox('BO - plné data','Put',mtarget+mApiTable+'/' + mid + '       ' + mQuery)    ;
                               ...
                               if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0) and (index=1) then
                                                                mboolean:=InputQuery('API','PUT - aktualizace a doplnění',mtarget+'/'+mApiTable+'/' + mid + Chr(10) + chr(10) +mQuery);
