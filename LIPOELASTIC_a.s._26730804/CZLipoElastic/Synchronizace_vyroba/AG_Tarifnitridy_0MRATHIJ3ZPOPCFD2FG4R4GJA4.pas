...
const
mTable='PLMSalaryClasses';
mApiTable='PLMSalaryClasses';
...
begin
    mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (Code,HourlyRate,ID) VALUES (' +
                                            quotedstr(Self.GetFieldValueAsString('Code'))
                                            + ','+ quotedstr(NxFloatToIBStr(Self.GetFieldValueAsFloat('HourlyRate')))
                                            + ','+ quotedstr(Self.OID)
                                            + ')"}';
         result:=mNewQueryID;
end;
