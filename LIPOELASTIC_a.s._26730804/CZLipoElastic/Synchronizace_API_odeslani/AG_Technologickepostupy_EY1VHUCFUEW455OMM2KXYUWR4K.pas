                      ...
                      mr.free;
                                              ...
                                              for mICount:=0 to mIBookmark do begin
                                                  ...
                                                  if (not mKumulovane) or ((mKumulovane) and (mICount=0)) then begin
                                                           mquery:=NxGetAPIHeadJSON(msite.BaseObjectSpace,self,'Routine',mUser);
                                                  end;

                                                  mquery:=mquery + NxGetAPIRoutine(msite,self);
                                                   ...
                                                   if not mKumulovane then begin      // index=1 then begin
                                                    ...
                                                    if mOnline then begin
                                                          if (mSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0) then
                                                                       mboolean:=InputQuery('API','Post 1 doklad',mtarget+'/script/NxApiLib/lib/APINxPLMRoutine' + Chr(10) + chr(10) +mQuery);

                                                           mString:=APICallString(msite.BaseObjectSpace,'POST',mtarget+'/script/NxApiLib/lib/APINxPLMRoutine',mQuery, true);
                                                                   // mReturnNewDocNumber:='';
                                                                   // mReturnSourceDoc:='';
                                                                    mReturnJSON:=TJSONSuperObject.create;
                                                                       ...
                                                                       mReturnJSON:= TJSONSuperObject.ParseString(mString,true);
                                                                        ...
                                                                        mReturnJSON.free;
                                                                              ...
                                                                              mTarget:=mExportDir + trim(copy(mTargetAPI,20,50));
    ...
    if copy(muser.GetFieldValueAsString('X_Button_parametr'),9,1)='1' then begin    // hromadná změna stavu
            ...
            {    mMAction := Self.GetNewMultiAction;
               ...
               { mMAction := Self.GetNewMultiAction;
                ...
                mMAction.ShowControl := True;
                mMAction.ShowMenuItem := True;
                mMAction.Caption := 'Synchronizace Export OFFline';
                mMAction.Hint := 'Odeslání položky 1:1 ';
                mMAction.Category := 'tabList';
                mMAction.Items.Add('Synchronizace Offline ');
                ...
                mMAction.ShowControl := True;
                mMAction.ShowMenuItem := True;
                mMAction.Caption := 'Synchronizace Export ONline';
                mMAction.Hint := 'Odeslání položky 1:1 ';
                mMAction.Category := 'tabList';
                mMAction.Items.Add('Synchronizace Online ');
                mMAction.OnExecuteItem := @PoDokladu_Online;

          mMAction := Self.GetNewMultiAction;
                mMAction.ShowControl := True;
                mMAction.ShowMenuItem := True;
                mMAction.Hint := 'Synchronizace Import OFFline ';
                                ...
                                for i:=0 to mFileList.count-1 do begin
                                     ...
                                     if not FileExists(mdir + mFileList.Strings[i]+'.json') then begin
                                          ...
                                          try
                                              ...
                                              mString:=APICallString(msite.BaseObjectSpace,'POST',mSourceAPI + '/script/NxApiLib/lib/APINxPLMRoutine',mQuery, true);
