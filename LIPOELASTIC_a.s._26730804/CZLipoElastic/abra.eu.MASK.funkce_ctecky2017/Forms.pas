uses 'abra.eu.MASK.funkce_ctecky2017.Libs';
const
...
mSQL_office='select max(ro2.id),ro2.Storecard_ID,max(l.code) from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join StoreSubCards SSC on ((SSC.StoreCard_ID=sc.id) and (SSC.Store_ID=%S)) left join Locations L on l.id=ssc.Location_ID left join Receivedorders2 RO2 on ro2.storecard_id=sc.id left join Receivedorders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and  ((sc.Storecardcategory_ID<>' + QuotedStr('~000000002') + ') and (sc.Storecardcategory_ID<>' + QuotedStr('~00000000G') + '))  and (ro2.X_specifikace_id is null) and  ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>0) and ((RO.FirmOffice_ID in (%s))) group by l.code,ro2.Storecard_ID order by l.code,ro2.Storecard_ID';
mSQL_Doklad='select max(ro2.id),ro2.Storecard_ID,l.code from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join StoreSubCards SSC on ((SSC.StoreCard_ID=sc.id) and (SSC.Store_ID=%S)) left join Locations L on l.id=ssc.Location_ID left join Receivedorders2 RO2 on ro2.storecard_id=sc.id left join Receivedorders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>0) and ((sc.Storecardcategory_ID<>' + QuotedStr('~00000000G') + ') and (sc.Storecardcategory_ID<>' + QuotedStr('~000000002') + '))  and  ((RO.ID in (%s))) group by l.code,ro2.Storecard_ID order by l.code,ro2.Storecard_ID';
...
begin
      ...
      mOldBarCode:='';
                      ...
                      mr1:=TStringList.create;
                        ...
                        mcelkem:=GetFloatFromTable(xSite,'Select sum(RO2.quantity) from Receivedorders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
                        mdodano:=GetFloatFromTable(xSite,'Select sum(RO2.DeliveredQuantity) from Receivedorders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
                        mvychystano:=GetFloatFromTable(xSite,'Select sum(X_Vychystano) from Receivedorders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
