...
{     if (NxGetActualUserID(Self.BaseObjectSpace)='AUTO000000') or //"Automat"	"Automat"	"Ne"	"Auto"	""
        ...
        if Assigned(mDBGrid) then begin
          _RemoveColumn(mDBGrid ,'colUnitPrice');
          _RemoveColumn(mDBGrid ,'colUnitPrice1');
          _RemoveColumn(mDBGrid ,'colUnitPrice2');
          _RemoveColumn(mDBGrid ,'colUnitPrice3');
          _RemoveColumn(mDBGrid ,'colTotalPrice');
          _RemoveColumn(mDBGrid ,'colTotalPrice1');
          _RemoveColumn(mDBGrid ,'colTotalPrice2');
          _RemoveColumn(mDBGrid ,'colTotalPrice3');
        end;
      end;
    finally
      mPar.Free;
...
Procedure _RemoveColumn(AGrid: TMultiGrid ; AColumn: string);
begin
  if Assigned(AGrid.ColumnByName(AColumn)) then begin
    AGrid.RemoveColumn(AGrid.ColumnByName(AColumn));
  end;
end;
