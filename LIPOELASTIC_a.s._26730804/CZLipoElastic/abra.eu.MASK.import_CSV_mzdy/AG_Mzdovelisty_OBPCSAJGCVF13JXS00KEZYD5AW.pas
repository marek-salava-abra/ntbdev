...
//    mCustomBusinessObject:= TBusRollSiteForm(mSite).CurrentObject;
    ...
    if index=0 then Import_stravenek(msite, TDynSiteForm(mSite).CurrentObject.ObjectSpace, mfilename, mdir,mfile);
    if index=1 then Import_dosys(msite, TDynSiteForm(mSite).CurrentObject.ObjectSpace, mfilename, mdir,mfile);
end;
            ...
            if mImportFile.Count >1 then begin
                 ...
                 while i < mImportFile.Count do begin
                    ...
                    try
                                mfieldValue:= TStringList.Create;
                                //mTargetFile.strings[i]:=mImportFile.strings[i];
                                Parsevalue(mImportFile.strings[i],';',mImportFile.strings[i],mfieldValue,11);
                                                                ...
                                                                // odpracovaný čas
                                                                  if mfieldValue.Strings[4] <> '00:00' then begin
                                                                      mFcas:=0;
                                                                          ...
                                                                          mFcas:=nxstrtoint(copy(mScas, 1, pos(':', mScas) - 1)) + (nxstrtoint(copy(mScas, (pos(':', mScas) + 1),2 )) / 60);
                                                                      mBO_mzdove_listy.setfieldvalueasfloat('S_WorkHours',mFcas);
                                                                  end;

                                                                  // odpracovaný LE lékař
                                                                  if mfieldValue.Strings[5] <> '00:00' then begin
                                                                      mFcas:=0;
                                                                          mScas:= mfieldValue.Strings[5] ;
                                                                           mFcas:=nxstrtoint(copy(mScas, 1, pos(':', mScas) - 1)) + (nxstrtoint(copy(mScas, (pos(':', mScas) + 1),2 )) / 60);
                                                                            ...
                                                                            mFcas:=mFcas + nxstrtoint(copy(mScas, 1, pos(':', mScas) - 1)) + (nxstrtoint(copy(mScas, (pos(':', mScas) + 1),2 )) / 60);
                                                                      end;
                                                                           mBO_mzdove_listy.setfieldvalueasfloat('S_PaidFree',mFcas);
                                                                  end;

                                                                   // odpracovaný SL  služební cesta
                                                                  if mfieldValue.Strings[6] <> '00:00' then begin
                                                                      mFcas:=0;
                                                                  ...
                                                                  if mfieldValue.Strings[7] <> '00:00' then begin
                                                                      mFcas:=0;
                                                                          mScas:= mfieldValue.Strings[7] ;
                                                                       mFcas:=nxstrtoint(copy(mScas, 1, pos(':', mScas) - 1)) + (nxstrtoint(copy(mScas, (pos(':', mScas) + 1),2 )) / 60);
                                                                              mBO_mzdove_listy.setfieldvalueasfloat('S_Holiday',(mFcas/mBO_mzdove_listy.getfieldvalueasfloat('S_AvgShiftLength')));
                                                                  ...
                                                                  if mfieldValue.Strings[9] <> '00:00' then begin
                                                                    ...
                                                                    // odpracovaný STravne
                                                                  if mfieldValue.Strings[10] <> '0' then begin
