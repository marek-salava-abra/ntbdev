...
begin
     mQuery:='{}';
                  ...
                  //mquery:=mquery + '"id": "' +  Self.OID +'"'  ;
                                    ...
                                    mquery:=mquery + ' "BlobData":"'  +  Self.GetFieldValueAsString('BlobData') +'" ' ;

                                    mquery:=mquery + ', "Class_ID":"'  +  inttostr(Self.GetFieldValueAsInteger('Class_ID')) +'" ' ;
                                    mquery:=mquery + ', "ClassID":"'  +  inttostr(Self.GetFieldValueAsInteger('ClassID')) +'" ' ;

                                    mquery:=mquery + ', "ExternalFile":"'  +  (Self.GetFieldValueAsString('ExternalFile')) +'" ' ;
                                    mquery:=mquery + ', "IsProtected":'  +  booltostr(Self.GetFieldValueAsBoolean('IsProtected')) +' ' ;
                                    mquery:=mquery + ', "PathAndFileName":"'  +  (Self.GetFieldValueAsString('PathAndFileName')) +'" ' ;
                                    mquery:=mquery + ', "PictureData":"'  +  (Self.GetFieldValueAsString('PictureData')) +'" ' ;
                                    mquery:=mquery + ', "PictureTitle":"'  +  (Self.GetFieldValueAsString('PictureTitle')) +'" ' ;
//                                    mquery:=mquery + ', "PictureType ":'  +  (Self.GetFieldValueAsString('PictureType ')) +' ' ;
...
begin
     mNewQueryID:='{"info_type": "New_value" '
                                     +','+' "mSQL": "INSERT INTO ' + mtable + ' (PictureTitle,classid,ID,class_id,externalfile,isprotected,pathandfilename) VALUES (' +
                                            quotedstr(Self.GetFieldValueAsString('PictureTitle'))
                                            + ','+ quotedstr(Self.GetFieldValueAsString('classid'))
                                            + ','+ quotedstr(Self.OID)
                                            + ','+ quotedstr(Self.GetFieldValueAsString('class_id'))
                                            + ','+ quotedstr('A')
                                            + ','+ quotedstr('A')
                                            + ','+ quotedstr(Self.GetFieldValueAsString('pathandfilename'))
                                           + ')"}';
   result:=mNewQueryID;
end;
                        ...
                        IF mID='' THEN BEGIN
                            //NxShowSimpleMessage('Nový záznam se stejným ID',nil);
                                // mNewQuery:=GetNewQuery(mBO,i);
                                 ...
                                 mNewQuery:=mNewQuery + ' {"blobdata":"",   ';
                                 mNewQuery:=mNewQuery + '"class_id":"KCBDEFEPAYUOZ3D0GK3URSGKIO","class_name":"Obrázek technologických postupů","externalfile":false,"id":"~000000C1S","isprotected":false,"objversion":1,"pathandfilename":"",';
                                 mNewQuery:=mNewQuery + '"picturedata":"\",';
                                 mNewQuery:=mNewQuery + '"picturetitle":"' + mbo.GetFieldValueAsString('PictureTitle')+ '","refcount":0}';
                                 //mString:=ApiCallNewValue(mBO,'POST',mtarget+'script/Synchronizace/API/NewValueWithID',mNewQuery, true);

                                 mString:=APICallRest(mBO,'Post',mtarget,'Pictures','',mNewQuery,true);
                                 //mString:=ApiCallNewValue(mBO,'POST',mtarget+'script/Synchronizace/API/NewValueWithID',mNewQuery, true);

                                   if (xSite.CompanyCache.GetUserID='SUPER00000') and (mICount=0) then
                                                                mboolean:=InputQuery('API','post doklad',mtarget+'Pictures/' + '     ' + mNewQuery);
