    ...
    try
        ...
        mImportFile.LoadFromFile(AFileName);
         ProgressInit(msite, 'Načtení souboru ' + AFileName, 100);
                 ...
                 for i:=5 to mImportFile.Count-1 do begin   // načtení souboru
                          ProgressSetPos(1+NxFloor((i/mImportFile.Count)*99), inttostr(i) +' z '+inttostr(mImportFile.Count));
                                  ...
                                  try
                                         ...
                                         Parsevalue(mstringline,',',mstringline,mfieldValue,10);
                                          ...
                                          mr:=tstringlist;
                                          try
                                                os.SQLSelect('select from StoreBatches where name=' + quotedstr(mfieldValue.Strings[4]),mr) ;
                                             ...
                                             if mid='' then begin
                                                     ...
                                                     mbonew:= msite.BaseObjectSpace.CreateObject('C32QXZWCTVDL342W01C0CX3FCC');
                                                     try
                                                     mbonew.new;
                                                     mbonew.Prefill;
                                                     mbonew.SetFieldValueAsString('Name',mfieldValue.Strings[4]);
                                                     mbonew.SetFieldValueAsString('StoreCard_ID',mfieldValue.Strings[2]);
                                                     mbonew.SetFieldValueAsDateTime('ExpirationDate$DATE', NxIBStrToFloat(mfieldValue.Strings[3]));
                                                     mbonew.SetFieldValueAsString('Note',mfieldValue.Strings[7]);
                                                     mbonew.SetFieldValueAsDateTime('ProductionDate$DATE',NxIBStrToFloat(mfieldValue.Strings[9]));
                                                     mbonew.SetFieldValueAsString('Specification',mfieldValue.Strings[8]);
                                                     mbonew.save;
                                                    finally
                                                         mbonew.free;
                                                     end;
...
//  if copy(mUser.GetFieldValueAsstring('X_Parametr'),1,1)='1' then begin
          ...
          mMAction.Caption := 'Vytvoření šarží ze souboru';
          mMAction.Items.Add('Scipter');
          mMAction.Items.Add('TXT');
          mMAction.Items.Add('XML');
          mMAction.Items.Add('XML_B2B_Doc');
