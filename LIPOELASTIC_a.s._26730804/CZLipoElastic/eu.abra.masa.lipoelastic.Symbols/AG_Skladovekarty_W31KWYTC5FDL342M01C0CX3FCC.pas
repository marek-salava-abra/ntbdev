...
begin
  try
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Obrázek';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Order := 3;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Width := 600;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'PictureTitle';
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := Roll_Pictures;
    ...
    mGridColumn.Caption := 'Název úhrady';
      ...
      while not mDataSet.Eof do begin
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            mBO.SetFieldValueAsString('X_Picture_ID', mDataSet.FieldByName('Property_ID').AsString);
            ...
            //mBo.SetFieldValueAsFloat('X_FloatValue1',mDataSet.FieldByName('Quantity').AsFloat);
            mStoreCardBO.free;
            mBO.Save;
          finally
            mBO.Free;
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    ' PAR.X_Picture_ID as Property_ID, ' +
  ...
  try
    ...
    if mResult.Active then begin
      ...
      while not mResult.eof do begin
        ...
        //mDataSet.FieldByName('Quantity').AsFloat:=mResult.FieldByName('Quantity').AsFloat;
        mCanLookup := true;
        mDataSet.Post;
        mResult.Next;
      end;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject('535FINKNYNDL3C5P00CA141B44');
          try
