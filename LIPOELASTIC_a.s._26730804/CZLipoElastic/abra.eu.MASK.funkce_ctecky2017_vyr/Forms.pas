uses 'abra.eu.MASK.funkce_ctecky2017_vyr.Libs';
const
...
mSQL_office='select max(ro2.id),ro2.Storecard_ID,max(l.code) from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join StoreSubCards SSC on ((SSC.StoreCard_ID=sc.id) and (SSC.Store_ID=%S)) left join Locations L on l.id=ssc.Location_ID left join IssuedOrders2 RO2 on ro2.storecard_id=sc.id left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and  ((sc.Storecardcategory_ID<>' + QuotedStr('1A00000101') + ') and (sc.Storecardcategory_ID<>' + QuotedStr('9000000101') + '))  and (ro2.X_specifikace_id is null) and  ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>0) and ((RO.FirmOffice_ID in (%s))) group by l.code,ro2.Storecard_ID order by l.code,ro2.Storecard_ID';
mSQL_Doklad='select max(ro2.id),ro2.Storecard_ID,l.code from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join StoreSubCards SSC on ((SSC.StoreCard_ID=sc.id) and (SSC.Store_ID=%S)) left join Locations L on l.id=ssc.Location_ID left join IssuedOrders2 RO2 on ro2.storecard_id=sc.id left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>0) and ((sc.Storecardcategory_ID<>' + QuotedStr('1A00000101') + ') and (sc.Storecardcategory_ID<>' + QuotedStr('9000000101') + '))  and  ((RO.ID in (%s))) group by l.code,ro2.Storecard_ID order by l.code,ro2.Storecard_ID';
...
mSQL_Doklad_quantitysp='select sum(ro2.quantity) from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join IssuedOrders2 RO2 on ro2.storecard_id=sc.id left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>=0) and (se.EAN=%s and (RO.ID in (%s)))';
...
mSQL_Doklad_zapis='select ro2.id from IssuedOrders2 RO2 left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_vychystano)>0) and (ro2.storecard_id=%s and (RO.ID in (%s)))';
mSQL_Doklad_zapisDL='select ro2.id from IssuedOrders2 RO2 left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_dodano)>0) and (ro2.storecard_id=%s and (RO.ID in (%s)))';
...
begin
      ...
      mStorecard_id:='';
                     ...
                     mjednotka:='';
                      mbarcode:='';
      mBO:=xsite.BaseObjectSpace.CreateObject('CDMK5QAWZZDL342X01C0CX3FCC');
      try
      ...
      //                                 mBtn := TButton.Create(mForm);mBtn.Width := 75;mBtn.Height := 40;mBtn.Caption := 'Storno';mBtn.ModalResult := mrCancel;mBtn.Cancel := True;mBtn.Left := mForm.Width - (mBtn.Width+2) - 20;mBtn.Top := mForm.Height - mBtn.Height - 60;mBtn.Name := 'btnCancel';mForm.InsertControl(mBtn);
                                       ...
                                       if mix_result=20 then begin
                                        ...
                                        if (mix_result = 22) then begin
                                           //if mCLSID_DOC='CDMK5QAWZZDL342X01C0CX3FCC' then begin
                                                              // postupný zápis
                                                              mi_SQL:=xsite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set x_dodano=' + NxFloatToIBStr(mpocet + mbo.GetFieldValueAsFloat('X_dodano')) + ' where id=' + QuotedStr(mBO_Row_idx));
                                                              nxbeep(btSuccess);
                                                              mBO_Row_idx:='';
                                                              mpocet:=0;
                                                        // end;
...
begin
 ...
 //                                CreateMemo(AName, ACaption: string;
 // ALeft, ATop, AWidth, AHeight: Integer;
 // ALblWidth: Integer;
 // ADefaultValue: string; AParent: TWinControl;
 // AEditToNewLine: Boolean = False): TMemo;
                                 ...
                                 if mi_resulta<>10 then begin
                                      abarcode:=mBarCodeEdt.text;
                                        ...
                                        if mi_resulta=1 then begin
                                            if mBarCodeEdt.text<>'' then begin
                                                mID_doklad:=GetDocument_ID(xSite,'IssuedOrders',abarcode);
                                ...
                                if (mix<>10) and (mBarCodeEdtx.Text<>'') then begin
                                    ...
                                    if mBarCodeEdtx.Text=copy(mbuton9,24,Length(mBarCodeEdtx.Text)) then result:=9;
                                     //NxShowSimpleMessage(inttostr(result),nil);
                                       if result=0 then mID_doklad:=GetDocument_ID(xSite,'IssuedOrders',mBarCodeEdtx.Text);
                                end else begin
                                    result:=mix;
                                end;
                    finally
...
function BarCodeDialog(xSite:TSiteForm;mCLSID_DOC:string;mTBatches:boolean;
                       mbo_ReceivedOrder_row:TNxCustomBusinessObject;
                       mTop:integer;mLeft:integer;mWith:integer;mHeight:integer;mLabel:string;
                       mTyp_prace:integer;mids:integer; mID_Filtr:string;
                       mbutton1,mbutton2,mbutton3,mbutton4:string;
...
begin
      ...
      mOldBarCode:='';
                      ...
                      mr1:=TStringList.create;
                        Try
                           xsite.BaseObjectSpace.SQLSelect('select id from StoreSubCards where StoreCard_ID =' + quotedstr(mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID')) + ' and Store_id=' + quotedstr(mbo_ReceivedOrder_row.GetFieldValueAsString('Store_ID')),mr1);
                        ...
                        mcelkem:=GetFloatFromTable(xSite,'Select sum(RO2.quantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
                        mdodano:=GetFloatFromTable(xSite,'Select sum(RO2.DeliveredQuantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
                        mvychystano:=GetFloatFromTable(xSite,'Select sum(X_Vychystano) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and ((ro2.X_specifikace_id is null) and (ro2.X_ExternalSpecification ='''')) and (RO2.Parent_ID in (%s))',mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_ID'),mID_Filtr);
       ...
       if mpokracovat then begin
             try
                 ...
                 if True then mForm.Color := clBtnFace else mForm.Color:= clRed ;
                                        ...
                                        mMemNote := CreateMemo('ChMemNote','Zboží', 10, 80, 320,80, 150, mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_id.Name'), mForm,true,true,False,round(480/36), [fsItalic],255);


                                        mLocEdt:=CreateEdit('mLocEdt', 'Umístění',mform, 10,200,320,80,150,m_umisteni,true,true,false,round(480/24), [fsBold],255) ;
                                        mBtn := TButton.Create(mForm);mBtn.Width := 65;mBtn.Height := 70;mBtn.Caption := '-';mBtn.ModalResult := mrNo;mBtn.Cancel := False;mBtn.Default := false;mBtn.Left:=10;mBtn.Top := 270;mBtn.Name := 'btnIgnore';mForm.InsertControl(mBtn);
                                        mQuantityEdt:=CreateEdit('mQuantityEdt', 'Množství',mForm,80, 270, 120,80,50,NxFloatToIBStr(mpocet_zapis),true,true,true,round(480/24),[fsBold],255);
                                        mBtn := TButton.Create(mForm);mBtn.Width := 65;mBtn.Height := 70;mBtn.Caption := '+';mBtn.ModalResult := mrYes;mBtn.Cancel := False;mBtn.Default := false;mBtn.Left:=260;mBtn.Top := 270;mBtn.Name := 'btnYea';mForm.InsertControl(mBtn);
                                        mUnitEdt:=CreateEdit('mUnitEdt', 'Jedn.',mForm,210, 270, 40,80,50,mbo_ReceivedOrder_row.GetFieldValueAsString('qunit'),true,true,false,round(480/48),[fsBold],255);
                                        mQuanEdt:=CreateEdit_noformat('mQuanEdt', 'Celkem',mform, 10,350,70,80,150,NxFloatToIBStr(mcelkem),true,true,false,round(120/10),[fsBold] ,255) ;
                                        mDelQuanEdt:=CreateEdit_noformat('mDelQuanEdt', 'Dodano',mform, 90,350,70,80,150,NxFloatToIBStr(mdodano),true,true,false,round(120/10),[fsBold],255) ;
                                        mStorQuanEdt:=CreateEdit_noformat('mStorQuanEdt', 'Skladem',mform, 170,350,70,80,150,NxFloatToIBStr(mskladem),true,true,false,round(120/10),[fsBold],255) ;
                                        mVychQuanEdt:=CreateEdit_noformat('mVychQuanEdt', 'Vychystano',mform, 270,350,70,80,150,NxFloatToIBStr(mvychystano+mpocet_zapis),true,true,false,round(120/10),[fsBold],255) ;
                                       ...
                                       if mix_result=99 then begin
                                              mll:=TStringList.create;
                                                                try
                                                                    //
                                                                    xsite.BaseObjectSpace.SQLSelect(format('select sb.name||ssb.ExpirationDate$Date||sb.quantity from StoreSubBatches SSB left join StoreBatches SB on (sb.id=ssb.StoreBatch_ID) where sb.name=%s and ssb.store=%s and sb.Storecard_id=%S'
                                                                    ,
                                                                    [quotedstr(mBarCodeEdt.text),quotedstr(mbo_ReceivedOrder_row.GetFieldValueAsString('Store_id')),mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_id')])
                                                                      ,mLL);
                                                                    ...
                                                                    if mLL.Count>0 then begin
                                                                         ...
                                                                         mI_Resultxax:=mForm_FunctionDoklad(xsite,0,0,360,480,'Výběr','Šarže',quotedstr(mbo_ReceivedOrder_row.GetFieldValueAsString('Storecard_id.name')),
                                                                                mll.strings[0],
                                                                                mll.strings[1],
                                                                                mll.strings[2],
                                                                                mll.strings[3],
                                                                         ...
                                                                         if (mI_Resultxax<>10) and (mI_Resultxax<>0) then begin
                                                                            ...
                                                                            if mI_Resultxax=0 then begin NxShowSimpleMessage('Šarže nebyla dohledána',nil);
                                                                                nxbeep(btfailure);
                                                                            end;
                                       ...
                                       if mix_result=22 then begin
                                           mbarcode:='';
                                           mOldBarCode:='';
                                           result:=(mpocet_zapis*(-1))-1;
                                           mpocet_zapis:=0;
                                       end;

                                       if mix_result=mrNo then mpocet_zapis:=mpocet_zapis-1;
                                       if mix_result=mrYes then begin
                                               mpocet_zapis:=mpocet_zapis+1;
                                             if mpocet_zapis>mcelkem-mdodano-mvychystano then begin
                                                 NxShowSimpleMessage('Max množství pro položku je ' + NxFloatToIBStr(mcelkem-mdodano-mvychystano) +
                                                 NxFloatToIBStr(mpocet_zapis) + ' - ' +NxFloatToIBStr(mcelkem-mdodano-mvychystano)
                                                 ,nil);
                                                 nxbeep(btfailure);
                                                 mpocet_zapis:=mcelkem-mdodano-mvychystano ;
                                             end;
                                        end;
                                             ...
                                             if (mbarcode<>'') then begin
                                                         if mOldBarCode<>'.' then begin
                                                                  if mix_result=mrOk then begin
                                                                           ...
                                                                           try
                                                                               xsite.BaseObjectSpace.SQLSelect(format('select max(sc.ID) from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id where (se.EAN=%s)', [quotedstr(mbarcode)]),
                                                                               mr);
                                                                               if (mr.Strings[0]<>'') and (mr.Strings[0]<>'0') then begin
                                                                                   mID_Storecard:=mr.Strings[0];
                                                                               end else begin
                                                                                   mID_Storecard:='';
                                                                               end;
                                                                           finally
                                                                           ...
                                                                           if mID_Storecard=mbo_ReceivedOrder_row.GetFieldValueAsstring('Storecard_id') then begin

                                                                                              if (mOldBarCode=mBarCode) or (mOldBarCode='') then begin
                                                                                                    mpocet_zapis:=mpocet_zapis+1;
                                                                                                    if mpocet_zapis=mcelkem-mdodano-mvychystano then begin

                                                                                                          //NxShowSimpleMessage('Max množství pro položku je ' + NxFloatToIBStr(mcelkem-mdodano-mvychystano),nil);
                                                                                ...
                                                                                NxShowSimpleMessage('není načten správný EAN',nil);
                                                                                nxbeep(btfailure);
                                                                           end;
             ...
             try
                 ...
                 mForm := TForm.Create(xsite);
                                       ...
                                       if mix_result=2 then begin
                                                ...
                                                mr:=tstringlist.create;
                                                try
                                                    xsite.BaseObjectSpace.SQLSelect(format('select ro2.id from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join IssuedOrders2 RO2 on ro2.storecard_id=sc.id left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((RO2.Quantity - Ro2.DeliveredQuantity)>=0)  and (se.EAN=%s and (RO.ID in (%s)))'
                                                                             ...
                                                                             for ixi:=0 to mxx.count-1 do begin
                                                                                  if mpocet>0 then begin
                                                                                      ...
                                                                                      if mpocet<=mbo.GetFieldValueAsFloat('quantity') - mbo.GetFieldValueAsFloat('DeliveredQuantity')-mbo.GetFieldValueAsFloat('X_vychystano')  then begin
                                                                                           mi_SQL:=xsite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set x_vychystano=' + NxFloatToIBStr(mpocet + mbo.GetFieldValueAsFloat('X_vychystano')) + ' where id=' + QuotedStr(mBO.OID));
                                                                                           mpocet:=0;
                                                                                      end else begin
                                                                                           ...
                                                                                           mi_SQL:=xsite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set x_vychystano=' + NxFloatToIBStr(mpomoc_pocet+ mbo.GetFieldValueAsFloat('X_vychystano')) + ' where id=' + QuotedStr(mBO.OID));
                                                                ...
                                                                if mr.count>0 then begin
                                                                 try
                                                                  ...
                                                                  mBO_Row_id:=mbo.oid;
                                                                        ...
                                                                        mTypSC:=mbo.GetFieldValueAsInteger('storecard_id.category');
                                                                        mstorecard_text:=mbo.GetFieldValueAsString('storecard_id.name');
                                                                               mcelkem:=GetFloatFromTable(xSite,'Select sum(RO2.quantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                                                               mdodano:=GetFloatFromTable(xSite,'Select sum(RO2.DeliveredQuantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                                                               mvychystano:=GetFloatFromTable(xSite,'Select sum(X_Vychystano) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                                                             ...
                                                                             for ixi:=0 to mxx.count-1 do begin
                                                                                  if mpocet>0 then begin
                                                                                      ...
                                                                                      if mpocet<=mbo.GetFieldValueAsFloat('quantity') - mbo.GetFieldValueAsFloat('DeliveredQuantity')-mbo.GetFieldValueAsFloat('X_dodano')  then begin
                                                                                           mi_SQL:=xsite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set x_dodano=' + NxFloatToIBStr(mpocet + mbo.GetFieldValueAsFloat('X_dodano')) + ' where id=' + QuotedStr(mBO.OID));
                                                                                           mpocet:=0;
                                                                                      end else begin
                                                                                           ...
                                                                                           mi_SQL:=xsite.BaseObjectSpace.SQLExecute('update IssuedOrders2 set x_dodano=' + NxFloatToIBStr(mpomoc_pocet+ mbo.GetFieldValueAsFloat('X_dodano')) + ' where id=' + QuotedStr(mBO.OID));
                                                                                           mpocet:=mpocet-mpomoc_pocet;
                                                                                      end;
                                                                                   end;
                                                                             end;
                                        ...
                                        if (mix_result=0) then begin
                                               ...
                                               if true then begin
                                                  ...
                                                  mr:=tstringlist.create;
                                                          ...
                                                          try
                                                             xsite.BaseObjectSpace.SQLSelect(format('select ro2.id from STOREEANS SE left join StoreUnits SU on SU.id=se.Parent_id left join Storecards SC on sc.id=su.parent_id left join IssuedOrders2 RO2 on ro2.storecard_id=sc.id left join IssuedOrders RO on ro.id=ro2.parent_id where (RO.closed = ''N'' ) and ((RO2.Quantity - Ro2.DeliveredQuantity-Ro2.X_dodano)>0) and  (se.EAN=%s and (RO.ID in (%s)))',[quotedstr(mbarcode),mIDs_dDocument]),mr);
                                                                 ...
                                                                 try
                                                                  ...
                                                                  mBO_Row_id:=mbo.oid;
                                                                        ...
                                                                        mstorecard_text:=mbo.GetFieldValueAsString('storecard_id.name');
                                                                               mcelkem:=GetFloatFromTable(xSite,'Select sum(RO2.quantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                                                               mdodano:=GetFloatFromTable(xSite,'Select sum(RO2.DeliveredQuantity) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                                                               mvychystano:=GetFloatFromTable(xSite,'Select sum(X_dodano) from IssuedOrders2 RO2 where (Ro2.Storecard_id=%s) and (RO2.Parent_ID in (%s))',quotedstr(mbo.GetFieldValueAsString('Storecard_ID')),mIDs_dDocument);
                                 ...
                                 if mix_result=81 then begin
                                     mlist_batch:=TStringList.create;
                                         try;
                                             ...
                                             mlist_batch:=
                                              BarCode_batch(xSite,'CDMK5QAWZZDL342X01C0CX3FCC',
                                                      0,0,360,480,
