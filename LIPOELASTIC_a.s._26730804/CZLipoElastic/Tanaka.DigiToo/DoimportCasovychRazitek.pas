...
procedure DoimportCasovychRazitek(pmSite: TSiteForm);
var
  lfProgress: TForm;
...
begin
  ...
  try
    ...
    lfProgress := ShowProgress('Doimport chybějících časových razítek', 1, 0, 'Probíhá příprava doimportu ...', true);
    ...
    for i:=0 to laList.Count-1 do begin
      if SetProgress(
        ...
        i+1,
        laList.Count,
        'Probíhá doimport časových razítek ('+IntToStr(i+1)+'/'+IntToStr(laList.Count)+')'
      ) then exit;
      ...
      try
        GetTimeStamps(OS, lcDocumentType, lcDigitooID, lcDocument_ID, AUTH_TOKEN, logs, False);
      except
        lcResult:= 'Při stahování časových razítek z Digitoo došlo k neočekávané chybě:'+#13#10+ExceptionMessage;
        ShowMessage(lcResult);
        exit;
      end;
    end;
    lfProgress.Hide;
    ShowMessage('Doimport chybějících časových razítek dokončen.');
  finally
    lfProgress.Free;
    laList.Free;
    laPom.Free;
