...
// odberatel
        if mXMLHead.getElementAsString('ABRADocument.Customer')='' then begin    // kontrolliji ID
               if mXMLHead.getElementAsString('ABRADocument.Customer.Name')<>'' then begin // kontroluji název firmy
                        ...
                        try
                                OS.SQLSelect(format('SELECT ID FROM Firms A WHERE Name=''%s'' AND A.Hidden=''N'' and Firm_ID is null order by id', [(mXMLHead.getElementAsString('ABRADocument.Customer.Name'))]), mRes);      //hledám id firmy
                                                 ...
                                                 if mI_modalresult=mrYes then begin                 // založ
                                                    ...
                                                    try
                                                          ...
                                                          if mtyp_Eshopu='OPZE' then begin
                                                                ...
                                                                try
                                                                    OS.SQLSelect('SELECT max(code) FROM Firms A WHERE substring(Code from 1 for 1)=' + quotedstr('0'), mRes1);
                                                        ...
                                                        mFirm.SetFieldValueAsString('ResidenceAddress_ID.Street',copy(mXMLHead.getElementAsString('ABRADocument.Customer.ResidenceAddress.Street'),1,60));
                                                        mFirm.SetFieldValueAsString('ResidenceAddress_ID.City',copy(mXMLHead.getElementAsString('ABRADocument.Customer.ResidenceAddress.City'),1,60));
