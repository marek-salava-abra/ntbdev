              ...
              mCustomBusinessObject:= TDynSiteForm(mSite).CurrentObject;
                    ...
                    //if mresult then begin
                        ...
                        try
                                          ...
                                          if NxCopyFile(adir+'\'+mfilename+'.pdf',constStoragePath+'\historie\'+mfilename+'_'+(FormatDateTime('YYYY_MM_DD_HH_NN',now))) then begin
                                              ...
                                              //NxShowSimpleMessage('uvolněni',nil);
                                                  ...
                                                  //NxShowSimpleMessage('Archivace výběrem ' + mid_reportx,nil);
                                                            mid:=iPrintDocument(mCustomBusinessObject,'KLFAXNODG3DL3ACT03KIU0CLP4',mid_reportx,NxCreateContext(msite.BaseObjectSpace),mStringlist,mfilename,adir);
                          ...
                          mi:=mCustomBusinessObject.ObjectSpace.SQLExecute('update IssuedCreditNotes set X_PrintReport_ID=' + quotedstr(mid_reportx) + ' where id='+ quotedstr(mCustomBusinessObject.oid));
                          mi:=mCustomBusinessObject.ObjectSpace.SQLExecute('update IssuedCreditNotes set X_Uzamceno=' + quotedstr('A') + ' where id='+ quotedstr(mCustomBusinessObject.oid));
                        finally
                            mStringlist.free;
                        end;
              end;
              ...
              if index=1 then begin
                    if mzruseni then begin
                        ...
                        adir:=Format('%s\%s\%s', [constStoragePath, mCustomBusinessObject.GetFieldValueAsString('Period_id.code'),mCustomBusinessObject.GetFieldValueAsString('Docqueue_id.code')]);
                            ...
                            mi:=mCustomBusinessObject.ObjectSpace.SQLExecute('update IssuedCreditNotes set X_PrintReport_ID=' + quotedstr('0000000000') + ' where id='+ quotedstr(mCustomBusinessObject.oid));
                            mi:=mCustomBusinessObject.ObjectSpace.SQLExecute('update IssuedCreditNotes set X_Uzamceno=' + quotedstr('N') + ' where id='+ quotedstr(mCustomBusinessObject.oid));
                         end else begin
                            NxShowSimpleMessage('Doklad ještě nebyl archovován',nil);
                         end;
                    end else begin
                            ...
                            mresult:=Create_folder(mCustomBusinessObject);
                                    ...
                                    try
                                       ...
                                       if mid_reportx<>'' then begin
                                                       mid:=iPrintDocument(mCustomBusinessObject,'KLFAXNODG3DL3ACT03KIU0CLP4',mid_reportx,NxCreateContext(mCustomBusinessObject.ObjectSpace),mStringlist,mfilename,adir);
