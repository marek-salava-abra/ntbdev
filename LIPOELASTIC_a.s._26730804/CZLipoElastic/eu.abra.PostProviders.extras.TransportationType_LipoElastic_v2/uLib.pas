uses
  'eu.abra.PostProviders.extras.TransportationType_LipoElastic_v2.uConst',
  'eu.abra.PostProviders.extras.TransportationType_LipoElastic_v2.uForm';
/////*****//////
...
begin
  ...
  case ABO.CLSID of
    ...
    Class_ReceivedOrder : mTable := 'ReceivedOrders2';
  end;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            ///Lipoelastic - Adrea
            if mDocBO.HasField('X_Delivery_adress_id' ) then
              if not CFxOID.IsEmptyOrFull(mDocBO.GetFieldValueAsString('X_Delivery_adress_id')) then
              begin
                mBOAddress := mOS.CreateObject(Class_Address);
                try
                  mBOAddress.Load(mDocBO.GetFieldValueAsString('X_Delivery_adress_id'));
                  mPackagesDataSet.FieldByName(cFDAdrName).AsString := mBOAddress.GetFieldValueAsString('City')+', '+mBOAddress.GetFieldValueAsString('Street');
                  mPackagesDataSet.FieldByName(cFDAdrStreet).AsString := mBOAddress.GetFieldValueAsString('Street');
                  mPackagesDataSet.FieldByName(cFDAdrCity).AsString := mBOAddress.GetFieldValueAsString('City');
                  mPackagesDataSet.FieldByName(cFDAdrPostCode).AsString := mBOAddress.GetFieldValueAsString('PostCode');
                  mPackagesDataSet.FieldByName(cFDAdrCountryCode).AsString := mBOAddress.GetFieldValueAsString('CountryCode');
                  mPackagesDataSet.FieldByName(cFDAdrPhoneNumber).AsString := mBOAddress.GetFieldValueAsString('PhoneNumber1');
            ...
            begin
              ...
              mHeaderDataSet.FieldByName(cFDDivision).AsString := mRows.BusinessObject[0].GetFieldValueAsString('Division_ID');
              //LIPO
              ...
              mNote := NxSearchReplace( NxSearchReplace(mNote,#10,' ',[srAll] ) ,#13,'',[srAll] );
              OutputDebugString('poznámka:'+mNote);
            ...
            begin
              ...
              begin
                ...
                //Lipoelastic - příklad DPE-35029/2023 z DEV_CZ_LIPO...


                mBranchID := '';
                if mDocBO.HasField('X_Delivery_adress_id' ) then
                  if not CFxOID.IsEmptyOrFull(mDocBO.GetFieldValueAsString('X_Delivery_adress_id' )) then
                    mBranchID := mOS.SQLSelectFirstAsString( Format('select X_PickupPoint from addresses where id = ''%s'' ',[ mDocBO.GetFieldValueAsString('X_Delivery_adress_id' ) ]));

                OutputDebugString('LIPO X_Delivery_adress_id.X_PickupPoint: '+mBranchID);

                if (mBranchID = '') or (mBranchID = '0') then
                  if mDocBO.HasField('X_PD_BB_Branches') then
                    if ( mDocBO.GetFieldValueAsString('X_PD_BB_Branches') <> '' )  then
                      mBranchID :=  UpperCase(mDocBO.GetFieldValueAsString('X_PD_BB_Branches'));

                mSQL := format(cSQLBranches, [QuotedStr(mPostProvider_ID), QuotedStr(  mBranchID  )  ]);
