...
function GetDeliveryaddress_ID(AOS:TNxCustomObjectSpace;SourceDocType:String;SourceDocID:String):String;
const cSQL = 'Select X_Delivery_adress_id from %s where ID = ''%s''  ';
begin
  Result:= '';
  if not CFxOID.IsEmptyOrFull(SourceDocID) then
  begin
    case SourceDocType of
    '03':Result:= AOS.SQLSelectFirstAsString( Format(cSQL,['IssuedInvoices',SourceDocID]) ,'');
    '21':Result:= AOS.SQLSelectFirstAsString( Format(cSQL,['StoreDocuments',SourceDocID]) ,'');
    'RO':Result:= AOS.SQLSelectFirstAsString( Format(cSQL,['ReceivedOrders',SourceDocID]) ,'');
    '22':Result:= AOS.SQLSelectFirstAsString( Format(cSQL,['StoreDocuments',SourceDocID]) ,'');
    end;
  end;
...
begin
  ...
  mID := GetDeliveryaddress_ID(Self.ObjectSpace,Self.GetFieldValueAsString('X_PD_SourceDocType'),Self.GetFieldValueAsString('X_PD_SourceDocID') );
  if not CFxOID.IsEmptyOrFull(mID) then
  begin
    Self.SetFieldValueAsString('X_Delivery_adress_id',mID);
  end;
