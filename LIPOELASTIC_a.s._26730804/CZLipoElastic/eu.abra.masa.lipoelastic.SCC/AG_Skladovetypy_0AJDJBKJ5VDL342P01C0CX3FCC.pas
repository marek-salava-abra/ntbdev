...
var mPnl: TPanel;
  ...
  mCostList,mGainList, mStoreList:TStringList;
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'PTComponents'));
    mCostList:=TStringList.create;
    mStoreList:=TStringList.create;
    mGainList:=TStringList.create;
    Result := TTabSheet.Create(APageControl);
    Result.PageControl := APageControl;
    Result.Caption := '&Skladové účtování';
    Result.Name := 'tabParamsPT';
    ...
    //self.BaseObjectSpace.SQLSelect('Select id from accounts where accounttype=''G'' and isincomplete=''N'' ',mGainList);
    //self.BaseObjectSpace.SQLSelect('Select id from accounts where accounttype=''C'' and isincomplete=''N'' ',mCostList);
    self.BaseObjectSpace.SQLSelect('Select id from accounts where code like''1%'' and isincomplete=''N'' ',mStoreList);
    ...
    try
      ...
      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'StoreAccount_ID', ftString, 10, False, 300006);
      with mFieldDef.CreateField(mDataSet, nil, 'xStoreAccount_ID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'StoreAccount_ID';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'CostAccount_ID', ftString, 100, False, 300008);
      with mFieldDef.CreateField(mDataSet, nil, 'xCostAccount_ID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'CostAccount_ID';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'GainAccount_ID', ftString, 100, False, 300009);
      with mFieldDef.CreateField(mDataSet, nil, 'xGainAccount_ID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'GainAccount_ID';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'PurchaseAccount_ID', ftString, 100, False, 300010);
      with mFieldDef.CreateField(mDataSet, nil, 'xPurchaseAccount_ID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'PurchaseAccount_ID';
      end;
      ...
      mFieldDef:= TFieldDef.Create(mDataSet.FieldDefs, 'Quantity', ftFloat, 0, True, 300008);
      with mFieldDef.CreateField(mDataSet, nil, 'xQuantity', False) do
      begin
        FieldKind:= fkData;
        FieldName:= 'Quantity';
      end;
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'PTComponents'));
    mGrd.Parent := mPnl;
    mGrd.Name := 'grdParamsTT';
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Kód skladu';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Order := 3;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Width := 100;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'Code';
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := Roll_Stores;
    ...
    mGridColumn.Caption := 'Název skladu';
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Layout := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Line := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).FieldName := 'StoreAccount_ID';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Skladový účet';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Order := 5;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Width := 100;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'Code';
    mGridObjectRollColumn.Parameters.Add('_Allowed='+mStoreList.CommaText);
    mGrd.AddColumn(mGridObjectRollColumn);
    mGridObjectRollColumn.ClassID := Roll_Accounts;

    mGridObjectRollColumn := TNxMultiGridObjectRollColumn.Create(mGrd);
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Complete := True;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Layout := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Line := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).FieldName := 'PurchaseAccount_ID';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Účet pořízení';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Order := 6;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Width := 100;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'Code';
    //mGridObjectRollColumn.Parameters.Add('_Allowed='+mCostList.CommaText);
    mGrd.AddColumn(mGridObjectRollColumn);
    mGridObjectRollColumn.ClassID := Roll_Accounts;
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Layout := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Line := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).FieldName := 'GainAccount_ID';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Výnosový účet';
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Order := 7;
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Width := 100;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'Code';
    mGridObjectRollColumn.Parameters.Add('_Allowed='+mGainList.CommaText);
    mGrd.AddColumn(mGridObjectRollColumn);
    mGridObjectRollColumn.ClassID := Roll_Accounts;   }
    ...
    if not mCanEdit then
      mAct.Tag := -1;
    mListBox.Items.AddObject('mactPTLSEdit', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactPTLSRefresh';
    mAct.Caption := 'Občerstvit';
    mAct.Category := Result.Name;
    mAct.ShortCut := TextToShortCut('F11');
    mAct.OnExecute := @actLSRefreshExecute;
    mListBox.Items.AddObject('mactPTLSRefresh', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactPTLSSave';
    mAct.Caption := 'Uložit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactPTLSSave', mAct);

    mAct := Self.GetNewAction;
    mAct.Name := 'mactPTLSCancel';
    mAct.Caption := 'Zrušit';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    ...
    mListBox.Items.AddObject('mactPTLSCancel', mAct);
...
begin
  ...
  mLbl1.Left := 8;
  mLbl1.Top := 5;
  mLbl1.Caption := 'Ke skladovému typu:';
...
begin
  ...
  for i := 0 to AWinControl.ControlCount - 1 do begin
    if (AWinControl.Controls[i].Name = 'grdParamsTT') then begin // Grid nezakazujeme ale nastavime na ReadOnly
...
begin
  ...
  if not NxIsEmptyOID(mStorecardID) then begin
    ...
    ' PAR.X_StoreCard_ID as Property_ID, SC.Name as Property_Name,' +
    ' PAR.X_PosIndex as PosIndex' +
    ' from DefRollData PAR ' +
    ' LEFT JOIN TransportationTypes SC ON SC.ID=PAR.X_TransportationType_ID '+
...
begin
  ...
  if Sender.Tag = -1 then begin
    NxShowMessage(Application.Title, 'Nemáte oprávnění editace skladových typů', mdInformation, False, mSite.GetSiteAppForm);
    Exit;
  end;
  mListBox := TListBox(NxFindChildControl(mSite, 'PTComponents'));
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSEdit'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSRefresh'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSSave'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSCancel'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSSave'))).Enabled := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSCancel'))).Enabled := True;

  setParamsEnabled(TWinControl(mListBox.Items.Objects(mListBox.Items.IndexOf('tabParamsPT'))), True);
end;
...
begin
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSEdit'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSRefresh'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSSave'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSCancel'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSSave'))).Enabled := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactPTLSCancel'))).Enabled := False;

  setParamsEnabled(TWinControl(aListBox.Items.Objects(aListBox.Items.IndexOf('tabParamsPT'))), False);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'PTComponents'));
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            //GlobParams.GetOrCreateParam(dtString, 'ParStorecardType_ID').AsString := '';
             mStoreCardBO:=ASite.BaseObjectSpace.CreateObject('SFMSB5W15BDL3CL100C4RHECN0');
             mStoreCardBO.load(mDataSet.FieldByName('Property_ID').AsString,nil);

            mBO.SetFieldValueAsString('X_Parent_ID', mOID);
            mBO.SetFieldValueAsString('X_Store_ID', mDataSet.FieldByName('Property_ID').AsString);
            mBO.SetFieldValueAsString('X_StoreAccount_ID', mDataSet.FieldByName('StoreAccount_ID').AsString);
            mBO.SetFieldValueAsString('X_PurchaseAccount_ID', mDataSet.FieldByName('PurchaseAccount_ID').AsString);
            mBO.SetFieldValueAsString('X_GainAccount_ID', mDataSet.FieldByName('GainAccount_ID').AsString);
            mBO.SetFieldValueAsString('X_CostAccount_ID', mDataSet.FieldByName('CostAccount_ID').AsString);
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    ' PAR.X_Store_ID as Property_ID, SC.Name as Property_Name,' +
    ' PAR.X_PosIndex as PosIndex, PAR.X_StoreAccount_ID as StoreAccount_ID, PAR.X_PurchaseAccount_ID as PurchaseAccount_ID, PAR.X_GainAccount_ID as GainAccount_ID, PAR.X_CostAccount_ID as CostAccount_ID' +
    ' from DefRollData PAR ' +
    ' LEFT JOIN Stores SC ON SC.ID=PAR.X_Store_ID '+
    ' where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) + 'and PAR.X_rel_def='+QuotedStr(cRelDef)+' and PAR.X_Parent_ID=%s order by PAR.X_PosIndex';
  ...
  if pgcDataViews.ActivePage <> TTabSheet(pgcDataViews.FindChildControl('tabParamsPT')) then Exit;
  ...
  try
    ...
    if mResult.Active then begin
      ...
      while not mResult.eof do begin
        ...
        mDataSet.FieldByName('GainAccount_ID').AsString := mResult.FieldByName('GainAccount_ID').AsString;
        mDataSet.FieldByName('PurchaseAccount_ID').AsString := mResult.FieldByName('PurchaseAccount_ID').AsString;
        mDataSet.FieldByName('StoreAccount_ID').AsString := mResult.FieldByName('StoreAccount_ID').AsString;
        mDataSet.FieldByName('CostAccount_ID').AsString := mResult.FieldByName('CostAccount_ID').AsString;
        mCanLookup := true;
        mDataSet.Post;
        mResult.Next;
      end;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject('SFMSB5W15BDL3CL100C4RHECN0');
          try
...
begin
  Result := True;
  mListBox := TListBox(NxFindChildControl(aSite, 'PTComponents'));
  if not (TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSEdit'))).Enabled) then begin
    ...
    try
      ...
      Result := TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactPTLSEdit'))).Enabled;
    end;
  end;
end;
