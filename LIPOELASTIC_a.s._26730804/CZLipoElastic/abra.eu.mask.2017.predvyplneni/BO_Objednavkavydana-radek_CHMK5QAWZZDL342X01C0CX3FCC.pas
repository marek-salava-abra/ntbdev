...
var
  mCode: integer;
  mdivision,mstore: string;
begin
  if self.GetFieldValueAsInteger('RowType')=3 then begin
         if mCode= Self.GetFieldCode('Storecard_ID') then begin
                     // if not NxIsEmptyOID(self.GetFieldValueAsString('Parent_id.Docqueue_ID.X_Store_ID')) then begin
                     //     mstore:=(Self.GetFieldValueAsString('Parent_id.Docqueue_ID.X_Store_ID'));
                     //     Self.SetFieldValueAsString('Store_ID',mstore );
                     //     NxShowSimpleMessage(mstore,nil);
                     //  end;
