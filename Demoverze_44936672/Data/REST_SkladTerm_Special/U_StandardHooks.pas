...
// vraci ID rady, ktera se ma doplnit na doklad (POZOR, ne vzdy musi byt k dispozici ADocument - melo by tedy vychazet z kombinace typu dokladu a scenare)
// ASourceDocType obsahuje typ zdrojoveho dokladu - tedy napr. prijemka pro naskl. do pozic - je vyplneno, pouze pokud dava smysl
// ASourceDocument obsahuje zdrojovy doklad - u rad pro skl. doklady je zde konkretni doklad, u
// pokud se nic nevraci, pouzije se vychozi rada z konstant (a ve scenarich bez dokladu se nevyplni nic)
...
begin
  Result := '';
  if (ADocType=DOC_IncomingTransfer) and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then Result:='7RB0000101';
...
// ziskani strediska podle uzivatele
function getDivisionForUser(AOS: TNxCustomObjectSpace; AUser_ID: String): String;
begin
  Result := STREDISKO_HLAVNI;
end;

// podminka, kterou lze omezit pridavané artikly  (listStoreCard a getStoreCardInfo)
function StoreCard_Where(AOS: TNxCustomObjectSpace; AModule: String): String;
begin
  Result := '';
end;
...
function defaultSearchString_Prefill(AOS: TnxCustomObjectSpace; ADocType, AUser_ID: String): String;
begin
  Result := '';
end;
...
begin
  ...
  begin
   ...
   {if (ADocType = DOC_VYD) and ((AUser_ID = '4030000101') or (AUser_ID = '3030000101') or (AUser_ID = '1030000101') or (AUser_ID = '2030000101')) then begin //fojtik M, benke M, svidroch M, klapec M
    Result:= Result + ' and SD.DocQueue_ID =  ''10C0000101''';  //DLM
  end;}
   if (ADocType = DOC_OutgoingTransfer) and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then Result:= Result + ' and SD.DocQueue_ID =  ''6RC0000101''';   //pda eshop
   if (ADocType = DOC_OutgoingTransfer) and (AUser_ID = '3EJ0000101') then Result:= Result + ' and SD.DocQueue_ID =  ''6RB0000101''';   //pda simon
   if (ADocType = DOC_IncomingTransfer) and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then Result:= Result + ' and SD.DocQueue_ID =  ''7RC0000101''';
   if (ADocType = DOC_IncomingTransfer) and (AUser_ID = '3EJ0000101') then Result:= Result + ' and SD.DocQueue_ID =  ''7RB0000101''';
   if (ADocType = DOC_ReceiptCard) and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then Result:= Result + ' and SD.DocQueue_ID =  ''6N20000101''';
   if (ADocType = DOC_ReceiptCard) and (AUser_ID = '3EJ0000101') then Result:= Result + ' and SD.DocQueue_ID =  ''7RD0000101''';
   if (ADocType = DOC_BillOfDelivery) and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then Result:= Result + ' and SD.DocQueue_ID =  ''8RC0000101''';
end;

// pole, ktera se maji ziskat v SELECTu, ale do ctecky se nedostanou (slouzi napriklad k vlastnimu razeni)
function listDocQueue_AuxNonVisibleFields(AOS: TNxCustomObjectSpace; AModule: String): String;
begin
  Result := ' '''' as "Aux"';
end;

// razeni fronty dokladu
function listDocQueue_OrderBy(AOS: TNxCustomObjectSpace; AModule: String): String;
begin
...
// vlastni join pro funkci vracejici seznam sarzi (SB je prefix tabulky StoreBatches)
function listStoreBatches_Join(AOS: TNxCustomObjectSpace; AModule, DocumentType, AUser_ID: String): String;
begin
  Result := '';
end;
...
begin
 // Result := 'SC.EAN';
  ...
  Result := 'SC.Code || '';'' || (select list(SE.EAN, '';'') ' +
        'from StoreUnits SU ' +
        'join StoreEANs SE on SE.Parent_ID = SU.ID ' +
        'join StoreCards SC2 on SC2.ID = SU.Parent_ID ' +
        'where SC2.ID =  SC.ID)';
...
begin
  if ADocType = DOC_LogStoreTransfer then
    ...
    Result := 'SC.Code' + CONCAT_STR + 'coalesce(SB.Name, '''')' + nxCrLf
  else
...
function putQueueDocDetailStartPicking_StoreBatchField(AOS: TNxCustomObjectSpace; AModuleName, ADocType: String): String;
begin
...
// funkce volana pred vytvorenim JSON z datasetu. Slouzi pro upravu radku pred odeslanim do ctecky
// pouze ve frontovych scenarich
...
procedure putQueueDocDetailStartPicking_beforeJSONCreate(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String; ARows: TMemTable);
begin
end;

// umoznuje zmenit JSON s dokladem pred odeslanim do ctecky
function putQueueDocDetailStartPicking_BeforeJSONSend(AModuleName, AJSON: String): String;
begin
  Result := AJSON;
end;
...
function get_StoreCardInfo_SummaryValue(AOS: TNxCustomObjectSpace; AStoreCard_ID: string; var ATitle: string): String;
begin
  ATitle := QuotedStr('Ve výdeji');
  Result := '(select sum(SSC.BookedQuantity) from StoreSubCards SSC where SSC.StoreCard_ID = ' + QuotedStr(AStoreCard_ID) + ')';
end;
...
var
  mSql: String;
begin
  Result := 'SSC.BookedQuantity';
end;

// pole, ktere se prida do dotazu pri ziskani informaci o artiklu
// vklada se do dotazu dostupnem v REST_SkladTerm.U_SQLQueries.getStoreCardInfoSql - artikl lze ziskat pod aliasem SC
function get_StoreCardInfo_AuxField(AOS: TNxCustomObjectSpace; AModule, AUser_ID, AStoreCard_ID, AFirm_ID, AStore_ID: String): String;
var
 mBO:TNxCustomBusinessObject;
 mPrice, mReserved, mStavS:Extended;
 mFlist:TStringList;
 mFNames:string;
 i:integer;
begin
  try
    ...
    mBO.Load(AStoreCard_ID,nil);
    mPrice:=NxEvalObjectExprAsFloatDef(mbo,'NxGetStoreCardUnitPriceDef('+Quotedstr('')+', '+Quotedstr('')+', ' + QuotedStr(mbo.OID) + ','+Quotedstr('1000000101')+', '+Quotedstr(mBO.GetFieldValueAsString('MainUnitCode'))+',False,'+QuotedStr('0000CZK000')+','+inttostr(trunc(Date))+')',0);
    ...
    mStavS:=AOS.SQLSelectFirstAsExtended('Select quantity-acceptedquantity from storesubcards where storecard_id='+QuotedStr(AStoreCard_ID)+' and store_id='+QuotedStr('1L00000101'));
    mFlist:=TStringList.create;
    aos.SQLSelect('select dq.code||'+Quotedstr('-')+'||ro.ordnumber||'+Quotedstr('/')+'||p.code, f.name, '+Quotedstr('(')+'||cast(ro2.quantity as numeric(13,2))||'+QuotedStr(')')+' from receivedorders ro left join receivedorders2 ro2 on ro.id=ro2.parent_id left join firms f on f.id=ro.firm_id left join docqueues dq on dq.id=ro.docqueue_id left join periods p on p.id=ro.period_id where ro2.storecard_id='+Quotedstr(AStoreCard_ID)+' and (ro2.deliveredquantity=0) and ro.closed=''N'' ',mFlist);
    if mFlist.count>0 then begin
      ...
      Result:= QuotedStr('Cena: ' + FloatToStr(mPrice) + ' Kč'+#13#10+'Typ: '+mbo.GetFieldValueAsString('StoreAssortmentGroup_ID.Code')+#13#10+'Rezervace: '+FloatToStr(mReserved)+' sklad 555'+FloatToStr(mStavS)+#13#10+mFNames+#13#10+mbo.GetFieldValueAsString('X_AES_NAME_CARD'));
    end;
    if mflist.Count=0 then
    Result:= QuotedStr('Cena: ' + FloatToStr(mPrice) + ' Kč'+#13#10+'Typ: '+mbo.GetFieldValueAsString('StoreAssortmentGroup_ID.Code')+#13#10+'Rezervace: '+FloatToStr(mReserved)+' sklad 555'+FloatToStr(mStavS)+#13#10+mbo.GetFieldValueAsString('X_AES_NAME_CARD'));
  except
   Result := QuotedStr('');
  end;
end;
...
function listDocQueue_Field_FirmName(AOS: TNxCustomObjectSpace): String;
begin
  Result := 'F.Name';
end;
...
procedure beforeSaveHook(AOS: TNxCustomObjectSpace; AModule: String; ASD: TNxCustomBusinessObject; ASDType: Integer; AJson: TJSONSuperObject;
  ARows: TMemTable);
begin
end;
...
// Hacek volany ihned po ulozeni dokladu (pred vytvarenim PRP, polohovaku a noveho oddeleneho dokladu se zbytkem polozek)
// ASDType je typ ukladaneho dokladu: 0 - puvodni doklad, 1 - oddleny doklad na nezpracovane radky
procedure afterSaveHook(AOS: TNxCustomObjectSpace; AModule, AUser_ID: String; ADocument: TNxCustomBusinessObject; ASDType: Integer;
...
begin
   NxScriptingLog.WriteEvent(logInfo,'AModule: '+AModule+#13#10+
                                     'ADocType: '+ADocType+#13#10+
                                     'AUser_ID: '+AUser_ID+#13#10+
                                     'ASDNew_ID: '+ASDNew_ID+#13#10+
                                     'Objekt :'+ASD.DisplayName);
  if (asd.GetFieldValueAsString('DocQueue_ID.Code')='PPCT') and (AUser_ID in ['3EI0000101','2620000101','4EP0000101','3EB0000101']) then begin
   //ChangeStatusByRule(ASD,'');
   Result:=False;
  end  else begin
   Result := True;
  end;
...
function putQueueDocDetailStartPicking_isEditingProcessedItemAllowed(AOS: TNxCustomObjectSpace; AModuleName: String): Boolean;
begin
  Result := False;
end;
...
function putQueueDocDetailStartPicking_headerAuxFields(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String): String;
begin
  Result := '';
end;

// SQL sloupec, ktery se zobrazi jako dodatecna informace v seznamu radku
function rowAuxText(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String): String;
begin
  Result := 'SC.Code';
end;
...
// vracena hodnota by mela byt ID nalezeneho artiklu
...
procedure parseBarcodeForRowSpecial(AOS: TNxCustomObjectSpace; AModule, ADocType, ABarcode: String;
  ADocument_ID, ARow_ID, AFirm_ID, AStoreCard_ID, AStoreFrom_ID, AStoreFromPosition_ID, AStoreTo_ID, AStoreToPosition_ID, AStoreBatch_ID, AUnitCode: String; AUnitQuantity: Double;
  var OStoreCard_ID, OStoreBatch_ID, OStoreFrom_ID, OStorePositionFrom_ID, OStorePositionTo_ID, OUnitCode, OStoreBatchAux: String; var OStoreBatchExpirationDate, OUnitQuantity: Double;
...
function customGetRowsSql(AOS: TNxCustomObjectSpace; AModule, AUser_Id, ADocType: String; var AWhere: String): String;
begin
  Result := '';
end;
...
function showSerNumberSelection(AOS: TNxCustomObjectSpace; AModule: String): Boolean;
begin
  if AModule in ['STD_ReceiptCardQueue', 'STD_ReceiptCardWithoutDoc', 'STD_TransferInQueue', 'STD_RefundedBillOfDeliveryQueue'] then
    Result := False
  else
    Result := True;
end;
...
// pokud je argument AStoreCard_ID prazdny, je funkce volana pri nacitani radku a ID lze najit v SC.ID,
// pokud je argument AStoreCard_ID vyplneni, je funkce volana pri nacteni artiklu. SC.ID lze ovsem pouzit taky
function enabledCustomFields(AOS: TNxCustomObjectSpace; AUser_ID, AModule: String; AStoreCard_ID: String = ''): String;
begin
  Result := QuotedStr('');
  ...
  if AStoreCard_ID = '5100000101' then
    Result := 'case when SC.Code = ''39'' then ''X_Note=Rychle;X_Cislo=5'' else '''' end'
  else if AStoreCard_ID = '2100000101' then
    Result := '''X_Note=Rychle''';}
end;
...
function listDocQueue_customRowColor(AOS: TNxCustomObjectSpace; AModule, AUser_ID: String): String;
begin
  //Result := QuotedStr('');
...
// Urci, ktera pole se navic oproti tem standardnim prenesou z JSONu do datasetu. Musi zacinat carkou.
function putQueueDocDetailStopPicking_rowsDatasetFields(AOS: TNxCustomObjectSpace; AModule: String): String;
begin
  Result := '';
end;
...
function LoginObjectAuxField(AUser_ID: String): String;
begin
  Result := '';
end;
...
// Podporovano ve scenarich: Scenare dle dokladu, Informace ze skladu, Vkladani EAN
...
begin
  Result := False;
  if ADocType in [DOC_ReceiptCard, DOC_MainInvProtocol, DOC_PartialInvProtocol] then
    Result := True;
end;
...
begin
  Result := 0;
  if ADocType = DOC_ReceiptCard then
    Result := 1;
end;
...
begin
  Result := 0;
  if AModule in ['STD_ReceiptCardQueue','STD_BillOfDeliveryQueue','STD_TransferQueue','STD_InfoFromStores'] then Result := 1;
end;
...
begin
  if AModule in ['STD_ReceiptCardQueue','STD_BillOfDeliveryQueue','STD_TransferQueue','STD_InfoFromStores','STD_TransferInQueue'] then begin
   ...
   for i:=1 to mcount do begin
     if AUser_ID in ['4EP0000101','2620000101','3EI0000101','3EB0000101','6EV0000101'] then
     CFxReportManager.PrintByIDs(NxCreateContext(AOS),mList,mCLSID2,'5VL1000101',rtoPrint,pekPDF,'Qoltec_eshop', '')
      else
     CFxReportManager.PrintByIDs(NxCreateContext(AOS),mList,mCLSID,'1K70000101',rtoPrint,pekPDF,'Datamax_BOSCH', '');
   end;
...
//   0 - Kalendar (výchozí)
//   1 - Textove pole
function EnterStoreBatchExpirationDate(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String; var OEnterType: Integer): String;
begin
  Result := '''False''';
  OEnterType := 0;
end;
...
// 1 - Skladnik muze hodnotu zmenit, ale musi ji potvrdit (nacist kódem) - zatím nepodporováno
...
procedure EnteredFieldsBehavior(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID: String; var OStoreCard, OStoreBatch, OStorePosition: String);
begin
end;
