...
begin
  ...
  try
    ...
    begin
      ...
      mForm := Create_FormOKCancel(mSiteForm, mPanel, getString('message_service_action_move_title'), 355, 200);

      mLabel := TLabel(mForm.FindComponent('Popisek'));
      if not Assigned(mLabel) then
        mLabel := Create_Label(mForm, mPanel, 5, 5, 150, 15, 'Popisek', Format(getString('message_service_action_move_select_document'), [mSiteForm.CurrentObject.GetFieldValueAsString('Name')]));

      mRadios := TRadioGroup(mForm.FindComponent('Scenarios'));
      if not Assigned(mRadios) then
        mRadios := Create_RadioGroup(mForm, mPanel, 40, 5, 200, 'Scenarios', 'Scénáře:', mScenarios.Text)
      else
      begin
      ...
      mForm.Height := mRadios.Height + 120;
