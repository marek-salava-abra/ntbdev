...
////////////////////////////////////////////////////////////////////////////////
procedure defaultSearchString(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mDocType, mUser_ID, mResult: String;
begin
  if slPath.Count = 3 then
  begin
    mDocType := slPath.Strings[1]; //ocekavam typ dokladu
    mUser_ID := slPath.Strings[2]; //ocekavam id uzivatele
  end else
  begin
  ...
  mResult := defaultSearchString_Prefill(Self.ObjectSpace, mDocType, mUser_ID);

  HTTPResponse(AResponse, HTTP_SC_OK, ContentType_JSON, PlainResponse(Trim(mResult)));
end;

////////////////////////////////////////////////////////////////////////////////
procedure listDocQueue(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
...
begin
  ...
  try
    ...
    mSql := GetListDocQueueSql(Self.ObjectSpace, mModule, mDocType, mUser_ID, mSearchStr);
...
////////////////////////////////////////////////////////////////////////////////
procedure putQueueDocDetailStartPicking(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mSql, mUser_Id, mDocType, mDocTypeB, mDoc_ID, mModuleName, mModule, mAuxField, mPerson_ID,
  ...
  try
    ...
    try
      ...
      begin
        ...
        if not ABRA then
        begin
          mPerson_ID := SQLSelectStr(Self.ObjectSpace, 'select Person_ID from SecurityUsers where ID = ' + QuotedStr(mUser_Id));
          if not CFxOID.IsEmpty(mPerson_ID) then
          begin
           mBO.SetFieldValueAsString('StoreMan_ID', mUser_Id);
           mBO.Save;
          end;
        end;
      end
      ...
      begin
        ...
        begin
          mBO2_BoD := Self.ObjectSpace.CreateObject(Class_BillOfDelivery);
          mBO2_OutT := Self.ObjectSpace.CreateObject(Class_OutgoingTransfer);
          try
            ...
            Self.ObjectSpace.SQLSelect2(mSql, dtStoreDocuments);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if mBO2_BoD.GetFieldValueAsString(mStatusField) <> STAV_K_VYSKLADNENI(DOC_BillOfDelivery, mModule) then
                    RaiseException(Format(getString('bod_wrong_status'), [mBO2_BoD.GetFieldValueAsString('DisplayName')]));

                  ChangeStatusByRule(mBO2_BoD, PRECHOD_ZAHAJENI(DOC_BillOfDelivery, mModule), '0000000000', mUser_Id);
                end
                ...
                begin
                  ...
                  if mBO2_OutT.GetFieldValueAsString(mStatusField) <> STAV_K_VYSKLADNENI(DOC_OutgoingTransfer, mModule) then
                    RaiseException(Format(getString('outtransfer_wrong_status'), [mBO2_OutT.GetFieldValueAsString('DisplayName')]));

                  ChangeStatusByRule(mBO2_OutT, PRECHOD_ZAHAJENI(DOC_OutgoingTransfer, mModule), '0000000000', mUser_Id);
                end
                else
      ...
      begin
        ...
        mSql := getRowsSql(Self.ObjectSpace, mModule, mUser_Id, mDocType, mWhere);
        Self.ObjectSpace.SQLSelect2(mSql, dtRows);
        if dtRows.Active then
        begin
...
////////////////////////////////////////////////////////////////////////////////
procedure putQueueDocDetailCancelPicking(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
...
begin
  ...
  begin
    ...
    mInv := NxStrToInt(slPath.Strings[3]);
  end else
  begin
    ErrREST(ARequest, AResponse, HTTP_SC_BadRequest, getString('wrong_parameters_count'));
    exit;
  ...
  LogWriteSectionStart('put-storedoc-cancel');
  ...
  try
    //ukoncim dle typu
    case mInv of
      0: cancelQueue(mDoc_ID, mModule, mDocType, mTemporaryStorageID, Self.ObjectSpace);
      1: cancelInventorizationByDIP(mUser_Id, mDoc_ID, mTemporaryStorageID, Self.ObjectSpace);
      2: cancelInventorizationFree(mTemporaryStorageID, Self.ObjectSpace);
      else RaiseException(getString('document_unknown_type'))
    end;
...
////////////////////////////////////////////////////////////////////////////////
procedure putQueueDocDetailStopPicking(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mSql, mUser_Id, mDocType, mListDocType, mLSD_ID, mDoc_ID, mLastStoreDocument2_ID, mLastStoreBatch_ID,
    ...
    mDocTypeB, mRowId, mSDNewDamaged_ID, mSDClassID, mLSDNotProcessed_ID: String;
  ...
  begin
    ...
    begin
      ...
      if ABRA then
      begin
        //PLMContainerMater_ID := SQLSelectStr(Self.ObjectSpace, 'select Parent_ID from PLMMIPLMaterialDistrib where StoreDocument2_ID = ' + QuotedStr(mSDNewRow.OID));
  ...
  begin
    ...
    if mDocType = DOC_IssuedOrder then
      mDocQueue_ID := GetDocQueueForDocument(Self.ObjectSpace, mModule, DOC_ReceiptCard, mUser_Id, RADA_PRIJEMKA, mDocType, mSD)
    else
      mDocQueue_ID := GetDocQueueForDocument(Self.ObjectSpace, mModule, mDocType, mUser_Id, mSD.GetFieldValueAsString('DocQueue_ID'), mDocType, mSD);
  ...
  begin
    ...
    putQueueDocDetailStopPicking_afterSDNewCreateHook(Self.ObjectSpace, mModule, mSD, mSDNew, json, dtJSONRows);
    ...
    mOriginSDField := putQueueDocDetailStopPicking_OriginStoreDocumentField(Self.ObjectSpace, mModule, mDocType, mUser_Id);
    ...
    begin
      ...
      beforeSaveHook(Self.ObjectSpace, mModule, mSDNew, 1, json, dtJSONRows);
      gLog.WriteEvent(logDebug, 'After beforeSaveHook');

      mSDNew.Save;

      gLog.WriteEvent(logDebug, 'Before afterSaveHook');
      afterSaveHook(Self.ObjectSpace, mModule, mUser_Id, mSDNew, 1, json, dtJSONRows);
      ...
      if (dtDocumentQuantity.RecordCount > 0)
        and (putQueueDocDetailStopPicking_CreateLogStoreDocumentForNotPickedRows(Self.ObjectSpace, mModule, mDocType, mUser_Id)
          or (mDocType = DOC_IssuedOrder))
      then
      begin
        mLSDNotProcessed_ID := REST_Create_LogStoreDocument(Self.ObjectSpace, mSDNew, '',
          mLogStoreDocumentClass,
          GetDocQueueForDocument(Self.ObjectSpace, mModule, mLogStoreDocumentDocType, mUser_ID, mLogStoreDocument_DocQueue_ID, mDocType, mSDNew),
  ...
  var
    mMySD: TNxCustomBusinessObject;
    mMySDRows: TNxCustomBusinessMonikerCollection;
    mMySD_ID: String;
  begin
    try
      mMySD := nil;
      ...
      if putQueueDocDetailStopPicking_NewRowsToSeperateDoc(Self.ObjectSpace, mModule, mDocType) then
      begin
        // pokud neni OBP, zalozime volny doklad
        mMySD := Self.ObjectSpace.CreateObject(mSDClassID);
        mMySD.ExplicitTransaction := True;
        mMySD.New;
        mMySD.Prefill;
        mMySD.SetFieldValueAsString('DocQueue_ID', mSD.GetFieldValueAsString('DocQueue_ID'));
        mMySD.SetFieldValueAsString('Firm_ID', mSD.GetFieldValueAsString('Firm_ID'));
        ...
        if cNewRowsDOcIssuedOrderField <> '' then
          mMySD.SetFieldValueAsString(cNewRowsDOcIssuedOrderField, SQLSelectStr(Self.ObjectSpace,
              'select ' + FIRST_TOP(1) + ' Provide_ID from StoreDocuments2 where Parent_ID = ' + QuotedStr(mSD.OID) + ' ' + FIRST_TOP_ORACLE(1)));
      end
      else
      begin
        mMySD := mSD;
      ...
      while not dtJSONRows.EOF do
      begin
        mSDRow := mMySDRows.AddNewObject;
        mSDRow.Prefill;

        //do datasetu doplnim ID - kvuli pozdejsimu vytvareni PrePri
        dtJSONRows.Edit;
        dtJSONRows.FieldByName('StoreDocument2_ID').AsString := mSDRow.OID;
        dtJSONRows.Post;
        ...
        if (mSDRow.GetFieldValueAsInteger('StoreCard_ID.Category') = 2)
          and not CFxOID.IsEmpty(dtJSONRows.FieldByName('StoreBatch_ID').AsString) then
        begin
          // pridame sarze podle napipane skutecnosti
        ...
        // pokud je artikl s evidenci seriovych cisel
        if (mSDRow.GetFieldValueAsInteger('StoreCard_ID.Category') = 1) then
        begin
          // musime do jsonu pro kolekci sernums
          jsonSerNums := json.A['rows'].O[dtJSONRows.FieldByName('jsonIndex').AsInteger].A['sernums'];
        ...
        putQueueDocDetailStopPicking_afterNewRowFill(Self.ObjectSpace, mModule, mDocType, mUser_ID, mSDRow, mMySD, mMySDRows, dtJSONRows);
    ...
    if not CFxOID.IsEmpty(mMySD_ID) then
    begin
      mMySD := Self.ObjectSpace.CreateObject(mSDClassID);
      try
        mMySD.ExplicitTransaction := True;
        mMySD.Load(mMySD_ID, nil);

        if (mMySD.GetFieldValueAsString(mStatusField) <> STAV_PO_ODDELENI_NOVE(mDocType, mModule))
          or (mMySD.GetFieldValueAsString('ResponsibleRole_ID') <> ROLE_PO_ODDELENI_NOVE(mDocType, mModule)) then
        begin
          ChangeStatus(mMySD, STAV_PO_ODDELENI_NOVE(mDocType, mModule), ROLE_PO_ODDELENI_NOVE(mDocType, mModule));
        end;
      finally
        mMySD.Free;
      end;
...
begin
  ...
  try
    ...
    case Request_Start(Self.ObjectSpace, mRequestID, 'putQueueDocDetailStopPicking') of
      1: begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          begin
            ...
            mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, Class_BillOfDelivery, Class_RefundedBillOfDelivery);
            try
              ...
              mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString :=
                GetDocQueueForDocument(Self.ObjectSpace, mModule, mDocType, mUser_ID, RADA_VRATKA_VYDEJKY, DOC_BillOfDelivery);
              ...
              if ABRA then
                mSD2.SetFieldValueAsString('DocQueue_ID', GetDocQueueForDocument(Self.ObjectSpace, mModule, mDocType, mUser_ID, RADA_VRATKA_VYDEJKY, DOC_BillOfDelivery));
          ...
          begin
            ...
            mSD := GetStoreDocBOID(dtStoreDocuments.FieldByName('Provide_ID').AsString, Self.ObjectSpace, mDocType);
          end
          else
            mSD := GetStoreDocBO(Self.ObjectSpace, mDocType);
          try
            ...
            begin
              ...
              begin
                mSql :=
                  ...
                  'where ' + nxCrLf +
                  '  A2.Parent_ID = ' + QuotedStr(mDoc_ID);
                Self.ObjectSpace.SQLSelect(mSql, mStoreDocRows);
              end;
              ...
              mLSD_ID := SQLSelectStr(self.ObjectSpace,
              ...
              begin
                mLSD := Self.ObjectSpace.CreateObject(mLogStoreDocumentClass);
                try
              ...
              begin
                mPrePri_ID := SQLSelectStr(Self.ObjectSpace,
                ...
                begin
                  ...
                  mLSDPrePri_ID := SQLSelectStr(Self.ObjectSpace,
                    'select ' + FIRST_TOP(1) + ' LSD2.Parent_ID ' +
                    'from StoreDocuments2 SD2 ' +
                    'join LogStoreDocuments2 LSD2 on SD2.ID = LSD2.StoreDocRow_ID ' +
                  ...
                  begin
                    mLSD := Self.ObjectSpace.CreateObject(Class_LogStoreInput);
                    try
                      mLSD.ExplicitTransaction := True;
                  ...
                  mPrePri := Self.ObjectSpace.CreateObject(Class_IncomingTransfer);
                  try
              ...
              begin
                ...
                if putQueueDocDetailStopPicking_IgnoreRow(Self.ObjectSpace, mModule, mSDRow) then
                  continue;

                // pokud jde o PRP, tak si nechci mazat sarze, protoze bych ztratil vazbu
                mDocRowBatches := mSDRow.GetLoadedCollectionMonikerForFieldCode(mSDRow.GetFieldCode('DocRowBatches'));
                if (mModule <> 'STD_TransferInQueue') and (mDocType <> DOC_RefundedBillOfDelivery) then
                begin
                ...
                // jinak nastavime zpracovane mnozstvi
                  ...
                  mWasSelectedByBarcodeFieldName := putQueueDocDetailStopPicking_FieldForWasSelectedByBarcodeInfo(Self.ObjectSpace);
                  ...
                  begin
                    ...
                    if ((not dtJSONRows.FieldByName('Processed').AsBoolean)
                      and ((mModule <> 'STD_TransferInQueue') and (mDocType <> DOC_RefundedBillOfDelivery)))
              ...
              glog.WriteEvent(logDebug, 'Before beforeSaveHook');
              beforeSaveHook(Self.ObjectSpace, mModule, mSD, 0, json, dtJSONRows);
              glog.WriteEvent(logDebug, 'After beforeSaveHook');
              ...
              //Self.ObjectSpace.EnableHooks(False, skBusinessObject, Class_OutgoingTransfer, '_AfterSave_PostHook');
              //try
                if mSD.NeedSave then
                  mSD.Save;
              //finally
              //  Self.ObjectSpace.EnableHooks(True, skBusinessObject, Class_OutgoingTransfer);
              //end;
              glog.WriteEvent(logDebug, 'After mSD.Save');

              // hacek po ulozeni dokladu
              glog.WriteEvent(logDebug, 'Before afterSaveHook');
              afterSaveHook(Self.ObjectSpace, mModule, mUser_Id, mSD, 0, json, dtJSONRows);
              glog.WriteEvent(logDebug, 'After afterSaveHook');
                  ...
                  begin
                    ...
                    try
                      ...
                      if not NxIsBlank(mStoreBatch.GetFieldValueAsString('Specification'))
                        and not NxIsBlank(putQueueDocDetailStopPicking_AuxInfoForSerNumField(Self.ObjectSpace)) then
                      begin
                        mStoreBatch.SetFieldValueAsString(putQueueDocDetailStopPicking_AuxInfoForSerNumField(Self.ObjectSpace),
              ...
              begin
                ...
                begin
                  ...
                  if (putQueueDocDetailStopPicking_NewRowsToSeperateDoc(Self.ObjectSpace, mModule, mDocType) and dtJSONRows.FieldByName('IsNew').AsBoolean) then
                  begin
                    dtJSONRows.Next;
                    continue;
                  end;
              ...
              if dtDocumentQuantity.RecordCount > 0 then
                if CreateLogStoreDocument(Self.ObjectSpace, mModule, mDocType, mUser_Id, mLSD_ID, mSD, json, dtJSONRows, dtDocumentQuantity) then
                begin
                  mLSD_ID := REST_Create_LogStoreDocument(Self.ObjectSpace, mSD, mLSD_ID,
                    mLogStoreDocumentClass,
              ...
              begin
                ...
                mParams := TNxParameters.Create;
                mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, Class_OutgoingTransfer, Class_IncomingTransfer);
                try
                  ...
                  if ((mParams.GetOrCreateParam(dtstring,'Store_ID').AsString = '')
                    ...
                    mParams.GetOrCreateParam(dtString, 'Store_ID').AsString := SKLAD_HLAVNI;
                  mParams.GetOrCreateParam(dtString, 'DocQueue_ID').AsString :=
                    GetDocQueueForDocument(Self.ObjectSpace, mModule, DOC_IncomingTransfer, mUser_ID, RADA_PREPRIJEM, mDocType, mSD);
                  mDIM.LoadParams(mParams);
                  mDIM.Execute;
                  ...
                  begin
                    ...
                    mPrePri.ExplicitTransaction := Self.ObjectSpace.InTransaction;
                  end;
                  ...
                  if dtDocumentQuantity.RecordCount > 0 then
                    mLSDPrePri_ID := REST_Create_LogStoreDocument(Self.ObjectSpace, mPrePri, mLSDPrePri_ID,
                      Class_LogStoreInput,
                      GetDocQueueForDocument(Self.ObjectSpace, mModule, DOC_LogStoreInput, mUser_ID, LogStoreInput_DocQueue_ID, DOC_IncomingTransfer, mPrePri),
            ...
            begin
              ...
              try
                ...
                if (mSLROs.Count > 0) and (mJOs.Count > 0) then
                  ...
                  mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, Class_PRFJobOrder, mSDClassID)
                else if mSLROs.Count > 0 then
                  mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, mOrderClass, mSDClassID)
                else
                  // kdyz nejsou zadne vazby, tak udelam jen defaultniho manazera, abych ho pak korektne uvolnil
                  mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, Class_ReceivedOrder, Class_BillOfDelivery);

                // klasicke nezpracovane radky oddelime do zvlastniho dokladu
                mParams := TNxParameters.Create;
                mRO := Self.ObjectSpace.CreateObject(mOrderClass);
                mJO := Self.ObjectSpace.CreateObject(Class_PRFJobOrder);
                try
                  mSDNewFree := False;
                  ...
                  begin
                    ...
                    // pokud neni OBP, zalozime volny doklad
                    mSDNew := Self.ObjectSpace.CreateObject(mSDClassID);
                    mSDNew.ExplicitTransaction := True;
                    mSDNew.New;
                    mSDNew.Prefill;
          ...
          try
            ...
            if putQueueDocDetailStopPicking_changeSDStatus(Self.ObjectSpace, mModule, mDocType, mUser_ID, mSDNew_ID, 0, mSD, mSDNew) then
            begin
            ...
            if (mDocType = DOC_RefundedBillOfDelivery) and useBillOfDeliveryForRefunding then
            begin
              mSD2 := Self.ObjectSpace.CreateObject(Class_BillOfDelivery);
              try
            ...
            if not CFxOID.IsEmpty(mPrePri_ID) then
            begin
              mPrePri := Self.ObjectSpace.CreateObject(Class_IncomingTransfer);
              try
                mPrePri.ExplicitTransaction := True;
            ...
            begin
              ...
              try
                ...
                if putQueueDocDetailStopPicking_changeSDStatus(Self.ObjectSpace, mModule, mDocType, mUser_ID, mSDNew_ID, 1, mSD, mSDNew) then
                begin
      ...
      begin
        ...
        begin
          mDIM := NxCreateDocumentImportManager(Self.ObjectSpace, GetStoreDocClass(mDocType), GetStoreDocClass(mListDocType));
          mSDNew := Self.ObjectSpace.CreateObject(GetStoreDocClass(mDocType));
          try
            mParams := TNxParameters.Create;
            try
              ...
              mBONew.ExplicitTransaction := Self.ObjectSpace.InTransaction;
              mBONew.Save;
...
///////////////////////////////////////////////////////////////////////////////
procedure get_SumQuantityForStoreCardAndStoreDocument(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
...
begin
  ...
  begin
    ...
    mStoreDocument_ID := slPath.Strings[2]; //ocekavam skladovy doklad
  end else
  begin
    ErrREST(ARequest, AResponse, HTTP_SC_BadRequest, getString('wrong_parameters_count'));
    exit;
...
///////////////////////////////////////////////////////////////////////////////
procedure getStoreDocumentRow(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mStoreDocument2_ID, mUser_Id, mModule, mSql, mDocType, mWhere: String;
  dtRow: TMemTable;
  mSL : TStringList;
  json: TJSONSuperObject;
begin
  if (slPath.Count = 2) then
  begin
    mStoreDocument2_ID := slPath.Strings[1]; //ocekavam id radku
  end else
  begin
    ErrREST(ARequest, AResponse, HTTP_SC_BadRequest, getString('wrong_parameters_count'));
    exit;
  ...
  try
    ...
    mSql := getRowsSql(Self.ObjectSpace, mModule, mUser_Id, mDocType, mWhere);
    Self.ObjectSpace.SQLSelect2(mSql, dtRow);
    ...
    begin
      ...
      ErrREST(ARequest, AResponse, HTTP_SC_NotFound, Format(getString('error_row_not_found'), [mStoreDocument2_ID]));
    end;
  finally
    dtRow.Free;
    mSL.Free;
...
var
  ...
  mModules, mModuleParameters: TStringList;
  mFirst: Boolean;
  ...
  try
    ...
    begin
      ...
      if mModuleParameters.Count > 2 then
        mDocumentType := mModuleParameters.Strings[2];
      ...
      if mModule in ['STD_ReceiptCardQueue', 'STD_ReceiptCardFastQueue', 'STD_BillOfDeliveryQueue', 'STD_BillOfDeliveryFastQueue', 'STD_RemovalListsQueue',
          ...
          'STD_RefundedBillOfDeliveryQueue', 'STD_JobOrderQueue', 'STD_OutgoingSubstitutionQueue', 'STD_IncomingSubstitutionQueue',
          'STD_OutgoingTransformationQueue', 'STD_IncomingTransformationQueue', 'STD_TransferBetweenPositionsQueue'] then
        mSql := GetListDocQueueSql(AOS, mModule, mDocumentType, AUser_ID, defaultSearchString_Prefill(AOS, mDocumentType, AUser_ID), True)
      else if mModule in ['STD_InventorizationByDIP', 'STD_InventorizationByDIPFast'] then
        mSql := GetListPartialInvProtocolsSql(AOS, mModule, mDocumentType, AUser_ID, defaultSearchString_Prefill(AOS, mDocumentType, AUser_ID), True)
      else if mModule in ['STD_InventorizationFree', 'STD_InventorizationFreeFast'] then
        mSql := GetListMainInvProtocolsSql(AOS, mModule, mDocumentType, AUser_ID, defaultSearchString_Prefill(AOS, mDocumentType, AUser_ID), True);
      ...
      begin
        ...
        mFullSql := mFullSql + 'select ' + QuotedStr(mModule) + ' as "Module", count("ID") as "Count" from (' + nxCrLf
          + mSql + ') X' + nxCrLf;
      end
      else
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (trim(mModule) = trim(mResultModules.FieldByName('Module').AsString))
            and (mResultModules.FieldByName('Count').AsInteger >= 0) then
          begin
            mModules.Strings[i] := Format('%s-%d', [mModule, mResultModules.FieldByName('Count').AsInteger]);
          end;
          mResultModules.Next;
        end;
      end;
...
procedure GetMainMenuButtons(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mUser_Id, mModules,: String;
begin
  ...
  try
    ...
    mModules := ModifyVisibleModulesString(Self.ObjectSpace, getStoreForUser(Self.ObjectSpace, mUser_Id), mUser_ID);
    HTTPResponse(AResponse, HTTP_SC_OK, ContentType_JSON, PlainResponse(mModules));
  finally
    LogWriteSectionEnd;
  end;
end;

procedure put_CodeResult(Self: TNxWebServicesHelper; ARequest: TNxHTTPRequest;
  AResponse: TNxHTTPResponse; slPath: TStringList);
var
  mUser_ID, mModule, mDocType, mBarcode, mStoreDocument_ID: String;
begin
  if (slPath.Count <> 1) then
  begin
    ErrREST(ARequest, AResponse, HTTP_SC_BadRequest, getString('wrong_parameters_count'));
  ...
  try
    ...
    parseBarcodeForRowSpecial(Self.ObjectSpace, mModule, mDocType, mBarcode, '', '', '', '', '', '', '',
      ...
      -1, -1,
      '', -1, nil, nil, nil, nil);
    HTTPResponse(AResponse, HTTP_SC_OK, ContentType_JSON, PlainResponse(mStoreDocument_ID));
  finally
    LogWriteSectionEnd;
  end;
end;
