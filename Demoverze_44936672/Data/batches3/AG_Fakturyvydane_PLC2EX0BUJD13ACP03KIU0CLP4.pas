    ...
    if mRows.Count>0 then begin
      ...
      while not(TDataSet(mDataset).Eof) do begin
        if (mDataset.CurrentObject.GetFieldValueAsInteger('StoreCard_ID.Category')=2) then begin
             mBillOfDeliveryRow:=mDataset.CurrentObject.GetMonikerForFieldCode(mDataset.CurrentObject.GetFieldCode('ProvideRow_ID')).BusinessObject;
             mDocRowBatches:=mBillOfDeliveryRow.GetLoadedCollectionMonikerForFieldCode(mBillOfDeliveryRow.GetFieldCode('DocRowBatches'));
