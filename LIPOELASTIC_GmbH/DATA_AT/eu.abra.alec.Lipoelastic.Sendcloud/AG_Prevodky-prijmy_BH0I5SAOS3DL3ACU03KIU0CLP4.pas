...
begin
  mPDMIssuedDoc_ID:= Self.BaseObjectSpace.SQLSelectFirstAsString('SELECT R.LeftSide_ID FROM Relations R WHERE R.REL_DEF = 1439 AND R.RightSide_ID = '+QuotedStr(Self.CurrentObject.OID));
...
begin
  ...
  try
    ...
    if (mList.Count > 0) and (mList.Count < 1000) then
      ShowPDMIssuedDocs(mOS, 1439, mList, mSite);
  finally
    mList.Free;
  end;
end;
