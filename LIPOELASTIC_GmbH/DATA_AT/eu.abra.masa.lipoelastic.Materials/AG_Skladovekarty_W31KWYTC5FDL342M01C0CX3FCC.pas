...
begin
  ...
  if mControl <> nil then begin
    ...
    mListBox.Name := 'MaterialComponents';
...
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'MaterialComponents'));

    Result := TTabSheet.Create(APageControl);
    Result.PageControl := APageControl;
    Result.Caption := '&Materials';
    Result.Name := 'tabParamsMat';
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'MaterialComponents'));
    mGrd.Parent := mPnl;
    mGrd.Name := 'grdParamsMat';
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Material';
    ...
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := Roll_ND_Materials;
...

{
    ...
    mGridColumn.Order:= 4;
    mGridColumn.FieldName:= 'Quantity';
    mGridColumn.Caption:= 'Percent';
    mGrd.AddColumn(mGridColumn);
    x := 1;
    ...
    setParamsEnabled(Result, False);
        ...
        if not(mAllowed) then
          mAct.Tag := -1;
        mListBox.Items.AddObject('mactMaterLSEdit', mAct);

        mAct := Self.GetNewAction;
        mAct.Name := 'mactMaterLSRefresh';
        mAct.Caption := 'Občerstvit';
        mAct.Category := Result.Name;
        mAct.ShortCut := TextToShortCut('F11');
        mAct.OnExecute := @actLSRefreshExecute;
        mListBox.Items.AddObject('mactMaterLSRefresh', mAct);

        mAct := Self.GetNewAction;
        mAct.Name := 'mactMaterLSSave';
        mAct.Caption := 'Uložit';
        mAct.Category := Result.Name;
        mAct.KeepApart := True;
        mAct.Visible := False;
        ...
        mListBox.Items.AddObject('mactMaterLSSave', mAct);

        mAct := Self.GetNewAction;
        mAct.Name := 'mactMaterLSCancel';
        mAct.Caption := 'Zrušit';
        mAct.Category := Result.Name;
        mAct.KeepApart := True;
        mAct.Visible := False;
        ...
        mListBox.Items.AddObject('mactMaterLSCancel', mAct);
...
begin
  ...
  for i := 0 to AWinControl.ControlCount - 1 do begin
    if (AWinControl.Controls[i].Name = 'grdParamsMat') then begin // Grid nezakazujeme ale nastavime na ReadOnly
...
begin
  ...
  if not NxIsEmptyOID(mStorecardID) then begin
    ...
    ' PAR.X_Picture as Property_ID, SC.Name as Property_Name,' +
    ' PAR.X_PosIndex as PosIndex, PAR.X_FloatValue1 as Quantity' +
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(mSite, 'MaterialComponents'));
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSEdit'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSRefresh'))).Enabled := False;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSSave'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSCancel'))).Visible := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSSave'))).Enabled := True;
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSCancel'))).Enabled := True;

  setParamsEnabled(TWinControl(mListBox.Items.Objects(mListBox.Items.IndexOf('tabParamsMat'))), True);
end;
...
begin
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSEdit'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSRefresh'))).Enabled := True;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSSave'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSCancel'))).Visible := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSSave'))).Enabled := False;
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('mactMaterLSCancel'))).Enabled := False;

  setParamsEnabled(TWinControl(aListBox.Items.Objects(aListBox.Items.IndexOf('tabParamsMat'))), False);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'MaterialComponents'));
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            //GlobParams.GetOrCreateParam(dtString, 'ParStorecardType_ID').AsString := '';
             //mStoreCardBO:=ASite.BaseObjectSpace.CreateObject('535FINKNYNDL3C5P00CA141B44');
            ...
            mBO.SetFieldValueAsString('X_Material_ID', mDataSet.FieldByName('Property_ID').AsString);
            ...
            mBo.SetFieldValueAsFloat('X_NumericValue',mDataSet.FieldByName('Quantity').AsFloat);
            mStoreCardBO.free;
            mBO.Save;
          finally
            mBO.Free;
...
begin
  mSQLGetAllParams := 'Select PAR.ID as ID,' +
    ' PAR.X_Material_ID as Property_ID, Par.X_NumericValue as NumericValue, ' +
    ' PAR.X_PosIndex as PosIndex ' +
    ' from DefRollData PAR ' +
    //' LEFT JOIN PaymentTypes SC ON SC.ID=PAR.X_PaymentType_ID '+
  ...
  if pgcDataViews.ActivePage <> TTabSheet(pgcDataViews.FindChildControl('tabParamsMat')) then Exit;
  ...
  try
    ...
    if mResult.Active then begin
      ...
      while not mResult.eof do begin
        ...
        //GetCached(mCachePropertyNames, mResult.FieldByName('Property_ID').AsString, mResult.FieldByName('Property_ID_Name').AsString);
        ...
        mDataSet.FieldByName('Quantity').AsFloat:=mResult.FieldByName('NumericValue').AsFloat;
        mCanLookup := true;
        mDataSet.Post;
        mResult.Next;
      end;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject(Class_BO_ND_Materials);
          try
...
begin
  Result := True;
  mListBox := TListBox(NxFindChildControl(aSite, 'MaterialComponents'));
  if not (TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSEdit'))).Enabled) then begin
    ...
    try
      ...
      Result := TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('mactMaterLSEdit'))).Enabled;
    end;
  end;
end;
