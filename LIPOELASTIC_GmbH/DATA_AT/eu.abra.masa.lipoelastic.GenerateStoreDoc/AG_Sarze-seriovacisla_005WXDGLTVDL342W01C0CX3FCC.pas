...
begin
  ...
  mAction.Caption := '##Create Delivery note##';
  mAction.Items.Add('##Create Delivery note##');
  mAction.Items.Add('##Create Bill of transfer - out##');
  mAction.Items.Add('##Create Replacements - issuances##');
  mAction.Items.Add('##Create Changes - issuances##');
  mAction.Items.Add('##Create Receipt card##');
  ...
  mAction.ShowControl := True;
  mAction.ShowMenuItem := True;
  mAction.Caption := '##Import Quantity for storebatch##';
  mAction.Items.Add('##Set quantity from CSV##');
  mAction.Items.Add('##Reset quantity##');
  mAction.Hint:='Set quantity from CSV, structure BatchName, Quantity, from first line';
  mAction.Category := 'tabList';
  mAction.OnExecuteItem := @QuantityFromCSV;
end;

Procedure QuantityFromCSV(Sender:TComponent;index:integer);
var
 mSite:TSiteForm;
 ...
 i,j:integer;
 mList:TStringList;
 mBatchName, mTempstr, mBatch_ID:string;
 mQuantity:Extended;
...
begin
 ...
 if index=0 then begin
   ...
   mOpenDlg.Filter := 'Data file (*.csv)| *.csv';
   if mOpenDlg.Execute then begin
    try
      ...
      if mList.Count>0 then begin
        try
         WaitWin.StartProgress('Seting quantity ...', '', mlist.Count);
         for i:=0 to mList.count-1 do begin
           mTempstr:=mList.strings[i];
           mBatchName:=NxTrapStrTrim(mTempstr,';');
           mQuantity:=NxIBStrToFloat(NxTrapStrTrim(mTempstr,';'));
           mBatch_ID:=mOS.SQLSelectFirstAsString('Select id from storebatches where hidden='+QuotedStr('N')+' and name='+QuotedStr(mBatchName),'');
           if not(NxIsEmptyOID(mBatch_ID)) then begin
             mBO:=mOS.CreateObject(Class_StoreBatch);
             mBO.Load(mBatch_ID,nil);
             mbO.SetFieldValueAsFloat('X_Quantity',mQuantity);
             mBO.Save;
           end else begin
             NxShowSimpleMessage('Batch not found: '+mBatchName,mSite);
           end;
 ...
 if index=1 then begin
   mList:=TStringList.create;
   mOS.SQLSelect('Select id from storebatches where X_quantity>0', mList);
   if mList.Count>0 then begin
    try
     WaitWin.StartProgress('Reseting quantity ...', '', mlist.Count);
     for i:=0 to mList.count-1 do begin
       mBO:=mOS.CreateObject(Class_StoreBatch);
 ...
 if mlist.count>0 then begin
  ...
  if GetDataForBOD(msite, mStore_ID, mDivision_ID, mResult, mName) then begin
      if mResult=1 then begin
         ...
         frmStoreBatches.Width:= 720;
         frmStoreBatches.Height:= 600;
         frmStoreBatches.Caption:= 'Data for storebatches';
        ...
        if mlist.Count>0 then begin
            ...
            order:=order+1;
            fd := TFieldDef.Create(dtStoreBatches.FieldDefs, 'Quantity2', ftString, 40, False, order);
            ...
            for i := 0 to (mList.Count - 1) do begin
              ...
              if mOS.SQLSelectFirstAsExtended('Select sum(quantity) from storesubbatches where storebatch_id='+QuotedStr(mList.Strings[i])+' and store_id='+QuotedStr(mStore_ID),0)>=0 then begin
                ...
                dtStoreBatches.FieldByName('Quantity2').AsString:='0';
  ...
  //konec dialogu
       if iResult>0 then begin
        ...
        if iResult = mrOk then begin
          if NxMessageBox('Prompt', 'Generate '+mName+' for '+IntToStr(mSBCount)+' items?', mdConfirm, mdbYesNo, 2, 0, False, nil) = mrYes then begin
...
begin
 if ASite <> nil then begin
    ...
    mForm.Caption := 'Data for '+aName;


    mLabel1 := TLabel.Create(mForm);
    mLabel1.Parent := mForm;
    mLabel1.Caption := 'Warehouse:';
    mLabel1.Top := 12;
    mLabel1.Left := 17;
    mLabel1.Height := 13;
    mLabel1.Width := 100;
