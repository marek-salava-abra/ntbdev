...
var mCanLookup: Boolean; //příznak, kdy se smí zobrazovat názvy (kvůli rychlosti)
  ...
  mGridObjectRollColumnR:TNxMultiGridObjectRollColumn;
  mGridColumnChar, mGridColumnChar2: TNxMultiGridCustomColumn;
...
procedure SetRollClsid;
var
  i: integer;
  mTestList: TStringList;
  mSQL, mCLSID: string;
begin
 { mTestList := TStringList.Create;
  try
    mSQL := 'select X_ParamValue from defrolldata where ID = ''%s''';
    mSQL := Format(mSQL, [mDataSet.FieldByName('Property_ID').AsString]);
    NxShowSimpleMessage(mSQL,nil);
    mCLSID:=mOS.SQLSelectFirstAsString(mSQL, '');
    mGridObjectRollColumnR.ClassID := mCLSID;
  finally
...
procedure fldSCOnChange(Sender: TField);
var
  mSQL, mCLSID: string;
  mPrice:Extended;
...
begin
  //SetRollClsid;
  if (mDataset.State = dsInsert) or (mDataset.State = dsEdit) then begin
    //dtGoods.FieldByName('fldSavedUnitCode').AsString := '';
    // predvyplneni na hlavni jednotku pri zmene skl.karty
    if not NxIsEmptyOID(mDataset.FieldByName('Property_ID').AsString) then begin
        mBO:=mOS.CreateObject('KCAWICC3H2O4DG0YEDMLO4X0PK');
        mBO.Load(mDataset.FieldByName('Property_ID').AsString,nil);
        if mbo.GetFieldValueAsInteger('X_TypeOfValue')=0 then begin
         mGridColumnChar.ReadOnly:=false;
        end;
        if mbo.GetFieldValueAsInteger('X_TypeOfValue')=1 then begin
          mGridColumnChar.ReadOnly:=true;
        end;
    end;
  end;
end;
...
var mPnl: TPanel;
  ...
  mBoolGridColumn, mBoolGridColumn2:TNxMultiGridBooleanColumn;
  mColl:TNxMultiGridColumn;
  mBtn: array[0..7] of TButton;
  mCheckBox: TCheckBox;
  i, x: Integer;
  ...
  mSuper, mCanEdit, mCanDelete, mAllowed: Boolean;
  pnParamsTop: TPanel;
...
begin
  try
    ...
    mUser:=mOS.CreateObject(Class_SecurityUser);
    mUser.load(NxGetActualUserID(self.BaseObjectSpace),nil);
    mAllowed:=muser.GetFieldValueAsBoolean('X_SCTab');
    muser.free;
    try
    ...
    //mDataSet.AfterScroll := @dtAfterScroll;
    ...
    try
      ...
      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'RollValueName', ftString, 100, False, 300006);
      with mFieldDef.CreateField(mDataSet, nil, 'xRollValueName', False) do begin
        Size := 100;
        FieldKind := fkData;
        FieldName := 'RollValueName';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'RollValueID', ftString, 100, False, 300007);
      with mFieldDef.CreateField(mDataSet, nil, 'xRollValueID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'RollValueID';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'RollCLSID', ftString, 100, False, 300009);
      with mFieldDef.CreateField(mDataSet, nil, 'xRollCLSID', False) do begin
        Size := 10;
        FieldKind := fkData;
        FieldName := 'RollCLSID';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'MarkForDelete', ftBoolean, 0, False, 300010);
      with mFieldDef.CreateField(mDataSet, nil, 'xMarkForDelete', False) do begin
        FieldKind := fkData;
        FieldName := 'MarkForDelete';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'DataType', ftInteger, 0, False, 300011);
      with mFieldDef.CreateField(mDataSet, nil, 'xDataType', False) do begin
        FieldKind := fkData;
        FieldName := 'DataType';
      end;

      mFieldDef := TFieldDef.Create(mDataSet.FieldDefs, 'PosIndex', ftString, 10, False, 300012);
      with mFieldDef.CreateField(mDataSet, nil, 'xPosIndex', False) do begin
        FieldKind := fkData;
        FieldName := 'PosIndex';
      end;

      mFieldDef:= TFieldDef.Create(mDataSet.FieldDefs, 'ParamValue', ftString, 40, False, 300013);
      ...
      mFieldDef:= TFieldDef.Create(mDataSet.FieldDefs, 'NumericValue', ftFloat, 0, False, 300014);
      with mFieldDef.CreateField(mDataSet, nil, 'xNumericValue', False) do
      begin
        FieldKind:= fkData;
        FieldName:= 'NumericValue';
      end;

      mFieldDef:= TFieldDef.Create(mDataSet.FieldDefs, 'BooleanValue', ftBoolean, 0, False, 300015);
      with mFieldDef.CreateField(mDataSet, nil, 'xBooleanValue', False) do
      begin
        FieldKind:= fkData;
        FieldName:= 'BooleanValue';
      end;

      mFieldDef:= TFieldDef.Create(mDataSet.FieldDefs, 'VariantBool', ftBoolean, 0, False, 300016);
      with mFieldDef.CreateField(mDataSet, nil, 'xVariantBool', False) do
      begin
        FieldKind:= fkData;
        FieldName:= 'VariantBool';
      end;

    except
      OutputDebugString(ExceptionMessage);
    end;
    mDataSet.FieldByName('Property_ID').OnChange := @fldSCOnChange;
    mDataSet.FieldByName('Posindex').OnChange := @fldSCOnChange;
    mDataSet.FieldByName('ParamValue').OnChange := @fldSCOnChange;
    mDataSet.FieldByName('RollValueName').OnChange := @fldSCOnChange;
    mDataSet.FieldByName('NumericValue').Alignment := taRightJustify;
    ...
    TNxMultiGridCustomColumn(mGridObjectRollColumn).Caption := 'Code';
    ...
    {
    mGridObjectRollColumnR := TNxMultiGridObjectRollColumn.Create(mGrd);
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Complete := True;
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Layout := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Line := 0;
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).FieldName := 'RollValue';
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Caption := 'Čís. hodnota';
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Order := 5;
    TNxMultiGridCustomColumn(mGridObjectRollColumnR).Width := 80;
    mGridObjectRollColumn.SecurityMask := 2;
    mGridObjectRollColumn.TextField := 'Code';
    mGrd.AddColumn(mGridObjectRollColumnR);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumnR.ClassID := Roll_Barva;
...

{
    ...
    mGridColumnChar:= TNxMultiGridColumn.Create(mGrd);
    mGridColumnChar.Layout:= 0;
    mGridColumnChar.Width:= 100;
    mGridColumnChar.Order:= 7;
    mGridColumnChar.FieldName:= 'ParamValue';
    mGridColumnChar.Caption:= 'Value';
    mGrd.AddColumn(mGridColumnChar);

    mGridColumnChar2:= TNxMultiGridColumn.Create(mGrd);
    mGridColumnChar2.Layout:= 0;
    mGridColumnChar2.Width:= 100;
    mGridColumnChar2.Order:= 8;
    mGridColumnChar2.FieldName:= 'RollValueName';
    mGridColumnChar2.Name := 'gcRollValueName';
    mGridColumnChar2.Caption:= 'Roll value';
    mGridColumnChar2.ReadOnly:=True;
    mGrd.AddColumn(mGridColumnChar2);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;

    mColl := TNxMultiGridColumn.Create(mGrd);
    mColl.Layout := 0;
    mColl.Line := 0;
    mColl.Order := 9;
    mColl.FieldName := 'NumericValue';
    mColl.Caption := 'Numeric value';
    mColl.Width := 80;
    //mGridColumn.Grid.Align:= alRight;
    mGrd.AddColumn(mColl);
    //mGrd.columns.Alignment:= taRightJustify;


    mBoolGridColumn := TNxMultiGridBooleanColumn.Create(mGrd);
    mBoolGridColumn.Layout := 0;
    mBoolGridColumn.Line := 0;
    mBoolGridColumn.Order := 10;
    mBoolGridColumn.FieldName := 'BooleanValue';
    mBoolGridColumn.Caption := 'Bool';
    mBoolGridColumn.Width := 80;
    mGrd.AddColumn(mBoolGridColumn);

    mBoolGridColumn2 := TNxMultiGridBooleanColumn.Create(mGrd);
    mBoolGridColumn2.Layout := 0;
    mBoolGridColumn2.Line := 0;
    mBoolGridColumn2.Order := 11;
    mBoolGridColumn2.FieldName := 'VariantBool';
    mBoolGridColumn2.Caption := 'Variant';
    mBoolGridColumn2.Width := 80;
    mGrd.AddColumn(mBoolGridColumn2);
    ...
    mGridColumn.Order:= 9;
    mGridColumn.FieldName:= 'NumericValue';
    mGridColumn.Caption:= 'Číslo';
    mGrd.AddColumn(mGridColumn);  }
    ...
    for i := 0 to 7 do begin
      ...
      if i = 6 then begin
        mBtn[i].Width := 110;
        mBtn[i].Caption := 'Doplnit podle karty';
      ...
      if i = 7 then begin
        mBtn[i].Width := 110;
        mBtn[i].Caption := 'Doplnit podle skupiny';
        mBtn[i].onClick := @addFromClickP;
        mBtn[i].Tag := ObjToInt(mDataSet);
        mBtn[i].Name := 'btnAddFromP';
      end;
      x := x + mBtn[i].Width;
    end;
    ...
    mAct := Self.GetNewAction;
    mAct.Name := 'm2actLSRollValue';
    mAct.Caption := 'Čís. hodnota';
    mAct.Category := Result.Name;
    mAct.KeepApart := True;
    mAct.Visible := False;
    mAct.Enabled := false;
     mAct.ShortCut := TextToShortCut('F6');
    mAct.OnExecute := @actLSRollValue;
    mListBox.Items.AddObject('m2actLSRollValue', mAct);
  ...
  if not NxIsEmptyOID(mStorecardID) then begin
    ...
    try
      mSQL := 'Select PAR.ID as ID,' +
              ' PAR.X_Parameter_ID as Property_ID, '+
              ' D.Name as Property_Name,' +
              ' PAR.X_PosIndex as PosIndex, '+
              ' PAR.X_ParamValue as ParamValue' +
              ' PAR.X_RollValueID as RollValueID, '+
              ' PAR.X_RollValueName as RollValueName, ' +
              ' PAR.X_Variantni_polozka as VariantBool '+
              ' from DefRollData PAR ' +
              ' LEFT JOIN DefrollData D ON d.ID=PAR.X_parameter_ID '+
              ' where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) +
              ' and PAR.X_rel_def='+QuotedStr(cRelDef)+
              ' and PAR.X_Value_ID='+QuotedStr(mStorecardID)+
              ' order by PAR.X_PosIndex';
...
procedure addFromClickP(Sender: TButton);
var
  mDataSet: TDataSet;
  mGroup_ID: string;
  mSQL: string;
...
begin
  ...
  mObjSpace := TNxCustomObjectSpace(mListBox.Items.Objects(mListBox.Items.IndexOf('ObjectSpace')));
  if not Assigned(mObjSpace) then exit;
  mGroup_ID := GetGroupIDFromVisualRoll;
  if not NxIsEmptyOID(mGroup_ID) then begin
    mTmpDts := TMemoryDataSet.Create(nil);
    try
      mSQL := 'Select PAR.ID as ID,' +
              ' PAR.X_Parameter_ID as Property_ID, '+
              ...
              ' LEFT JOIN DefrollData D ON d.ID=PAR.X_parameter_ID '+
              ' where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) +
              ' and PAR.X_rel_def='+QuotedStr('11')+
              ' and PAR.X_Value_ID='+QuotedStr(mGroup_ID)+
              ' order by PAR.X_PosIndex';
      mObjSpace.SQLSelect2(mSQL, mTmpDts);
      if mTmpDts.Active then begin
        mTmpDts.First;
...
function GetGroupIDFromVisualRoll: string;
var
  mRoll: Variant;
  mOLE: Variant;
begin
  mOLE := GetAbraOLEApplication;
  mRoll := mOLE.GetRoll('HABF3IGER2UOXAJ3PCQWUYGVI4', 1);
  result := '0000000000';
  result := mRoll.SelectDialog2(True, result);
  mOLE := nil;
end;
...
begin
  ...
  if Sender.Tag = -1 then begin
    NxShowMessage(Application.Title, 'You are not allowed to change data.', mdInformation, False, mSite.GetSiteAppForm);
    Exit;
  end;
  ...
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('m2actLSRollValue'))).Visible := True;
  ...
  TAction(mListBox.Items.Objects(mListBox.Items.IndexOf('m2actLSRollValue'))).Enabled := True;
...
procedure actLSRollValue(Sender: TAction);
var mSite: TSiteForm;
  mListBox: TListBox;
  mBO, mVarBO:TNxCustomBusinessObject;
  mOLE, mRoll:Variant;
  mName, mID:string;
  mAllowedIDs:TStringList
begin
  if Sender.Enabled then begin
    //mDataSet := TDataSet(IntToObj(Sender.Tag));
    mSite := NxFindSiteForm(Sender);
    ...
    if not(NxIsEmptyOID(mDataset.FieldByName('Property_ID').AsString)) then begin
      ...
      mBO.Load(mDataset.FieldByName('Property_ID').AsString,nil);
      if mbo.GetFieldValueAsInteger('X_TypeOfValue')=1 then begin
         //NxShowSimpleMessage(TBusRollSiteForm(TComponent(Sender).BusRollSite).CurrentObject.GetFieldValueAsString('Name'),nil);
         mAllowedIDs:=TStringList.create;
         mOS.SQLSelect('Select id from defrolldata where clsid='+QuotedStr(mBO.GetFieldValueAsString('X_BOCLSID'))+
                       ' and X_Assortmentgroupslist like ' +
                       Quotedstr('%'+TBusRollSiteForm(TComponent(Sender).BusRollSite).CurrentObject.GetFieldValueAsString('StoreAssortmentGroup_ID')+'%'),mAllowedIDs);
         mOLE:=GetAbraOLEApplication;
         mRoll:=mOLE.GetRoll(mBO.GetFieldValueAsString('X_RollCLSID'),0);
         if mAllowedIDs.count>0 then
           mRoll.Params.add('_Allowed=' + mAllowedIDs.CommaText);
         mID := mRoll.SelectDialog2(True, mID);
         if not(NxIsEmptyOID(mID)) then begin
            mVarBO:=mOS.CreateObject(mBO.GetFieldValueAsString('X_BOCLSID'));
            mVarBO.Load(mID,nil);
            mName:=mVarBO.GetFieldValueAsString('Name');
         end;
      end;
      mDataSet.Edit;
      if mDataSet.State in [dsInsert, dsEdit] then begin
        mDataSet.FieldByName('RollValueName').AsString :=  mName;
        mDataSet.FieldByName('RollValueID').AsString :=  mID;
      end;
    end;
  end;
end;
...
begin
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('m2actLSRollValue'))).Visible := False;
  ...
  TAction(aListBox.Items.Objects(aListBox.Items.IndexOf('m2actLSRollValue'))).Enabled := False;
...
var mListBox: TListBox;
  //mDataSet: TMemoryDataSet;
  mID: string;
  mBO, mStoreCardBO, mParamBO: TNxCustomBusinessObject;
      ...
      while not mDataSet.Eof do begin
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            if not(NxIsEmptyOID(mBO.GetFieldValueAsString('X_parameter_ID'))) then begin
              mParamBO:=ASite.BaseObjectSpace.CreateObject('KCAWICC3H2O4DG0YEDMLO4X0PK');
              mParamBO.load(mbo.GetFieldValueAsString('X_parameter_ID'),nil);
              if mParamBO.GetFieldValueAsInteger('X_TypeOfValue')=0 then begin
                 mBo.SetFieldValueAsString('X_ParamValue',mDataSet.FieldByName('ParamValue').AsString);
                 mBo.SetFieldValueAsString('X_RollValueID','');
                 mBo.SetFieldValueAsString('X_RollValueName','');
                 mBo.SetFieldValueAsString('X_BOCLSID','');
                 mBo.SetFieldValueAsFloat('X_NumericValue',0);
                 mBo.SetFieldValueAsBoolean('X_BooleanValue',false);
                 mBO.SetFieldValueAsBoolean('X_Variantni_polozka', false);
              end;
              if mParamBO.GetFieldValueAsInteger('X_TypeOfValue')=1 then begin
                 mBo.SetFieldValueAsString('X_ParamValue','');
                 mBo.SetFieldValueAsString('X_RollValueID',mDataSet.FieldByName('RollValueID').AsString);
                 mBo.SetFieldValueAsString('X_RollValueName',mDataSet.FieldByName('RollValueName').AsString);
                 mBo.SetFieldValueAsString('X_BOCLSID',mParamBO.GetFieldValueAsString('X_BOCLSID'));
                 mBo.SetFieldValueAsFloat('X_NumericValue',0);
                 mBo.SetFieldValueAsBoolean('X_BooleanValue',false);
                 mBO.SetFieldValueAsBoolean('X_Variantni_polozka', mDataSet.FieldByName('VariantBool').AsBoolean);
              end;
              if mParamBO.GetFieldValueAsInteger('X_TypeOfValue')=2 then begin
                 mBo.SetFieldValueAsString('X_ParamValue','');
                 mBo.SetFieldValueAsString('X_RollValueID','');
                 mBo.SetFieldValueAsString('X_RollValueName','');
                 mBo.SetFieldValueAsString('X_BOCLSID','');
                 mBo.SetFieldValueAsFloat('X_NumericValue',mDataSet.FieldByName('NumericValue').AsFloat);
                 mBo.SetFieldValueAsBoolean('X_BooleanValue',false);
                 mBO.SetFieldValueAsBoolean('X_Variantni_polozka', false);
              end;
              if mParamBO.GetFieldValueAsInteger('X_TypeOfValue')=3 then begin
                 mBo.SetFieldValueAsString('X_ParamValue','');
                 mBo.SetFieldValueAsString('X_RollValueID','');
                 mBo.SetFieldValueAsString('X_RollValueName','');
                 mBo.SetFieldValueAsString('X_BOCLSID','');
                 mBo.SetFieldValueAsFloat('X_NumericValue',0);
                 mBo.SetFieldValueAsBoolean('X_BooleanValue',mDataSet.FieldByName('BooleanValue').AsBoolean);
                 mBO.SetFieldValueAsBoolean('X_Variantni_polozka', false);
              end;
            ...
            mBO.SetFieldValueAsDateTime('X_DateTimeOfLastChange',Now);
            mBO.Save;
          finally
            mBO.Free;
          end;
...
begin
  mSQLGetAllParams := ' Select PAR.ID as ID,' +
                      ...
                      ' PAR.X_RollValueID as RollValueID, '+
                      ' PAR.X_RollValueName as RollValueName, ' +
                      ' PAR.X_NumericValue as NumericValue, '+
                      ' PAR.X_BooleanValue as BooleanValue, ' +
                      ' PAR.X_Variantni_polozka as VariantBool '+
                      ' from DefRollData PAR ' +
                      ' LEFT JOIN DefrollData D ON d.ID=PAR.X_parameter_ID '+
                      ' where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) +
                      ' and PAR.X_rel_def='+QuotedStr(cRelDef)+
                      ' and PAR.X_Value_ID=%s order by PAR.X_PosIndex';
  ...
  try
    ...
    if mResult.Active then begin
      ...
      while not mResult.eof do begin
        ...
        mDataSet.FieldByName('RollValueName').AsString:=mResult.FieldByName('RollValueName').AsString;
        mDataSet.FieldByName('RollValueID').AsString:=mResult.FieldByName('RollValueID').AsString;
        mDataSet.FieldByName('NumericValue').AsFloat:=mResult.FieldByName('NumericValue').AsFloat;
        if mResult.FieldByName('BooleanValue').AsString='A' then
          mDataSet.FieldByName('BooleanValue').AsBoolean:=true
        else
          mDataSet.FieldByName('BooleanValue').AsBoolean:=false;
        if mResult.FieldByName('VariantBool').AsString='A' then
          mDataSet.FieldByName('VariantBool').AsBoolean:= True
        else
          mDataSet.FieldByName('VariantBool').AsBoolean:= false;
        mCanLookup := true;
        mDataSet.Post;
        mResult.Next;
      end;
      ...
      if not NxIsEmptyOID(mRollValue) then begin
        ...
        if mRollValueName = '' then begin
          ...
          try
            ...
            DataSet.FieldByName('RollValueName').AsString := mBO.GetFieldValueAsString('X_VALUE_' + mLang);
          finally
            mBO.Free;
          end;
        end else DataSet.FieldByName('RollValueName').AsString := mRollValueName;
      end else
        DataSet.FieldByName('RollValueName').AsString := '';
    except
      DataSet.FieldByName('RollValueName').AsString := '?';
    end; }
  end;
...
begin
  ...
  if (Sender.Name = 'gcRollValueName') then
    AReadOnly := True;
end;
