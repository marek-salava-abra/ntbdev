      ...
      try
        ...
        if ProcessJSONData(mQueueBO, mErrLog) then begin
          mRI_ID:= OS.SQLSelectFirstAsString(
            ...
            ' WHERE (exists (SELECT 1 FROM USERDATA WHERE FIELDCODE='+IntToStr(mRIBO.GetFieldCode('U_CZIssuedInvoice_ID'))+
          ...
          if NxIsEmptyOID(mResultJSON.S['id']) then begin
            LogInfoStr:= LogInfoStr + nxCrLf + mQueueName + ' - Nepodařilo se provést aktualizaci čísla dokladu FV v ABRA CZ.';
