...
var
 ...
 i,j:integer;
 mReceiptCardBO, mReceiptCardRowBO, mFPRowBO:TNxCustomBusinessObject;
 mRCRows, mFPRows:TNxCustomBusinessMonikerCollection;
 mReceiptCard_ID, mReceiptCardRow_ID, mFPDocQueue_ID, mDivision_ID:string;
 mImportManager: TNxDocumentImportManager;
 mInputParams: TNxParameters;
 mParam: TNxParameter;
 mFPAmount, mExtraAmount:Extended;
begin
  Result:=false;
  mOS:=aBO.ObjectSpace;
    ...
    try
      ...
      for i:=0 to mJSONData.A['Rows'].Length-1 do begin
        if mJSONData.A['Rows'].O[i].I['RowType']=3 then begin
          ...
          if NxIsEmptyOID(mReceiptCardRow_ID) then
            mReceiptCardRow_ID:=mOS.SQLSelectFirstAsString('select id from storedocuments2 where flowtype=''20'' and ProvideRow_ID='+QuotedStr(mJSONData.A['Rows'].O[i].S['IORowID']),'');
          if not(NxIsEmptyOID(mReceiptCardRow_ID)) then
            begin
            try
            ...
            if mJSONData.A['Rows'].O[i].S['StoreCardCode'] in ['P'] then begin
              mExtraAmount:=mExtraAmount+mJSONData.A['Rows'].O[i].D['TAmountWithoutVAT'];
            end;
          end;
        end;
      end;
      if (mReceiptCardRowList.count>0) and NxIsBlank(ErrorMessage) then begin
        ...
        try
          ...
          mImportManager.OutputDocument.SetFieldValueAsString('U_CZIssuedInvoice_ID',mJSONData.S['DocumentID']);
          ...
          mImportManager.OutputDocument.SetFieldValueAsString('Period_ID', GetPeriodID(mOS,mJSONData.DT8601['VATDate$DATE']));
          ...
          if mExtraAmount>0 then begin
            mFPRows:=mImportManager.OutputDocument.GetLoadedCollectionMonikerForFieldCode(mImportManager.OutputDocument.GetFieldCode('Rows'));
            mDivision_ID:=mFPRows.BusinessObject[0].GetFieldValueAsString('Division_ID');
            mFPRowBO:=mFPRows.AddNewObject;
            mFPRowBO.prefill;
            mFPRowBO.SetFieldValueAsString('VATIndex_ID','~000000301');
            mFPRowBO.SetFieldValueAsFloat('TAmount',mExtraAmount);
            mFPRowBO.SetFieldValueAsString('Division_ID',mDivision_ID);
            mFPRowBO.SetFieldValueAsString('Text', 'Transport');
          end;
          ...
          if mImportManager.OutputDocument.GetFieldValueAsFloat('Amount') = mJSONData.D['Amount'] then begin
            ...
            ErrorMessage:= ErrorMessage+nxCrLf+'Nesouhlasí ceny mezi FV '+mJSONData.S['DocumentName']+' na částku '+FloatToStr(mJSONData.D['Amount'])+' a FP na částku '+FloatToStr(mFPAmount)+nxCrLf+
                           ' doprava je v hodnotě: '+FloatToStr(mExtraAmount);
            Result:= false;
          end;
        except
          ErrorMessage:=ErrorMessage+#13#10+ExceptionMessage;
