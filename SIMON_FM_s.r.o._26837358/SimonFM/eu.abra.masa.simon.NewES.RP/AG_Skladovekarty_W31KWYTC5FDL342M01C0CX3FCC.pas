...
begin
  try
    mListBox := TListBox(NxFindChildControl(NxFindSiteForm(APageControl), 'RPComponents'));

    Result := TTabSheet.Create(APageControl);
    Result.PageControl := APageControl;
    Result.Caption := '&Souvis. produkty';
    Result.Name := 'tabParamsRP';
    ...
    mGrd := TMultiGrid.Create(NxFindChildControl(NxFindSiteForm(APageControl), 'RPComponents'));
    mGrd.Parent := mPnl;
    mGrd.Name := 'grdParams';
    ...
    mGrd.AddColumn(mGridObjectRollColumn);
    mGrd.OnGetColumnReadOnly := @scNameGetColumnReadOnly;
    mGridObjectRollColumn.ClassID := 'S3WZQKDB5FDL342M01C0CX3FCC';
    ...
    mGridColumn.Caption := 'Název karty';
...
begin
  ...
  if not NxIsEmptyOID(mStorecardID) then begin
    ...
    ' LEFT JOIN StoreCards SC ON SC.ID=PAR.X_StoreCard_ID '+
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(mSite, 'RPComponents'));
  ...
  setParamsEnabled(TWinControl(mListBox.Items.Objects(mListBox.Items.IndexOf('tabParamsRP'))), True);
end;
...
begin
  ...
  setParamsEnabled(TWinControl(aListBox.Items.Objects(aListBox.Items.IndexOf('tabParamsRP'))), False);
...
begin
  ...
  mListBox := TListBox(NxFindChildControl(ASite, 'RPComponents'));
        ...
        if not (mMarkForDelete) then begin //Bude se vytvaret nebo upravovat...
          ...
          try
            ...
            //GlobParams.GetOrCreateParam(dtString, 'ParStorecardType_ID').AsString := '';
             mStoreCardBO:=ASite.BaseObjectSpace.CreateObject('C3V5QDVZ5BDL342M01C0CX3FCC');
            ...
            mBO.SetFieldValueAsString('X_StoreCard_ID', mDataSet.FieldByName('Property_ID').AsString);
...
begin
  ...
  if pgcDataViews.ActivePage <> TTabSheet(pgcDataViews.FindChildControl('tabParamsRP')) then Exit;
    ...
    try
      ...
      if not NxIsEmptyOID(mPropertyID) then begin
        ...
        if mPropertyName = '' then begin
          mBO := mObjSpace.CreateObject('C3V5QDVZ5BDL342M01C0CX3FCC');
          try
