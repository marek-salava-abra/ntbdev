...
const
  ...
  gSkladTermResponseCode        = 'SkladTermResponseCode';
  gSkladTermResponseContentType = 'SkladTermResponseContentType';
  gSkladTermResponseBody        = 'SkladTermResponseBody';
...
var
  ...
  gSkladTermUser_ID : String = '';
  gSkladTermModule  : String = '';
  gSkladTermDocType : String = '';
...
begin
  ...
  try
    ...
    begin
      ...
      if ((mMajorVersion = 22) and (mMinorVersion >= 1))
          or (mMajorVersion >= 23) then
        Result := True
    end
    else
      RaiseException('');
...
function VerifyLicence(AOS: TNxCustomObjectSpace; ADeviceId: String): Boolean;
begin
  if CheckLicence then
    Result := not NxIsBlank(ADeviceId) and CheckLicense(AOS, ADeviceId, '7')
  else
    Result := True;

  if not Result then
    RaiseException(Format(getString('not_licenced'), [ADeviceId]));
end;

////////////////////////////////////////////////////////////////////////////////
//vraci OK, pokud je autorizace v poradku (spravne jmeno a heslo)
function HTTP_Authorization(AOS: TNxCustomObjectSpace; AAuthorization: String;
  AResponse: TStringList): Boolean;
var
  sInputDecodeAuth, sRequireAuth: String;
  ErrDesc: string;
begin
  glog.WriteEventFmt(logDebug, 'Authorization:%s', [AAuthorization]);
  ...
  Result := false;
  ErrDesc := '';
  if(pos('Basic ', AAuthorization) = 1) then
  begin
    AAuthorization := trim(copy(AAuthorization, Length('Basic ')+1, 100));
    ...
    begin
      sInputDecodeAuth := TEncoding.ANSI.GetString(DecodeBase64(AAuthorization));
    ...
    begin
      ...
      glog.WriteEventFmt(logWarning, 'Authorization: Error %s', [trim(TEncoding.ANSI.GetString(DecodeBase64(AAuthorization)))])
    end;
        //'('+
    ...
    ErrDesc := 'Authorization: ErrorHeader (' + AAuthorization + ')';

  if not Result then
    SetPlainResponse(AResponse, ErrDesc, HTTP_SC_Unauthorized);
end;
////////////////////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////////////////////////////////
//odpoved
procedure HTTPWSResponse(AResponse: TNxHTTPResponse; AToFill: TStringList);
var
  i: Integer;
begin
  glog.WriteEventFmt(logDebug, 'Response:%s', [AToFill.Values(gSkladTermResponseBody)]);
  AResponse.StatusCode := StrToInt(AToFill.Values(gSkladTermResponseCode));
  AResponse.Content.ContentType := AToFill.Values(gSkladTermResponseContentType) + '; charset=UTF-8';
  if(Length(AToFill.Values(gSkladTermResponseBody)) > 0)then
    AResponse.Content.Content := TEncoding.UTF8.GetBytes(AToFill.Values(gSkladTermResponseBody));
...
begin
  ...
  gLog.WriteEvent(logDebug, NxPadR('',  2 * (gLogSectionIndex - 1), '.') + Section);
end;
////////////////////////////////////////////////////////////////////////////////
...
begin
  ...
  if gLogSectionIndex = 0 then
  begin
    LogWriteEvent('ERROR: Trying to end non started section');
    exit;
  end;

  gLog.WriteEventFmt(logDebug, '%s%s - (%n s)',
    [NxPadR('',  2 * (gLogSectionIndex - 1), '.'),
...
procedure LogWriteEvent(AMessage: String);
begin
  try
    if Assigned(gLog) and (gLog <> nil) then
      gLog.WriteEvent(logDebug, NxPadR('', 2 * (gLogSectionIndex - 1), '.') + AMessage);
  except
  end;
end;
...
procedure ParsePath(path: string; slPath: TStringList; AFromAPI: Boolean = False);
var s, adr: string;
    i, len: integer;
begin
  ...
  // pokud je volano z API, tak odstranim vsechno pred opravdovou cestou
  if AFromAPI then
  begin
    path := '/' + copy(path, pos('/U_EntryPoint/API/', path) + Length('/U_EntryPoint/API/'), Length(path));
  end;
...
begin
  case ADocType of
    ...
    DOC_RefundedReceiptCard:            Result := 'StoreDocuments';
    ...
    DOC_JobOrder:                       if ABRA then Result := 'PLMJobOrders' else Result := 'PRFJobOrders';
...
begin
  case ADocType of
    ...
    DOC_RefundedReceiptCard:            Result := Class_RefundedReceiptCard;
...
begin
  docTyp := GetStoreDocType(docID, os);
...
begin
  case AClass_CLSID of
    ...
    Class_MaterialDistribution:   mDocumentType := DOC_MaterialDistribution;
    ...
    Class_RefundedReceiptCard:    mDocumentType := DOC_RefundedReceiptCard;
...
function GetDefaultDocQueue_ID(ADocType: String): String;
begin
  case ADocType of
    DOC_BillOfDelivery:                 Result := RADA_VYDEJKA;
    DOC_IncomingSubstitution:           Result := RADA_IncomingSubstitution;
    DOC_IncomingTransfer:               Result := RADA_PREPRIJEM;
    DOC_IncomingTransformation:         Result := RADA_IncomingTransformation;
    DOC_LogStoreInput:                  Result := LogStoreInput_DocQueue_ID;
    DOC_LogStoreOutput:                 Result := LogStoreOutput_DocQueue_ID;
    DOC_LogStoreTransfer:               Result := LogStoreTransfer_DocQueue_ID;
    DOC_MaterialDistribution:           Result := RADA_MaterialDistribution;
    DOC_OutgoingSubstitution:           Result := RADA_OutgoingSubstitution;
    DOC_OutgoingTransfer:               Result := RADA_PREVYDEJ;
    DOC_OutgoingTransformation:         Result := RADA_OutgoingTransformation;
    DOC_PartialInvProtocol:             Result := RADA_DILCI_INV_PROTOKOL;
    DOC_ReceiptCard:                    Result := RADA_PRIJEMKA;
    DOC_ReceivedOrder:                  Result := RADA_OBP;
    DOC_RefundedBillOfDelivery:         Result := RADA_VRATKA_VYDEJKY;
    DOC_RefundedReceiptCard:            Result := RADA_RefundedReceiptCard;
    DOC_ShippingList:                   Result := RADA_ShippingList;
    else Result := '';
  end;
end;
...
procedure AddTodtDocumentQuantity(dtDocumentQuantity: TMemTable; AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID: String;
  AQuantity, AUnitRate: Double; AContentUnit: String);
var
  Quantity: Double;
begin
  LogWriteSectionStart('AddTodtDocumentQuantity');
  if AUnitRate <> 0 then
    Quantity := AQuantity * AUnitRate
  else
    Quantity := AQuantity;

  if dtDocumentQuantity.FindKey([AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID, AContentUnit]) then
  begin
    ...
    LogWriteEvent(Format('DocumentQuantity - UPDATE - %s, %s, %s, %s',
      [AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID]));
  end
  else begin
    ...
    dtDocumentQuantity.FieldByName('ContentUnit').AsString := AContentUnit;
    dtDocumentQuantity.Post;
    // z nejakeho duvodu nefunguje - Chyba při uložení: Format 'DocumentQuantity - ADD    - %s,' invalid or incompatible with argument
    //LogWriteEvent(Format('DocumentQuantity - ADD    - %s, %s, %s, %s, %n, %n, %s',
    //  [AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID, AQuantity, AUnitRate, AContentUnit]));
    LogWriteEvent(Format('DocumentQuantity - ADD    - %s, %s, %s, %s',
      [AStore_ID, AStoreCard_ID, AStoreBatch_ID, AStorePosition_ID]));
  end;
  LogWriteSectionEnd;
end;

procedure ConfirmLSD(AOS: TNxCustomObjectSpace; AClass: TNxPackedGuid; ALSD_ID, AMethodName: String; AStartTransaction: Boolean = True);
var
    ...
    begin
      ...
      try
        ...
        if AStartTransaction then
          AOS.RollBack;
        if Assigned(gLog) then
          gLog.WriteEvent(logError, AMethodName + ' - LSD MakeExecuted - error - '+ALSD_ID+' - '+ExceptionMessage);
      end;
    end;
  end;
end;
...
begin
  case DB_TYPE of
    ...
    4: Result := '(select listagg(SU.Code || '':'' || SU.UnitRate, '';'') within group (order by SU.PosIndex) from StoreUnits SU where SU.Parent_ID = SC.ID)';
  end;
end;
...
var
  mStatuses: String;
begin
  LogWriteSectionStart('ChangeStatusByRule');
  try
    if ABRA then
      mStatuses := SQLSelectStr(ABO.ObjectSpace,
        'select FromState_ID' + CONCAT_STR + ''',''' + CONCAT_STR + 'ToState_ID from PMStatesTransitions where ID = ' + QuotedStr(ARule_ID))
    else
      mStatuses := SQLSelectStr(ABO.ObjectSpace,
        'select UserStatusesFrom_ID' + CONCAT_STR + ''',''' + CONCAT_STR + 'UserStatusesTo_ID from UserStatusesSwitchRules where ID = ' + QuotedStr(ARule_ID));

    LogWriteEvent(Format('Changing status of %s (actual status: %s) by rule %s (from %s to %s)',
      [ABO.OID, ABO.GetFieldValueAsString(GetStatusField), ARule_ID, copy(mStatuses, 1, 10), copy(mStatuses, 12, 10)]));
// ABRA Start
...
begin
  case ADocType of
    ...
    DOC_RefundedReceiptCard:            Result := rtRefundedReceiptCard_Doc;
    DOC_MainInvProtocol:                Result := rtMainInvProtocol_Doc;
    DOC_PartialInvProtocol:             Result := rtPartialInvProtocol_Doc;
...
function GetValueOrDefault(AValue_ID, ADefaultValue_ID: String): String;
begin
  if not CFxOID.IsEmpty(AValue_ID) then
    Result := AValue_ID
  else
    Result := ADefaultValue_ID;
end;

function CheckProfilerStart(AConfigPath: String): Boolean;
var
  mConfig: TStringList;
  mLastDate, mNow: TDateTime;
  mYearLast, mMonthLast, mDayLast, mHourLast, mMinLast, mSecLast, mMilsecLast,
    mYear, mMonth, mDay, mHour, mMin, mSec, mMilsec: Word;
begin
  Result := False;
  if FileExists(AConfigPath) then
  begin
    mConfig := TStringList.Create;
    try
      mConfig.LoadFromFile(AConfigPath);
      if (mConfig.Count > 0) then
      begin
        // all - profiluje se vse
        if mConfig.Strings[0] = 'all' then
        begin
          Result := True;
        end
        // hour - profiluje se prvni pozadavek kazdou hodinu
        else if mConfig.Strings[0] = 'hour' then
        begin
          mNow := Now;

          // na druhym radku v konfiguraci je datum v ISO - kdy se naposledy profilovalo
          if (mConfig.Count > 1) then
          begin
            mLastDate := CFxDateTime.ISO8601ToDateTime(mConfig.Strings[1]);

            // kontroluju, ze je z minulosti (kdybych chtel treba zacit profilovat az nekdy v budoucnu)
            if CompareDateTime(mLastDate,  mNow) < 0 then
            begin
              // dekoduju obe data a zkontroluju, ze je nova hodina
              DecodeDateTime(mLastDate, mYearLast, mMonthLast, mDayLast, mHourLast, mMinLast, mSecLast, mMilsecLast);
              DecodeDateTime(mNow, mYear, mMonth, mDay, mHour, mMin, mSec, mMilsec);

              if (mYear > mYearLast) or (mMonth > mMonthLast) or (mDay > mDayLast) or (mHour > mHourLast) then
              begin
                mConfig.Strings[1] := CFxDateTime.DateTimeToISO8601(mNow);
                mConfig.SaveToFile(AConfigPath);
                Result := True;
              end;
            end;
          end
          // v konfigu neni zadne datum, takze pozadavek profiluju a ulozim datum
          else
          begin
            mConfig.Add(CFxDateTime.DateTimeToISO8601(mNow));
            mConfig.SaveToFile(AConfigPath);
            Result := True;
          end;
        end;
...
procedure SetResponse(AResponse: TStringList; ABody: String; AContentType: String = ''; ACode: Integer = 200);
begin
  AResponse.Add(gSkladTermResponseBody + '=' + ABody);

  if(AContentType = '') then
    AResponse.Add(gSkladTermResponseContentType + '=' + ContentType_JSON)
  else
    AResponse.Add(gSkladTermResponseContentType + '=' + AContentType);

  AResponse.Add(gSkladTermResponseCode + '=' + IntToStr(ACode));
end;

procedure SetPlainResponse(AResponse: TStringList; ABody: String; ACode: Integer = 200);
begin
  SetResponse(AResponse, ABody, ContentType_PlainText, ACode);
end;

procedure HTTPAPIResponse(AResponse: TAPIResponse; AToFill: TStringList);
begin
  gLog.WriteEventFmt(logDebug, 'Response:%s', [AToFill.Values(gSkladTermResponseBody)]);
  AResponse.Status := StrToInt(AToFill.Values(gSkladTermResponseCode));
  AResponse.SetHeader('Content-Type', AToFill.Values(gSkladTermResponseContentType) + '; charset=UTF-8');
  if(Length(AToFill.Values(gSkladTermResponseBody)) > 0) then
    AResponse.Body := AToFill.Values(gSkladTermResponseBody);
  gLog.WriteEvent(logDebug, 'Response OK');
end;
