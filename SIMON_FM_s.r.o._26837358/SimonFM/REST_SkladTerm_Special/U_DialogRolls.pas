...
begin
  case ARollName of
    ...
    'Store': StoreList(AOS, AModule, ADocType, AUser_ID, ASearchStr, AInputValues, AList);
  end;
...
procedure StoreList(AOS: TNxCustomObjectSpace; AModule, ADocType, AUser_ID, ASearchStr: String; var AInputValues, AList: TMemTable);
var
  mSql: String;
begin
    mSql :=
      ...
      'where S.Hidden = ''N''  and S.Code in (''01-VO'',''05'',''MO-II'',''555'') ';

  // pripadne hledani
  if trim(ASearchStr) <> '' then
  begin
    mSql := mSql +
      ' and (S.Code' + COLLATION_AI + 'like ''%' + ASearchStr + '%''' + nxCrLf +
      '  or S.Name' + COLLATION_AI + 'like ''%' + ASearchStr + '%'')';
  end;
  mSql := mSql + 'order by S.Code';
  mSql := mSql + FIRST_TOP_ORACLE(100);
  AOS.SQLSelect2(mSql, AList);
end;
