  ...
  if not NxIsEmptyOID(mGroup_ID) then begin
    ...
    try
      mSQL := 'Select PAR.ID as ID,' +
              ...
              ' from DefRollData PAR ' +
              ' LEFT JOIN DefrollData D ON d.ID=PAR.X_parameter_ID '+
              ' where PAR.CLSID=' + QuotedStr(cBOPropertyCLSID) + 'and PAR.X_rel_def='+QuotedStr('01')+' and PAR.X_Value_ID='+QuotedStr(mGroup_ID)+' order by PAR.X_PosIndex';
      mObjSpace.SQLSelect2(mSQL, mTmpDts);
      if mTmpDts.Active then begin
        mTmpDts.First;
        while not mTmpDts.Eof do begin
