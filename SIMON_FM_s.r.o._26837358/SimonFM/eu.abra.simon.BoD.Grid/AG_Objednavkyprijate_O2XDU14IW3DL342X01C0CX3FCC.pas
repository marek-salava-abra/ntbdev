...
begin
  //mMG := TMultiGrid(TWinControl(Self.FindChildControl('tabRows')).FindChildControl('grdRows'));
  ...
  if Assigned(mMG) then begin
    ...
    for i:=mMG.ColumnCount-1 downto 0 do
      if mMG.Columns[i].FieldName = 'U_info' then
        b := False;
    if b then begin
      mFieldDef := TFieldDef.Create(mMG.DataSource.DataSet.FieldDefs, 'U_info', ftWideString, 0, False, 306);
    ...
    for i:=mMG.ColumnCount-1 downto 0 do
      if mMG.Columns[i].FieldName = 'U_marze' then
        b := False;
    if b then begin
      mFieldDef := TFieldDef.Create(mMG.DataSource.DataSet.FieldDefs, 'U_marze', ftFloat, 0, False, 303);
...
begin
  Result := nil;
  //mMG := TMultiGrid(TWinControl(ASiteForm.FindChildControl('tabRows')).FindChildControl('grdRows'));
