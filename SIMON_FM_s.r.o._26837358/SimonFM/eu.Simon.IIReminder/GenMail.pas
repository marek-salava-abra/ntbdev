...
procedure GenMail00 (mObjectSpace: TNxCustomObjectSpace; var Success: Boolean; var LogInfoStr: String);
  ...
  begin
      ...
      begin
        ...
        try
          ...
          mSQL:= 'SELECT II.ID ' +
                 ...
                 'WHERE not(II.docqueue_id=''1Z10000101'') and ((II.Amount - ii.CreditAmount- II.PaidAmount) > 0) ' +
                 '       AND ' +
            ...
            begin
              ...
              for i:= 0 to (mIIList.count() - 1) do
                begin
                  ...
                  mMessage:=mMessage+#13#10+mIIBO.GetFieldValueAsString('Firm_ID.Name');
                  ...
                  if (mIIBO.GetFieldValueAsFloat('Amount') - mIIBO.GetFieldValueAsFloat('PaidAmount')) > 0 then
                    begin
                      ...
                      //if not(NxIsValidEMail(mFirmMail,False)) then mFirmMail:= 'BezEmailu@ambg.cz';
                      //mFirmMail:='mario.zizka@simonfm.cz';
                      ...
                      mMail_ID:= SendInternalMail(mObjectSpace, mFirmMail, '', 'marek.zizka@simonfm.cz', mSubject, mMailText,
                      ...
                      LogInfoStr:= LogInfoStr +'faktura '+mIIBO.DisplayName+'  '+ mIIBO.GetFieldValueAsString('Firm_ID.name') + ' email pro upomínky ' + mFirmMail + Chr(10) + Chr(13);
    ...
    Success:= True;
    //LogInfoStr:='Cilové firmy '+#13#10+mMessage;
end;
 ...
 mMessage:='';
  ...
  if mFirmList.count>0 then begin
    for i:=0 to mFirmList.count-1 do begin
       ...
       OS.SQLSelect('select a.id from IssuedInvoices A left join firms f on f.id=A.firm_id where not(a.docqueue_id=''1Z10000101'') and (F.ID='+Quotedstr(mFirmList.strings[i])+' OR ((F.Firm_ID IS NOT NULL) AND (F.Firm_ID='+Quotedstr(mFirmList.strings[i])+'))) '+
                    'AND (((((A.Amount>=0) and ((A.PaidAmount<=0) and ((A.Amount + A.PaidCreditAmount - A.PaidAmount - A.CreditAmount) > 0))) or '+
       ...
       if mInvoiceList.count>0 then begin
         ...
         for j:=0 to mInvoiceList.count-1 do begin
           ...
           if i=0 then mMessage:=mMessage+#13#10+mInvoiceBO.GetFieldValueAsString('firm_ID.Name');
           ...
           if mInvoiceBO.GetFieldValueAsDateTime('DueDate$Date')<(date-mOffset) then begin
             mInvoiceMessage:='<tr>'+
                              '<td><i><a href="https://api.simonfm.cz/SimonFM/issuedinvoices/'+mInvoiceBO.OID+'.pdf?select=id&report=4VE1000101&Auth=UHJpbnRJbnZvaWNlOlRpc2thcg">'+mInvoiceBO.DisplayName+'</a></i></td>'+
           ...
           if mInvoiceBO.GetFieldValueAsDateTime('DueDate$Date')=(date-mOffset) then begin
             mInvoiceMessage:='<tr>'+
                              '<td><a href="https://api.simonfm.cz/SimonFM/issuedinvoices/'+mInvoiceBO.OID+'.pdf?select=id&report=4VE1000101&Auth=UHJpbnRJbnZvaWNlOlRpc2thcg">'+mInvoiceBO.DisplayName+'</a></td>'+
         ...
         SendInternalMail(OS, mTO, '', 'marek.zizka@simonfm.cz', mSubject, mBody,
                      '',  mFirm_ID, mDivision_ID, mBusOrder_ID, mReplyTo);
       end;
    end;
  end;
  LogInfoStr:='Cílové firmy'+#13#10+mMessage;
  Success:= True;
end;
