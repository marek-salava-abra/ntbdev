...
procedure UpdatePrices2(AStoreCardID: string; APrice, AUnitRate: Extended; APriceID, AQUnit, APriceListID : string; AOS: TNxCustomObjectSpace);
var
...
procedure UpdatePrices(AStoreCardID: string; APrice1, AUnitRate: Extended; APriceID, AQUnit, APriceListID : string;
  AObjectSpace: TNxCustomObjectSpace);
var
  mSQL: string;
  mStorePriceID: string;
  //mStorePrice: TNxHeaderBusinessObject;
  mStorePrice: TNxCustomBusinessObject;

  function iSelect(ASQL: string; var AID: string): Boolean;
  var
    mStrings: TStringList;
  begin
    Result := False;
    ...
    try
      ...
      Result := mStrings.Count > 0;
      if Result then
        AID := mStrings.Strings[0];
    finally
      mStrings.Free;
    end;
  end;

  procedure iAddPrice(APriceID, AStorePriceID: string; APrice, AUnitRate: Extended; AQunit:string);
  var
    mItemID: string;
    mItem: TNxCustomBusinessObject;
  begin
    mSQL := 'select ID from StorePrices2 where Parent_ID = ''%s'' and Price_ID = ''%s'' and QUnit = ''%s''';
    mItem := AObjectSpace.CreateObject('GHYLVQXQ3FE13DQC01C0CX3F40');
    try
      if not iSelect(Format(mSQL, [AStorePriceID, APriceID,AQunit]), mItemID) then
      begin
        mItem.New;
        mItem.Prefill;
        mItem.SetFieldValueAsString('Parent_ID', AStorePriceID);
        mItem.SetFieldValueAsString('Price_ID', APriceID);
        mItem.SetFieldValueAsString('QUnit', AQunit);
        mItem.SetFieldValueAsFloat('UnitRate', AUnitRate);
        mItem.SetFieldValueAsFloat('Amount', APrice);
        mItem.Save;
      end else
      begin
        mItem.Load(mItemID, nil);
        mItem.SetFieldValueAsFloat('Amount', APrice);
        mItem.Save;
      end;
    finally
...
begin
  mSQL := 'select first 1 sp.ID from storeprices sp left join PriceListValidities pv on sp.pricelistvalidity_id=pv.id where sp.PriceList_ID = ''%s'' and sp.StoreCard_ID = ''%s'' and sp.DeletedFromPriceList = ''N'' order by pv.validfromdate$date desc';
  //mSQL := 'select ID from StorePrices where PriceList_ID = ''%s'' and StoreCard_ID = ''%s'' and DeletedFromPriceList = ''N''';
 // mStorePrice := TNxHeaderBusinessObject(AObjectSpace.CreateObject('GDYLVQXQ3FE13DQC01C0CX3F40'));
 mStorePrice := AObjectSpace.CreateObject('GDYLVQXQ3FE13DQC01C0CX3F40');
  try
    if iSelect(Format(mSQL, [APriceListID, AStoreCardID]), mStorePriceID) then
      mStorePrice.Load(mStorePriceID, nil)
    else
    begin
      mStorePrice.New;
      ...
      mStorePrice.SetFieldValueAsString('StoreCard_ID', AStoreCardID);
      mStorePrice.Save;
      mStorePriceID := mStorePrice.OID;
    end;
    iAddPrice(APriceID, mStorePriceID, APrice1, AUnitRate,AQUnit);
    //iAddPrice(cCenaVOID, mStorePriceID, APrice2);
  finally
    mStorePrice.Free;
  end;
end;
